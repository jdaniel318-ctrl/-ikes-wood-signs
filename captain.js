(() => {
  ...
  const UPPER_COMMAND_BUILD='8.8.8';
...
/* 8.8.8 Bootstrap Admiral Fleet — canonical vessel identity, Admiral-controlled. */
;(()=>{
  ...
  install();installCommissionBrand();window.DarkSkySyncVesselBrandIdentity=sync;window.DarkSkyReadAdmiralFleet=()=>vessels.map(vessel=>({...vessel,logo_url:publicLogo(vessel.logo_storage_path)||defaultLogo(vessel.project_id)}));
...
/* 8.8.8 Bootstrap Build — local-first construction schedule proving ground. */
;(()=>{
...
/* 8.8.8 Bootstrap Admiral Fleet — permanent keel, editable working name. */
;(()=>{
  const SESSION_KEY='darkSkySupabaseAdmiralSessionV1';
