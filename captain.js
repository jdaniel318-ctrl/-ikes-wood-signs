/* 8.8.15.8 Fleet Runtime — a full document load must never inherit
...
;(() => {
  ...
  window.DarkSkyAdmiralFreshGate=Object.freeze({build:'8.8.15.8',requiredPerDocument:true,engineAuthoritySeparate:true});
})();
/* Dark Sky 8.8.15.8 Fleet Runtime — source-aware read-only Admiral observability. */
;(()=>{
  const $=id=>document.getElementById(id),safe=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c])),PROGRAM={project_id:'bootstrap-build',display_name:'Bootstrap Build',mission_class:'admiral_program',operating_model:'fleet_operated',ownership_model:'fleet_unassigned',lifecycle_state:'commissioning',logo_url:'bootstrap_build_icon.png',current_work:'Construction scheduling proving ground',truth_source:'Bundled Admiral Program manifest'};
  ...
  async function load(){const status=$('watchStatus');status.textContent='Verifying Admiral identity for a read-only fleet view…';try{const verified=await window.DarkSkyAdmiralIdentityStatus?.();$('watchIdentity').classList.toggle('is-verified',!!verified);['watchEmail','watchPassword','watchSignIn'].forEach(id=>$(id)?.closest('label')?.classList.toggle('hidden',!!verified));$('watchSignIn').classList.toggle('hidden',!!verified);$('watchSignOut').classList.toggle('hidden',!verified);$('watchTools').classList.toggle('hidden',!verified);if(!verified){$('watchRows').innerHTML='<article class="bootstrap-fleet-empty"><div><b>Fleet Watch is awaiting an authenticated read</b><span>Authenticate the Admiral account. No vessel state will change.</span></div></article>';$('watchPrograms').innerHTML='';status.textContent='Admiral account authentication is required. Fleet Watch remains closed.';return;}await window.DarkSkySyncVesselBrandIdentity?.();const truth=await window.DarkSkyReadAdmiralWatchTruth?.();lastSync=new Date().toISOString();render(Array.isArray(truth)?truth:(window.DarkSkyReadAdmiralFleet?.()||[]));}catch(error){status.textContent=error?.message||'Fleet Watch could not be read. No data changed.';}}
  function open(){const fleet=$('bootstrapFleet');fleet.classList.remove('hidden');fleet.setAttribute('aria-hidden','false');document.body.classList.add('bootstrap-fleet-open');fleet.scrollTop=0;$('watchStatus').textContent='Fleet Watch opened. Reading Fleet Core…';requestAnimationFrame(()=>$('watchClose')?.focus({preventScroll:true}));load();}
  function close(){$('bootstrapFleet').classList.add('hidden');$('bootstrapFleet').setAttribute('aria-hidden','true');document.body.classList.remove('bootstrap-fleet-open');}
  function overview(id){const v=id===PROGRAM.project_id?PROGRAM:records.find(x=>x.project_id===id);if(!v)return;const f=facts(v);$('watchOverviewBody').innerHTML=`<header>${mark(v)}<div><small>ADMIRAL OBSERVATION · EXACT PROJECT CONTEXT</small><h2>${safe(v.display_name)}</h2><p>${safe(v.project_id)}</p></div><em>READ ONLY</em></header><section class="fleet-vessel-context"><article><small>LIFECYCLE</small><b>${safe(f.ready)}</b></article><article><small>WATCH FRESHNESS</small><b>${safe(id===PROGRAM.project_id?'PROGRAM / LOCAL':f.freshness)}</b></article><article><small>CURRENT WORK</small><b>${safe(f.work)}</b></article><article><small>UNRESOLVED ISSUES</small><b>${f.issues===null?'Awaiting vessel report':f.issues}</b>${f.issueSummary?`<span>${safe(f.issueSummary)}</span>`:''}</article><article><small>LAST ACTIVITY</small><b>${safe(f.activity?new Date(f.activity).toLocaleString():'Not reported')}</b></article><article><small>TRUTH SOURCE</small><b>${safe(f.source)}</b>${f.reported?`<span>Reported ${safe(new Date(f.reported).toLocaleString())}</span>`:'<span>No vessel telemetry has been published yet.</span>'}</article></section><aside><b>Scoped observation established</b><span>Only ${safe(v.display_name)} (${safe(v.project_id)}) is in context. No other vessel data is visible, and no modifying control is exposed.</span></aside>`;$('watchOverview').classList.remove('hidden');$('watchOverview').setAttribute('aria-hidden','false');$('watchOverviewClose').focus({preventScroll:true});}
  install();window.DarkSkyOpenFleetWatchtower=open;window.DarkSkyOpenAdmiralFleet=open;
  window.addEventListener('darksky:admiral-fleet-updated',e=>{if(!$('bootstrapFleet').classList.contains('hidden'))render(e.detail?.records||[]);});
  document.addEventListener('click',async e=>{const t=e.target;if(t.closest?.('#watchClose'))return close();if(t.closest?.('#watchRefresh'))return load();if(t.closest?.('#watchSignIn')){const b=$('watchSignIn');try{b.disabled=true;b.textContent='AUTHENTICATING…';await window.DarkSkyAuthenticateAdmiral?.($('watchEmail').value,$('watchPassword').value);$('watchPassword').value='';await load();}catch(error){$('watchStatus').textContent=error?.message||'Admiral sign-in failed.';}finally{b.disabled=false;b.textContent='AUTHENTICATE & OPEN';}return;}if(t.closest?.('#watchSignOut')){window.DarkSkySignOutAdmiral?.();return load();}const vessel=t.closest?.('[data-watch-vessel]');if(vessel)return overview(vessel.dataset.watchVessel);if(t.closest?.('#watchOverviewClose')){$('watchOverview').classList.add('hidden');$('watchOverview').setAttribute('aria-hidden','true');$('watchStatus').textContent='Returned to Fleet Watch. No vessel state changed.';return;}const chosen=t.closest?.('[data-watch-filter]');if(chosen){filter=chosen.dataset.watchFilter;document.querySelectorAll('[data-watch-filter]').forEach(b=>{const yes=b===chosen;b.classList.toggle('is-selected',yes);b.setAttribute('aria-pressed',String(yes));});render();}},true);
  document.addEventListener('input',e=>{if(e.target.id==='watchSearch'){query=e.target.value.trim();render();}});
})();
...
(() => {
  ...
  const UPPER_COMMAND_BUILD='8.8.15.8';
  let authorized = false;

  const byId = (id) => document.getElementById(id);
  // Module-wide encoder for every Professional and Signal Report renderer.
  // The Cinematic renderer also keeps its own local helper; neither surface now
  // depends on a sibling function scope being active.
  const captainSafe=value=>String(value??'').replace(/[&<>"']/g,char=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[char]));
  ...
  function applyUpperCommandVisual(slot,blob){
    ...
    if(slot==='admiral'){
      ...
      if(gate){gate.style.removeProperty('--admiral-command-visual');gate.classList.remove('admiral-custom-visual');const status=gate.querySelector('#admiralVisualStatus');if(status)status.textContent='NEUTRAL AUTHORITY SHIELD • FLEET DATA CONCEALED';}
  ...
  let captainSubviewReturnFocus=null;
  let captainSignalReturnFocus=null;

  function enterCaptainSubview(id) {
    captainSubviewReturnFocus=document.activeElement instanceof HTMLElement?document.activeElement:null;
    hide('captainGlobalExit');
    show(id);
    requestAnimationFrame(()=>{
      const view=byId(id);
      const focusTarget=view?.querySelector?.('.captain-round-close,button,[href],[tabindex]:not([tabindex="-1"])');
  ...
  function leaveCaptainSubview(id) {
    ...
    const target=captainSubviewReturnFocus;
    captainSubviewReturnFocus=null;
    requestAnimationFrame(()=>{try{target?.focus?.({preventScroll:true});}catch(_){target?.focus?.();}});
  }
  ...
  function unlock() {
    ...
    clearHash();
    document.body.classList.add('captain-modal-open', 'captain-authorized');
    const cq=byId('captainQuarters'); if(cq){cq.dataset.defaultCommandMode='professional';cq.dataset.commandMode='professional';}
  ...
  function secure() {
    ...
    ['fleetSpineWorkspace','admiralDeck','admiralGateOverlay','captainBlueprint','captainFleetChart','captainSpyglassPanel','captainObjectPanel','captainTestAccessGate','captainCommandWorkspace','captainQuarters','captainQuartersGate','captainGlobalExit'].forEach(hide);
    document.body.classList.remove('fleet-spine-open');
    try{window.DarkSkyFleetSpine?.cancel?.('upper-command-secured');}catch(_){}
  ...
  function openSignalReport(projectId){
    ...
    captainSignalReturnFocus=byId('captainFleetVessels')?.querySelector(`[data-captain-vessel="${projectId}"]`)||
      (document.activeElement instanceof HTMLElement?document.activeElement:null);
    report.setAttribute('role','region');
    report.setAttribute('aria-label',`${vessel.name} signal report`);
    report.setAttribute('tabindex','-1');
    report.innerHTML=`<div class="cq-signal-opened" role="status"><i></i><span><b>SIGNAL REPORT OPEN</b><small>Exact vessel scope confirmed</small></span></div><div class="cq-signal-report-head">
    ...
    <div class="cq-signal-vessel-summary">
      <span>${captainSafe(vessel.launchLabel||'VESSEL')}</span>
      ...
      <span>${vessel.ownerState==='claimed'?'OWNER CLAIMED':vessel.ownerState==='invited'?'OWNER INVITED':'OWNER SETUP OPEN'}</span>
    </div>
    <div class="cq-fleet-ready-course"><small>FLEET READY COURSE</small><strong>STEP ${Number(vessel.launchStep)||1} OF 5 · ${captainSafe(vessel.launchLabel||'REVIEW')}</strong><p>${captainSafe(vessel.launchDetail||'Review this vessel’s current operating posture.')}</p></div>
    <div class="cq-signal-outposts">
      ${outposts.length?outposts.map(o=>{
        const reasons=o.attentionReasons||[];
        return `<article class="cq-signal-outpost ${o.state}">
          <i></i>
          <div><strong>${captainSafe(o.name)}</strong><small>${stateLabel(o.state)} • Manifest v${Number(o.manifestVersion)||1}</small><span class="cq-outpost-evidence"><em class="${o.customerTestRecorded?'pass':'open'}">${o.customerTestRecorded?'TEST RECORDED':'TEST PENDING'}</em><em class="${o.customerExperienceApproved?'pass':'open'}">${o.customerExperienceApproved?'EXPERIENCE APPROVED':'EXPERIENCE REVIEW'}</em><em class="${o.deviceLockVerified?'pass':'before'}">${o.deviceLockVerified?'DEVICE VERIFIED':'BEFORE LIVE'}</em></span>${reasons.length?`<p>${reasons.map(captainSafe).join(' • ')}</p>`:''}<p class="cq-outpost-next"><b>NEXT:</b> ${captainSafe(o.nextStep||'Review this outpost in Project Control.')}</p></div>
          <button type="button" data-captain-route-project="${captainSafe(vessel.projectId)}" data-captain-route-outpost="${captainSafe(o.id)}">${captainSafe(o.routeLabel||'OPEN OUTPOST')} <small>Exact route in Engine</small></button>
        </article>`;
    ...
    const closeReport=()=>{
      ...
      const fallback=byId('captainFleetVessels')?.querySelector(`[data-captain-vessel="${projectId}"]`);
      const target=captainSignalReturnFocus?.isConnected?captainSignalReturnFocus:fallback;
      captainSignalReturnFocus=null;
      requestAnimationFrame(()=>{try{target?.focus?.({preventScroll:true});}catch(_){target?.focus?.();}});
    };
    byId('captainSignalClose')?.addEventListener('click',closeReport,{once:true});
    requestAnimationFrame(()=>{try{report.focus({preventScroll:true});}catch(_){report.focus?.();}});
    const status=byId('captainProfessionalStatus');
    if(status)status.textContent=`${vessel.name} signal report opened in exact-vessel scope.`;
  }
  ...
  function refreshCaptainFleetChart(){
    ...
    const positions=[[15,27],[49,18],[83,29],[18,70],[51,68],[82,72]];
    ...
    if(summary) summary.innerHTML=`<strong>FLEET RUNTIME</strong><span>${fleet.length} vessels</span><span>${active} outpost${active===1?'':'s'} sailing</span>${attention?`<span class="attention">${attention} needing attention</span>`:'<span>waters steady</span>'}`;    refreshCaptainWatch();
  }
  ...
  function ensureCinematicHelm(){
    const room=byId('captainQuarters');
    if(!room)return null;
    let helm=byId('captainCinematicHelm');
    if(helm)return helm;
    helm=document.createElement('main');
    helm.id='captainCinematicHelm';
    helm.className='cq-cinematic-helm';
    helm.setAttribute('aria-label','Living cinematic fleet command chart');
    helm.innerHTML=`
      <header class="cq-helm-header">
        <div><small>CAPTAIN'S CHART TABLE · LIVE FLEET POSITION</small><h2>The Living Dark Sky</h2><p>Choose one vessel. Read one signal. Set one deliberate course.</p></div>
        <div class="cq-helm-weather" aria-label="Current fleet watch"><i></i><span><small>WATCH STATUS</small><b id="captainCineWatchState">READING</b></span></div>
      </header>
      <section class="cq-helm-stage">
        <div class="cq-living-chart" aria-label="Six-vessel fleet map">
          <div class="cq-chart-aurora" aria-hidden="true"></div>
          <div class="cq-chart-stars" aria-hidden="true"><i></i><i></i><i></i><i></i><i></i><i></i><i></i><i></i><i></i></div>
          <div class="cq-chart-depth" aria-hidden="true"><i></i><i></i><i></i><i></i></div>
          <svg class="cq-chart-routes" viewBox="0 0 1000 600" preserveAspectRatio="none" aria-hidden="true">
            <path d="M170 150 C300 240 380 240 490 95"/><path d="M490 95 C650 185 690 130 800 168"/><path d="M170 150 C110 315 120 360 200 430"/><path d="M200 430 C340 365 385 410 510 402"/><path d="M510 402 C650 335 715 430 820 430"/><path d="M800 168 C875 255 875 330 820 430"/>
          </svg>
          <div class="cq-chart-north" aria-hidden="true"><span>N</span><b>✦</b></div>
          <div class="cq-chart-bearing bearing-west" aria-hidden="true">270° · WESTERN REACH</div>
          <div class="cq-chart-bearing bearing-east" aria-hidden="true">090° · EASTERN REACH</div>
          <div id="captainCineVessels" class="cq-cine-vessels"></div>
          <div class="cq-chart-legend"><span><i class="steady"></i>IN FLEET</span><span><i class="trial"></i>SEA TRIAL</span><span><i class="signal"></i>NEEDS ATTENTION</span></div>
        </div>
        <aside id="captainCineIntel" class="cq-cine-intel" aria-live="polite"></aside>
      </section>
      <nav class="cq-cine-command-loop" aria-label="Captain command loop">
        <button type="button" data-cine-stage="watch" aria-pressed="true"><small>01</small><span><b>WATCH</b><em>What changed?</em></span></button>
        <button type="button" data-cine-stage="decide" aria-pressed="false"><small>02</small><span><b>DECIDE</b><em>Why does it matter?</em></span></button>
        <button type="button" data-cine-stage="act" aria-pressed="false"><small>03</small><span><b>ACT</b><em>Take the exact route</em></span></button>
        <button type="button" data-cine-stage="record" aria-pressed="false"><small>04</small><span><b>RECORD</b><em>Keep the outcome</em></span></button>
        <button type="button" data-cine-stage="systems" aria-pressed="false"><small>+</small><span><b>MORE SYSTEMS</b><em>Open support stations</em></span></button>
      </nav>
      <section id="captainCineSystems" class="cq-cine-systems hidden" aria-label="Captain support stations">
        <button type="button" data-cine-route="readiness"><b>Fleet Readiness</b><small>Prove the hull</small></button>
        <button type="button" data-cine-route="workshop"><b>Workshop</b><small>Ideas and experiments</small></button>
        <button type="button" data-cine-route="forge"><b>Visual Forge</b><small>Presentation and brand</small></button>
        <button type="button" data-cine-route="shipyard"><b>Shipyard</b><small>Future vessels</small></button>
        <button type="button" data-cine-route="blueprint"><b>Blueprint</b><small>Architecture map</small></button>
        <button type="button" data-cine-route="test"><b>Test Access</b><small>Session controls</small></button>
        <button type="button" data-cine-route="admiral"><b>Admiral's Gate</b><small>Provisional higher command</small></button>
      </section>`;
    helm.dataset.stage='watch';
    helm.addEventListener('click',event=>{
      const vessel=event.target.closest('[data-cine-vessel]');
      if(vessel){helm.dataset.selectedVessel=vessel.dataset.cineVessel;helm.dataset.userSelected='1';helm.dataset.stage='watch';helm.classList.remove('signal-acquired');void helm.offsetWidth;helm.classList.add('signal-acquired');refreshCinematicHelm();return;}
      const action=event.target.closest('[data-cine-action]');
      if(action){
        if(action.dataset.cineAction==='readiness'){runCinematicFleetReadiness(helm);return;}
        if(action.dataset.cineAction==='findings'){openAdmiralGate('captain','findings');return;}
        const projectId=helm.dataset.selectedVessel;
        if(action.dataset.cineAction==='report'&&projectId){byId('captainDarkSkyChartBtn')?.click();requestAnimationFrame(()=>openSignalReport(projectId));return;}
        if(action.dataset.cineAction==='firstmate'){byId('captainSpyglassBtn')?.click();return;}
        if(action.dataset.cineAction==='record'){byId('captainLogDoor')?.click();return;}
      }
      const stage=event.target.closest('[data-cine-stage]');
      if(stage){
        const name=stage.dataset.cineStage;
        if(name==='systems'){const systems=byId('captainCineSystems');systems?.classList.toggle('hidden');stage.setAttribute('aria-pressed',String(Boolean(systems&&!systems.classList.contains('hidden'))));}
        else{helm.dataset.stage=name;byId('captainCineSystems')?.classList.add('hidden');refreshCinematicHelm();}
        return;
      }
      const route=event.target.closest('[data-cine-route]');
      if(route){
        if(route.dataset.cineRoute==='readiness'){runCinematicFleetReadiness(helm);return;}
        const target=ensureCaptainDeskIndex()?.querySelector(`[data-cq-desk-route="${route.dataset.cineRoute}"]`);
        if(target)target.click();
        else showCaptainDeskNotice('That support station is not available on this hull.','unavailable');
      }
    });
    room.appendChild(helm);
    return helm;
  }

  function refreshCinematicHelm(){
    const helm=ensureCinematicHelm();
    const host=byId('captainCineVessels');
    const intel=byId('captainCineIntel');
    if(!helm||!host||!intel)return;
    const fleet=fleetSnapshot();
    const attentionFleet=fleet.filter(v=>Number(v.attentionOutposts)>0);
    const positions=[[17,25],[49,15],[80,28],[20,72],[51,67],[82,72]];
    if(!fleet.length){host.innerHTML='<p class="cq-cine-empty">Fleet position is awaiting Engine truth.</p>';intel.innerHTML='<small>FIRST MATE</small><h3>Standing by</h3><p>No vessel manifest is available yet.</p>';return;}
    if(!fleet.some(v=>v.projectId===helm.dataset.selectedVessel)||(!helm.dataset.userSelected&&attentionFleet.length))helm.dataset.selectedVessel=(attentionFleet[0]||fleet[0]).projectId;
    const chosen=fleet.find(v=>v.projectId===helm.dataset.selectedVessel)||fleet[0];
    const htmlSafe=value=>String(value??'').replace(/[&<>"']/g,char=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[char]));
    host.innerHTML=fleet.map((v,index)=>{
      const trials=(v.outposts||[]).filter(o=>o.state==='sea_trial').length;
      const tone=Number(v.attentionOutposts)>0?'signal':trials?'trial':Number(v.activeOutposts)>0?'sailing':'steady';
      const status=Number(v.attentionOutposts)>0?`${v.attentionOutposts} SIGNAL`:trials?`${trials} SEA TRIAL`:Number(v.activeOutposts)>0?`${v.activeOutposts} SAILING`:'IN FLEET';
      const pos=positions[index%positions.length];
      const bearing=['NW · 315°','N · 000°','NE · 045°','SW · 225°','S · 180°','SE · 135°'][index%6];
      return `<button type="button" class="cq-cine-vessel ${tone}${v.projectId===chosen.projectId?' selected':''}" data-cine-vessel="${htmlSafe(v.projectId)}" style="--cine-x:${pos[0]}%;--cine-y:${pos[1]}%" aria-pressed="${v.projectId===chosen.projectId}" aria-label="${htmlSafe(v.name)}, ${status}"><i></i><span>${htmlSafe(String(v.code||'PRJ').slice(0,3))}<em>${bearing}</em></span><b>${htmlSafe(v.name)}</b><small>${status}</small></button>`;
    }).join('');
    const trials=(chosen.outposts||[]).filter(o=>o.state==='sea_trial');
    const reasons=[];(chosen.outposts||[]).forEach(o=>(o.attentionReasons||[]).forEach(r=>{if(r&&!reasons.includes(r))reasons.push(r);}));
    const stage=helm.dataset.stage||'watch';
    const readinessReport=window.__lastAdmiralReadinessReport;
    const readinessChecking=helm.dataset.readinessState==='checking';
    const readinessUnavailable=helm.dataset.readinessState==='unavailable';
    const readinessLabel=readinessUnavailable?'UNAVAILABLE':readinessReport?(readinessReport.pass?(readinessReport.warnings?'WATCH':'CLEAR'):'HOLD'):'NOT RUN';
    const stageCopy={
      watch:{label:'WHAT CHANGED',title:reasons[0]||`${chosen.name} has no active attention signal.`,copy:trials.length?`${trials[0].name} is presently in Sea Trial.`:'The latest deployment manifest is reflected on this chart.'},
    ...
    helm.querySelectorAll('[data-cine-stage]').forEach(btn=>btn.setAttribute('aria-pressed',String(btn.dataset.cineStage===stage)));
    const state=byId('captainCineWatchState');if(state)state.textContent=stage==='readiness'?(readinessChecking?'CHECKING':readinessLabel):(attentionFleet.length?`${attentionFleet.length} SIGNAL${attentionFleet.length===1?'':'S'}`:'STEADY');
    helm.classList.toggle('attention',attentionFleet.length>0);
  }

  function runCinematicFleetReadiness(helm=ensureCinematicHelm()){
    if(!helm)return;
    byId('captainCineSystems')?.classList.add('hidden');
    const systemsButton=helm.querySelector('[data-cine-stage="systems"]');
    if(systemsButton)systemsButton.setAttribute('aria-pressed','false');
    helm.dataset.stage='readiness';
    helm.dataset.readinessState='checking';
  ...
  function showCaptainDeskNotice(message,tone='future'){
    ...
    showCaptainDeskNotice.timer=setTimeout(()=>notice.classList.remove('show'),6500);
  }
  ...
  function primeAdmiralFirstPaint8658(){
    ...
    // Carry a completed Captain proof across the authenticated Admiral gate.
    // A view transition must not discard verified findings or imply that a new
    // check is running. Background proof remains an acceptable fallback only
    // when no deliberate readiness report exists in this document.
    const report=window.__lastAdmiralReadinessReport||window.__lastAdmiralBackgroundReadinessReport||null;
    if(report){
      syncAdmiralReadiness(report);
    }else{
      if(findings)findings.innerHTML='<span>No current readiness report. Run Fleet Readiness to populate verified findings.</span>';
      for(const id of ['admiralDeckReadinessState','admiralCeremonialReadinessState']){const el=byId(id);if(el){el.textContent='NOT RUN';el.dataset.state='not-run';}}
      for(const id of ['admiralDeckReadinessCopy','admiralCeremonialReadinessCopy']){const el=byId(id);if(el)el.textContent='No Fleet Readiness verification is currently running. Run the check when a fresh diagnosis is required.';}
      const notice=byId('admiralDeckNotice');if(notice)notice.textContent='Admiral command ready. Run Fleet Readiness when a verified current report is required.';
    }
    requestAnimationFrame(()=>{
    ...
    deck.innerHTML=`
      <div class="admiral-deck-shell">
        ...
        <main class="admiral-deck-grid" aria-label="Admiral professional command view">
          ...
          <section class="admiral-command-map" aria-label="Admiral command boundary"><article><small>OBSERVE</small><strong>Read across the fleet</strong><span>Authenticated and read-only by default</span></article><article><small>COMMAND</small><strong>One exact vessel</strong><span>Previewed, reasoned and authorized</span></article><article><small>RETAIN</small><strong>Durable evidence</strong><span>Audit, rollback and fleet learning</span></article></section>
  ...
  <p class="admiral-doctrine-note">This is the current governed course, not a transient readiness notice. Readiness verifies this registry against the running build.</p>
    ...
    byId('admiralUnlockBtn').onclick=async()=>{
      ...
      const directTarget=gate.dataset.directTarget||'';
      gate.dataset.directTarget='';
      if(error)error.textContent='';hide('admiralGateOverlay');hide('captainQuarters');hide('captainGlobalExit');
      if(directTarget==='fleet'){
        hide('admiralDeck');
        requestAnimationFrame(()=>window.DarkSkyOpenAdmiralFleet?.({source:'admiral-gate'}));
        return;
      }
      show('admiralDeck');
      deck.dataset.mode='professional';
      ...
      if(directTarget==='findings'){
        setAdmiralLane('govern');
        window.setTimeout(()=>{
          const findings=byId('admiralReadinessFindings');
          const scroller=deck.querySelector('.admiral-deck-grid');
          if(findings&&scroller){scroller.scrollTop=Math.max(0,findings.offsetTop-170);findings.setAttribute('tabindex','-1');findings.focus({preventScroll:true});}
        },120);
      }
  ...
  const FOUNDRY_SESSION_KEY='darkSkySupabaseAdmiralSessionV1';
  let foundryServerRows=[];
  const foundryCfg=()=>window.BlackFlagV3Identity?.productionAuth?.readClientConfig?.()||null;
  const foundrySession=()=>{try{return JSON.parse(sessionStorage.getItem(FOUNDRY_SESSION_KEY)||'null')}catch(_){return null}};
  const foundryHeaders=(token='')=>{const c=foundryCfg(),h={apikey:c?.publishableKey||'','Content-Type':'application/json'};if(token)h.Authorization='Bearer '+token;return h;};
  async function foundryCurrentSession(){let row=foundrySession(),c=foundryCfg();if(row&&row.expires_at>Date.now()+30000)return row;if(!row?.refresh_token||!c)return null;const r=await fetch(c.url+'/auth/v1/token?grant_type=refresh_token',{method:'POST',headers:foundryHeaders(),body:JSON.stringify({refresh_token:row.refresh_token})});if(!r.ok)return null;const data=await r.json();row={access_token:data.access_token,refresh_token:data.refresh_token||row.refresh_token||'',expires_at:Date.now()+Math.max(60,Number(data.expires_in||3600))*1000,user:data.user||row.user||null};sessionStorage.setItem(FOUNDRY_SESSION_KEY,JSON.stringify(row));return row;}
  async function foundryRpc(name,body={}){const c=foundryCfg(),session=await foundryCurrentSession();if(!c?.url||!session?.access_token)throw new Error('Authenticate the Admiral identity before writing Foundry state.');const r=await fetch(c.url+'/rest/v1/rpc/'+name,{method:'POST',headers:{...foundryHeaders(session.access_token),Accept:'application/json'},body:JSON.stringify(body)});if(!r.ok){let m='Fleet Core refused the Foundry command.';try{const e=await r.json();m=e?.message||e?.hint||m}catch(_){}throw new Error(String(m).replaceAll('_',' '));}return r.json();}
  function foundryScoutEsc(v){return String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));}
  function foundryScoutStatus(row){return String(row?.status||'hold')==='sea_trial_cleared'?'SEA TRIAL CLEARED':'HOLD';}
  function foundryScoutRowsMarkup(){const rows=foundryServerRows.slice();if(!rows.length)return '<div class="foundry-scout-empty">No durable Scout candidates yet. Start with a real mission or recurring customer friction—not a random repository.</div>';return rows.map(r=>{const st=foundryScoutStatus(r);return `<article class="foundry-scout-row" data-foundry-candidate="${foundryScoutEsc(r.id)}"><div><small>${foundryScoutEsc(r.product_route||'utility-dock').replaceAll('-',' ').toUpperCase()} • FLEET CORE CANDIDATE</small><strong>${foundryScoutEsc(r.upstream_name||'Unnamed candidate')}</strong><span>${foundryScoutEsc(r.source_url||'No source URL')}</span><p>${foundryScoutEsc(r.mission||'No mission recorded')}</p><span>${foundryScoutEsc(r.target_user||'Target user not recorded')} · ${foundryScoutEsc(r.job_frequency||'frequency unrecorded')}</span></div><div class="foundry-scout-row-state"><b class="${st==='SEA TRIAL CLEARED'?'clear':'hold'}">${st}</b><em>${foundryScoutEsc(r.license_id||'LICENSE UNSET')}</em><button type="button" data-foundry-delete="${foundryScoutEsc(r.id)}">ARCHIVE CANDIDATE</button></div></article>`}).join('');}
  async function syncFoundryIdentity(overlay){const ok=!!(await window.DarkSkyAdmiralIdentityStatus?.());overlay.classList.toggle('identity-verified',ok);const state=byId('foundryIdentityState');if(state)state.textContent=ok?'ADMIRAL IDENTITY VERIFIED':'ADMIRAL IDENTITY REQUIRED';['foundryIdentityEmail','foundryIdentityPassword'].forEach(id=>byId(id)?.closest('label')?.classList.toggle('hidden',ok));byId('foundryAuthenticate')?.classList.toggle('hidden',ok);byId('foundrySignOut')?.classList.toggle('hidden',!ok);if(byId('foundrySaveCandidate'))byId('foundrySaveCandidate').disabled=!ok;if(ok){const data=await foundryRpc('admiral_list_foundry_candidates',{p_limit:50});foundryServerRows=Array.isArray(data?.records)?data.records:[];}else foundryServerRows=[];const list=byId('foundryScoutRows');if(list)list.innerHTML=foundryScoutRowsMarkup();overlay.__bindFoundryDeletes?.(overlay);return ok;}
  function bindFoundryScout(overlay){
    const notice=byId('foundryNotice');
    const recalc=()=>{const req=['commercial','attribution','identity','security','dependencies','privacy','maintenance'];const ready=req.every(k=>byId('foundryCheck_'+k)?.checked)&&/^https:\/\/github\.com\//i.test(String(byId('foundrySourceUrl')?.value||''))&&String(byId('foundryCandidateName')?.value||'').trim()&&String(byId('foundryLicenseId')?.value||'').trim()&&String(byId('foundryMission')?.value||'').trim();const badge=byId('foundryClearanceState');if(badge){badge.textContent=ready?'SEA TRIAL CLEARED':'HOLD';badge.className='foundry-clearance-state '+(ready?'clear':'hold');}return ready;};
    overlay.querySelectorAll('[data-foundry-check]').forEach(x=>x.addEventListener('change',recalc));['foundryCandidateName','foundrySourceUrl','foundryLicenseId','foundryMission'].forEach(id=>byId(id)?.addEventListener('input',recalc));
    byId('foundryAuthenticate')?.addEventListener('click',async()=>{const b=byId('foundryAuthenticate');try{b.disabled=true;b.textContent='AUTHENTICATING…';await window.DarkSkyAuthenticateAdmiral?.(byId('foundryIdentityEmail')?.value,byId('foundryIdentityPassword')?.value);if(byId('foundryIdentityPassword'))byId('foundryIdentityPassword').value='';await syncFoundryIdentity(overlay);if(notice)notice.textContent='Admiral identity verified. Foundry candidates are now reading durable Fleet Core state.';}catch(e){if(notice)notice.textContent=e?.message||'Admiral identity could not be verified.';}finally{b.disabled=false;b.textContent='AUTHENTICATE ADMIRAL';}});
    byId('foundrySignOut')?.addEventListener('click',async()=>{window.DarkSkySignOutAdmiral?.();await syncFoundryIdentity(overlay);if(notice)notice.textContent='Admiral identity signed out. Durable Foundry writes are closed.';});
    byId('foundrySaveCandidate')?.addEventListener('click',async()=>{const btn=byId('foundrySaveCandidate');const checks={commercial:!!byId('foundryCheck_commercial')?.checked,attribution:!!byId('foundryCheck_attribution')?.checked,identity:!!byId('foundryCheck_identity')?.checked,security:!!byId('foundryCheck_security')?.checked,dependencies:!!byId('foundryCheck_dependencies')?.checked,privacy:!!byId('foundryCheck_privacy')?.checked,maintenance:!!byId('foundryCheck_maintenance')?.checked};const row={name:String(byId('foundryCandidateName')?.value||'').trim(),sourceUrl:String(byId('foundrySourceUrl')?.value||'').trim(),mission:String(byId('foundryMission')?.value||'').trim(),licenseId:String(byId('foundryLicenseId')?.value||'').trim(),route:String(byId('foundryProductRoute')?.value||'utility-dock'),dataClass:String(byId('foundryDataClass')?.value||'local-files-only'),targetUser:String(byId('foundryTargetUser')?.value||'').trim(),frequency:String(byId('foundryFrequency')?.value||'').trim(),workaround:String(byId('foundryWorkaround')?.value||'').trim(),valueLayer:String(byId('foundryValueLayer')?.value||'').trim(),checks};if(!row.name||!row.sourceUrl||!row.mission||!row.licenseId){if(notice)notice.textContent='Candidate name, GitHub source, mission and license review are required before save.';return;}try{btn.disabled=true;btn.textContent='SAVING TO FLEET CORE…';const result=await foundryRpc('admiral_upsert_foundry_candidate',{p_upstream_name:row.name,p_source_url:row.sourceUrl,p_mission:row.mission,p_license_id:row.licenseId,p_product_route:row.route,p_data_posture:row.dataClass,p_target_user:row.targetUser,p_job_frequency:row.frequency,p_current_workaround:row.workaround,p_value_layer:row.valueLayer,p_clearance:row.checks,p_interface_blueprint:{schema:'black-flag-universal-utility-interface-v1',targetUser:row.targetUser,workflow:['input','process','progress','result','recovery'],valueLayer:row.valueLayer}});await syncFoundryIdentity(overlay);if(notice)notice.textContent=result?.message||'Candidate saved to Fleet Core.';}catch(e){if(notice)notice.textContent=e?.message||'Candidate could not be saved.';}finally{btn.disabled=false;btn.textContent='SAVE GOVERNED CANDIDATE';}});
    function bindFoundryScoutDeletes(root){root.querySelectorAll('[data-foundry-delete]').forEach(btn=>btn.onclick=async()=>{try{btn.disabled=true;btn.textContent='ARCHIVING…';await foundryRpc('admiral_archive_foundry_candidate',{p_candidate_id:btn.dataset.foundryDelete,p_reason:'Archived from Admiral Foundry'});await syncFoundryIdentity(overlay);if(notice)notice.textContent='Candidate archived in Fleet Core. No capability, vessel entitlement, or customer surface changed.';}catch(e){if(notice)notice.textContent=e?.message||'Candidate could not be archived.';}finally{btn.disabled=false;btn.textContent='ARCHIVE CANDIDATE';}});}
    overlay.__bindFoundryDeletes=bindFoundryScoutDeletes;bindFoundryScoutDeletes(overlay);recalc();syncFoundryIdentity(overlay).then(()=>bindFoundryScoutDeletes(overlay)).catch(e=>{if(notice)notice.textContent=e?.message||'Foundry identity check failed.';});
  }
  ...
  function ensureFoundryWorkspace(){
    ...
    const gateChecks=[['commercial','Commercial-use rights verified'],['attribution','Attribution / notice obligations captured'],['identity','Upstream identity stays visible'],['security','Security review completed'],['dependencies','Dependency / supply-chain health reviewed'],['privacy','Privacy and data flow reviewed'],['maintenance','Maintenance / upstream health reviewed']].map(([id,label])=>`<label class="foundry-clearance-check"><input id="foundryCheck_${id}" data-foundry-check type="checkbox"><span>${label}</span></label>`).join('');
    overlay=document.createElement('div');
    overlay.id='foundryWorkspace';overlay.className='foundry-workspace hidden';overlay.setAttribute('role','dialog');overlay.setAttribute('aria-modal','true');overlay.setAttribute('aria-labelledby','foundryTitle');
    overlay.innerHTML=`<div class="foundry-shell">
      <header class="foundry-head"><div><small>ADMIRAL • 8.8.15.8 SPINE CONTINUITY</small><h2 id="foundryTitle">The Foundry</h2><p>Discover useful capability, clear it above board, forge the interface, prove it, then decide how—or whether—to commercialize it.</p></div><button id="foundryClose" type="button">RETURN TO ADMIRAL'S DECK</button></header>
      <section class="foundry-law"><strong>UPSTREAM TRUTH IS PART OF THE PRODUCT.</strong><span>Fleet value may be interface, workflow, hosting, integration, support and original additions. Upstream ownership, license and required notices never disappear.</span></section>
      <main class="foundry-body">
        <section class="foundry-section foundry-mission-pipeline"><header><small>MISSION-LED PIPELINE</small><h3>Demand first. Repository second.</h3><p>Start with a real user or vessel problem. Scout for an existing legal capability before rebuilding from scratch.</p></header><div class="foundry-pipeline">${['MISSION DEMAND','SCOUT','CLEARANCE','INTERFACE FORGE','SEA TRIAL','PROVEN','UTILITY DOCK / STANDALONE'].map((x,i)=>`<div><b>${String(i+1).padStart(2,'0')}</b><strong>${x}</strong></div>`).join('')}</div></section>
        <section class="foundry-section foundry-scout"><header><div><small>FOUNDRY SCOUT</small><h3>Public-source capability intake</h3><p>Demand first. Candidate state is durable in Fleet Core. Nothing auto-scrapes, clones, publishes, sells, or entitles.</p></div><strong id="foundryClearanceState" class="foundry-clearance-state hold">HOLD</strong></header><div class="foundry-identity"><div><small>SERVER-GOVERNED CLEARANCE</small><b id="foundryIdentityState">ADMIRAL IDENTITY REQUIRED</b></div><label>Email<input id="foundryIdentityEmail" type="email" autocomplete="username" autocapitalize="none"></label><label>Password<input id="foundryIdentityPassword" type="password" autocomplete="current-password"></label><button id="foundryAuthenticate" type="button">AUTHENTICATE ADMIRAL</button><button id="foundrySignOut" type="button" class="hidden">SIGN OUT</button></div>
          <div class="foundry-scout-grid"><label>Candidate / upstream project<input id="foundryCandidateName" placeholder="Example: useful CLI converter"></label><label>Public source URL<input id="foundrySourceUrl" inputmode="url" placeholder="https://github.com/owner/repo"></label><label>License identifier<input id="foundryLicenseId" placeholder="Example: MIT, Apache-2.0, GPL-3.0"></label><label>Product route<select id="foundryProductRoute"><option value="utility-dock">Utility Dock module</option><option value="standalone">Standalone application</option><option value="hosted-service">Hosted service</option><option value="vessel-capability">Vessel capability</option></select></label><label>Data posture<select id="foundryDataClass"><option value="local-files-only">Local files only</option><option value="user-provided-data">User-provided data</option><option value="account-data">Connected account data</option><option value="sensitive-data">Sensitive / regulated data</option></select></label><label>Target user<input id="foundryTargetUser" placeholder="Who pays or uses this?"></label><label>Job frequency<input id="foundryFrequency" placeholder="Daily, weekly, occasional…"></label><label>Current workaround<input id="foundryWorkaround" placeholder="Terminal, manual steps, expensive software…"></label><label>Fleet value layer<input id="foundryValueLayer" placeholder="UI, hosting, integration, support…"></label><label class="wide">Mission / customer friction<textarea id="foundryMission" rows="3" placeholder="What useful job exists, who struggles with it, and what friction are we removing?"></textarea></label></div>
          <div class="foundry-clearance-grid">${gateChecks}</div><div class="foundry-scout-actions"><button id="foundrySaveCandidate" type="button">SAVE GOVERNED CANDIDATE</button><span>Clearance means Sea Trial only. It never means automatic publication, entitlement or commercial launch.</span></div>
          <div id="foundryScoutRows" class="foundry-scout-rows">${foundryScoutRowsMarkup()}</div>
        </section>
        <section class="foundry-section foundry-interface-forge"><header><small>INTERFACE FORGE</small><h3>Universal Utility Interface</h3><p>When a candidate clears, Fleet design standards wrap the capability without hiding what powers it.</p></header><div class="foundry-interface-grid"><article><b>SAFE DEFAULTS</b><span>No terminal knowledge required; dangerous or destructive operations are explicit.</span></article><article><b>CONSISTENT UX</b><span>Upload, process, progress, result, recovery and support patterns stay familiar across utilities.</span></article><article><b>UPSTREAM TRUTH</b><span>Project identity, license notices and attribution remain available wherever required.</span></article><article><b>COMMERCIAL LAYER</b><span>Charge for Fleet-added interface, hosting, integrations, workflow, support and original capability—not false authorship.</span></article></div></section>
        ...
        <section class="foundry-section"><header><small>FOUNDATION AUDIT</small><h3>Existing pieces promoted into the registry</h3><p>Foundations and candidates are not promises of live production behavior.</p></header><div class="foundry-capability-list">${capabilityRows}</div></section>
        <section class="foundry-section foundry-boundary"><header><small>BOUNDARY CONTRACT</small><h3>Capability ≠ Vessel ≠ Service Instance ≠ Upstream Project</h3></header><p><b>Upstream project:</b> software we did not author. <b>Capability:</b> reusable function the Fleet may adapt. <b>Vessel:</b> independent business or major fleet operation. <b>Service instance:</b> explicit agreement letting one vessel consume one capability.</p><p>No interface wrapper changes who authored the upstream code. No license review widens vessel authority. No commercial route may cross private project data by default.</p></section>
      </main>
      <footer class="foundry-foot"><span>ADMIRAL DRAFTS ONLY • NO AUTO-SCRAPE • NO AUTO-PUBLISH • NO CROSS-PROJECT DATA GRANT</span><button id="foundryAudit" type="button">SHOW FOUNDATION STATUS</button></footer>
      <div id="foundryNotice" class="foundry-notice" role="status" aria-live="polite">Foundry Gate is ready. Start from a mission, then clear the candidate before Sea Trial.</div>
    </div>`;
    document.body.appendChild(overlay);byId('foundryClose').onclick=()=>{hide('foundryWorkspace');show('admiralDeck');};byId('foundryAudit').onclick=()=>{const n=byId('foundryNotice');if(n)n.textContent=`${registry.length} capability foundations classified across ${Object.keys(domains).length} Foundry domains. Foundry candidates are durable Fleet Core records; nothing is published or commercially activated by registration alone.`;};bindFoundryScout(overlay);return overlay;
  }
  function openFoundryWorkspace(){
    ensureFoundryWorkspace();hide('admiralDeck');show('foundryWorkspace');
  }

  function openAdmiralGate(source='captain',directTarget=''){
    ensureAdmiralDeck();
    hide('captainGlobalExit');
    const gate=byId('admiralGateOverlay');
    const deck=byId('admiralDeck');
    ...
    if(gate){gate.dataset.entrySource=fromEngine?'engine':'captain';gate.dataset.directTarget=directTarget;}
    ...
    if(copy)copy.textContent=directTarget==='fleet'?'Verify the Admiral PIN once, then go directly to the read-only Fleet Registry. Account identity remains separately protected.':directTarget==='findings'?'Verify Admiral authority, then open the current governed findings without changing vessel state.':"The Admiral's Deck governs Dark Sky, Black Flag and the fleet. This trial entrance exists so the Captain can prove the machinery before the rank is earned.";
    if(unlock)unlock.textContent=directTarget==='fleet'?'VERIFY & OPEN MY FLEET →':directTarget==='findings'?'VERIFY & VIEW CURRENT FINDINGS →':"ENTER ADMIRAL'S DECK →";
    if(deckReturn){
  ...
  function ensureVisualForge(){
    ...
    byId('visualForgeInstall').onclick=async()=>{
      ...
      try{
        ...
        byId('visualForgeStateCopy').textContent=slot==='admiral'?'The authenticated Admiral Deck will use this visual; the pre-auth gate remains neutral and Professional Mode remains clean.':'Captain reference saved for Forge/Sea Trial. The proven command-room geometry is not silently replaced.';
  ...
  function refreshChartroomLive(){
    ...
    if(watch) watch.innerHTML=signalRows.length?signalRows.slice(0,1).map(({v,reasons})=>`<button type="button" class="captain-pro-signal" data-captain-pro-vessel="${v.projectId}"><span><b>${v.name}</b><small>${reasons[0]||`${v.attentionOutposts} deployment signal${v.attentionOutposts===1?'':'s'} need review.`}</small></span><em>OPEN SIGNAL →</em></button>`).join(''):`<div class="captain-pro-empty">No active attention signals.</div>`;
    const decide=byId('captainProDecideItems');
    if(decide) decide.innerHTML=signalRows.length?signalRows.slice(0,1).map(({v,reasons})=>`<article><b>${v.name}</b><p>${reasons.length?reasons.join(' • '):'A deployment signal requires Captain review.'}</p><small>FIRST MATE: inspect the evidence before choosing a route.</small></article>`).join(''):`<div class="captain-pro-empty">No decision analysis is waiting.</div>`;
    const act=byId('captainProActItems');
    if(act) act.innerHTML=signalRows.length?signalRows.slice(0,1).map(({v})=>{const priority=(v.outposts||[]).find(o=>o.state==='sea_trial'||o.state==='paused'||(o.attentionReasons||[]).length)||(v.outposts||[])[0];return `<article><b>${captainSafe(v.name)}</b><p>${captainSafe(priority?.nextStep||v.launchDetail||'Open the scoped signal report and review the exact route.')}</p><button type="button" data-captain-pro-vessel="${captainSafe(v.projectId)}">OPEN EXACT COURSE</button></article>`;}).join(''):`<article><b>Fleet steady</b><p>No attention route is required right now.</p><button type="button" data-captain-pro-route="fleet">OPEN FLEET MAP</button></article>`;
  ...
  function ensureCaptainCommandMode(){
    ...
    if(!bar){
      ...
      bar.innerHTML=`<div><small>ENGINE ROOM / CAPTAIN WATCH • ${UPPER_COMMAND_BUILD}</small><strong>One signal → one decision → one exact route → one durable outcome</strong></div><div class="captain-command-mode-actions"><button id="captainCommandReturnBtn" type="button">← RETURN TO ENGINE</button><button id="captainCommandModeToggle" type="button">CINEMATIC VIEW</button></div>`;
    ...
    if(!surface){
      ...
      surface.innerHTML=`
        ...
        <section class="captain-command-decks" aria-label="Captain command loop">
          ...
          <article class="captain-command-lane"><header><small>02 · DECIDE</small><h3>Why does it matter?</h3><p>Understand the signal before choosing a course.</p></header><div id="captainProDecideItems" class="captain-lane-body"></div></article>
          <article class="captain-command-lane"><header><small>03 · ACT</small><h3>Take the right route.</h3><p>Route deliberately without breaking project boundaries.</p></header><div id="captainProActItems" class="captain-lane-body"></div></article>
          <article class="captain-command-lane"><header><small>04 · RECORD</small><h3>Keep the decision.</h3><p>Orders, notes, history, outcomes and retained lessons.</p></header><div id="captainProRecordItems" class="captain-lane-body"></div></article>
        </section>
        <details class="captain-support-drawer"><summary><span><b>ENGINEERING SYSTEMS</b><small>Signals, intelligence, readiness, construction and retained history</small></span><em>SYSTEMS AVAILABLE</em></summary><div><button type="button" data-captain-pro-route="watch"><b>All Signals</b><small>Full watch</small></button><button type="button" data-captain-pro-route="spyglass"><b>First Mate Analysis</b><small>Fleet intelligence</small></button><button type="button" data-captain-pro-route="fleet"><b>Fleet Map</b><small>Vessel posture</small></button><button type="button" data-captain-pro-route="log"><b>Captain's Log</b><small>Durable history</small></button><button type="button" data-captain-pro-route="readiness"><b>Fleet Readiness</b><small>Check fleet health</small></button><button type="button" data-captain-pro-route="workshop"><b>Workshop</b><small>Build and configure</small></button><button type="button" data-captain-pro-route="forge"><b>Visual Forge</b><small>Brand and experience</small></button><button type="button" data-captain-pro-route="blueprint"><b>Blueprint</b><small>Layouts and architecture</small></button><button type="button" data-captain-pro-route="admiral"><b>Admiral's Gate</b><small>Trial higher command</small></button></div></details>
    ...
    room.dataset.defaultCommandMode='professional';
  ...
  function bind() {
    ...
    document.querySelectorAll('[data-command-open="fleet-registry"]').forEach(btn=>btn.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();openAdmiralGate('engine','fleet');}));
    ...
    byId('captainFleetChart')?.addEventListener('click',event=>{
      ...
      route.innerHTML='ROUTING… <small>Exact outpost in Engine</small>';
...
;(()=>{
  ...
  const defaultLogo=projectId=>({'construction-scheduling-01':'bootstrap_build_logo.png','ikes-wood-signs':'ike_character.png','bor-north-richmond':'signal_restoration_logo.png','bf-p-f92f87e8ec44':'https://legacyplumbingrva.com/wp-content/uploads/2026/04/LOGO.png'}[projectId]||'');
...
/* 8.8.13 Quick Passage — fleet registry isolated from retired schedules. */
;(()=>{
  const el=id=>document.getElementById(id);
  const esc=value=>String(value??'').replace(/[&<>"']/g,char=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[char]));
  function install(){
    if(el('bootstrapFleet'))return;
    document.body.insertAdjacentHTML('beforeend',`<section id="bootstrapFleet" class="bootstrap-fleet hidden" role="dialog" aria-modal="true" aria-hidden="true" aria-label="Black Flag Admiral Fleet Registry"><div class="bootstrap-fleet-shell"><header><img src="black_flag_primary_lockup.png" alt="Black Flag Business Command Platform"><div><small>BLACK FLAG · SECURE FLEET PASSAGE</small><h2>My Fleet</h2><p>One protected registry. Independent vessels. No detour through the full Admiral command deck.</p></div><div><button id="bootstrapFleetRefresh" type="button">REFRESH</button><button id="bootstrapFleetClose" type="button">CLOSE</button></div></header><section id="bootstrapFleetIdentity" class="bootstrap-fleet-identity"><div><small>ADMIRAL ACCOUNT</small><b>Complete the second security check</b><span>The PIN opened this passage. Your active Admiral account authorizes the fleet read.</span></div><label>Email<input id="bootstrapFleetEmail" type="email" inputmode="email" autocomplete="username" autocapitalize="none" spellcheck="false" placeholder="Admiral email"></label><label>Password<input id="bootstrapFleetPassword" type="password" autocomplete="current-password" placeholder="Password"></label><button id="bootstrapFleetSignIn" type="button">AUTHENTICATE & OPEN</button><button id="bootstrapFleetSignOut" class="hidden" type="button">SIGN OUT</button></section><p id="bootstrapFleetStatus" class="bootstrap-fleet-status" role="status">Reading the Fleet Registry…</p><div id="bootstrapFleetRows" class="bootstrap-fleet-rows"></div><aside class="bootstrap-fleet-passage"><b>Shared Capability Passage</b><span>Any fleet member may adopt an approved capability at its own level. Adoption never transfers ownership, authority, branding, or project data.</span></aside><footer><b>Read-only registry boundary</b><span>This view cannot rename, commission, publish, entitle, or otherwise change a vessel.</span></footer></div></section>`);
  }
  ...
  async function load(){
    ...
    if(statusNode)statusNode.textContent='Verifying Admiral identity for a read-only fleet view…';
    try{
      const verified=await window.DarkSkyAdmiralIdentityStatus?.();
      el('bootstrapFleetIdentity')?.classList.toggle('is-verified',!!verified);
      ['bootstrapFleetEmail','bootstrapFleetPassword','bootstrapFleetSignIn'].forEach(id=>el(id)?.closest('label')?.classList.toggle('hidden',!!verified));
      el('bootstrapFleetSignIn')?.classList.toggle('hidden',!!verified);
      el('bootstrapFleetSignOut')?.classList.toggle('hidden',!verified);
      if(!verified){render([]);if(statusNode)statusNode.textContent='Admiral account authentication is required. The registry remains closed.';return;}
      await window.DarkSkySyncVesselBrandIdentity?.();render(window.DarkSkyReadAdmiralFleet?.()||[]);
    }
  ...
  document.addEventListener('click',async event=>{
    ...
    if(target.closest?.('#bootstrapFleetSignIn')){event.preventDefault();event.stopImmediatePropagation();const button=el('bootstrapFleetSignIn');try{button.disabled=true;button.textContent='AUTHENTICATING…';await window.DarkSkyAuthenticateAdmiral?.(el('bootstrapFleetEmail')?.value,el('bootstrapFleetPassword')?.value);if(el('bootstrapFleetPassword'))el('bootstrapFleetPassword').value='';await load();}catch(error){if(el('bootstrapFleetStatus'))el('bootstrapFleetStatus').textContent=error?.message||'Admiral sign-in failed.';}finally{button.disabled=false;button.textContent='AUTHENTICATE & OPEN';}return;}
    if(target.closest?.('#bootstrapFleetSignOut')){event.preventDefault();event.stopImmediatePropagation();window.DarkSkySignOutAdmiral?.();await load();}
  },{capture:true});
})();
...
;(()=>{
  ...
  function move(id,day){const task=state.tasks.find(item=>item.id===id);if(!task)return;const from=task.start,next=Math.max(0,Math.min(DAYS-Math.max(1,task.duration||1),Number(day)));task.start=next;if(from!==next)state.activity.unshift({id:'move-'+Date.now(),at:new Date().toISOString(),task:task.title,vendor:task.vendor,from:labelDate(from,true),to:labelDate(next,true)});state.activity=state.activity.slice(0,100);selectedId='';showOver(null);renderSchedule();document.querySelectorAll('#bootstrapSchedule [data-task-id="'+CSS.escape(task.id)+'"]').forEach(node=>node.classList.add('is-settling'));status(task.title+' moved to '+labelDate(task.start,true)+'. Saving vendor update…');window.setTimeout(()=>{save();renderVendorLayer();status(task.title+' moved to '+labelDate(task.start,true)+'. '+task.duration+'-workday duration preserved; '+task.vendor+' can see the change.');},32);}
  function gridRects(){return [...document.querySelectorAll('.bootstrap-ground-week-grid')].map(grid=>({week:Number(grid.dataset.week),rect:grid.getBoundingClientRect()}));}
  function dayAtPoint(x,y,rects=gridRects()){for(const item of rects){const r=item.rect;if(x>=r.left&&x<=r.right&&y>=r.top&&y<=r.bottom){const col=Math.max(0,Math.min(4,Math.floor((x-r.left)/(r.width/5))));return item.week*5+col;}}return null;}
  function showOver(day){if(day===overDay)return;const prior=overDay;overDay=day;if(prior!==null)document.querySelectorAll('[data-schedule-day="'+prior+'"],[data-schedule-move="'+prior+'"]').forEach(node=>node.classList.remove('is-over'));if(day!==null)document.querySelectorAll('[data-schedule-day="'+day+'"],[data-schedule-move="'+day+'"]').forEach(node=>node.classList.add('is-over'));}
  function makeDragGhost(card,event){const rect=card.getBoundingClientRect(),ghost=card.cloneNode(true);ghost.removeAttribute('draggable');ghost.removeAttribute('tabindex');ghost.classList.remove('is-selected');ghost.classList.add('bootstrap-drag-ghost');ghost.style.width=rect.width+'px';ghost.style.left=rect.left+'px';ghost.style.top=rect.top+'px';document.body.appendChild(ghost);return {node:ghost,offsetX:event.clientX-rect.left,offsetY:event.clientY-rect.top};}
  ...
  document.addEventListener('pointerdown',event=>{const card=event.target.closest?.('#bootstrapSchedule [data-task-id]');if(!card||event.pointerType==='mouse')return;event.preventDefault();event.stopImmediatePropagation();selectedId=card.dataset.taskId;touchDrag={id:selectedId,pointerId:event.pointerId,rects:gridRects(),ghost:makeDragGhost(card,event)};card.setPointerCapture?.(event.pointerId);card.classList.add('is-dragging');moveDragGhost(touchDrag,event);},{capture:true});
  document.addEventListener('pointermove',event=>{if(!touchDrag||event.pointerId!==touchDrag.pointerId)return;event.preventDefault();event.stopImmediatePropagation();moveDragGhost(touchDrag,event);showOver(dayAtPoint(event.clientX,event.clientY,touchDrag.rects));},{capture:true,passive:false});
  document.addEventListener('pointerup',event=>{if(!touchDrag||event.pointerId!==touchDrag.pointerId)return;event.preventDefault();event.stopImmediatePropagation();const drag=touchDrag,id=drag.id,day=dayAtPoint(event.clientX,event.clientY,drag.rects);clearTouchDrag();suppressClickUntil=Date.now()+350;if(day!==null)move(id,day);else{showOver(null);render();status('Task selected. Tap a workday heading to move it.');}},{capture:true});
  document.addEventListener('pointercancel',event=>{if(!touchDrag)return;event.stopImmediatePropagation();clearTouchDrag();showOver(null);render();},{capture:true});
...
/* 8.8.13 Project Roster — three isolated schedules with durable local records. */
;(()=>{
  ...
  const projects=[
    ...
    {id:'1234-B',region:'ABC',division:'CBA',community:'AB',lot:'1234',unit:'B',startWeek:1},
    {id:'1235-A',region:'ABC',division:'CBA',community:'AB',lot:'1235',unit:'A',startWeek:2}
  ];
  let activeId='',state=null,cache=new Map(),selectedId='',vendorView='',touchDrag=null,overDay=null,suppressClickUntil=0,dbPromise=null;
  const el=id=>document.getElementById(id);
  const esc=value=>String(value??'').replace(/[&<>"']/g,char=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[char]));
  const project=id=>projects.find(item=>item.id===id)||projects[0];
  function vendorFor(trade){return ({Sitework:'Piedmont Siteworks',Survey:'Cardinal Survey',Inspection:'City & County Inspections',Concrete:'Blue Ridge Concrete',Masonry:'Commonwealth Masonry',Foundation:'DryLine Foundation',Delivery:'Builder Supply Logistics',Framing:'Timberline Framing',Roofing:'Summit Roofing',Carpentry:'Heritage Carpentry',Mechanical:'AirFlow Mechanical',Plumbing:'Legacy Plumbing',Electrical:'BrightWire Electric',Insulation:'Thermal Shield',Milestone:'Bootstrap Build Team',Drywall:'Finish Line Drywall',Exterior:'Carolina Exteriors',Painting:'TrueCoat Painting',Flooring:'Piedmont Floors',Closeout:'Bootstrap Build Team'})[trade]||trade+' Partner';}
  function seedTasks(){return [
    ['clearing','Clearing & erosion control','Sitework',0,2,'site'],['layout','House layout & utility locate','Survey',1,1,'inspection'],['excavation','Excavation','Sitework',3,2,'site'],['footers','Footers formed & poured','Concrete',5,2,'work'],['footing-insp','Footing inspection','Inspection',7,1,'inspection'],['foundation','Foundation block','Masonry',8,3,'work'],['foundation-insp','Foundation inspection','Inspection',11,1,'inspection'],['waterproof','Waterproofing & drain tile','Foundation',11,2,'site'],['backfill','Backfill & rough grade','Sitework',13,2,'site'],['slab-prep','Slab prep','Concrete',14,2,'work'],['slab-insp','Slab inspection','Inspection',16,1,'inspection'],['slab','Garage slab','Concrete',17,1,'work'],['lumber','Lumber delivery','Delivery',18,1,'delivery'],['framing','Floor system & framing','Framing',19,7,'work'],['frame-check','Frame quality check','Inspection',25,1,'inspection'],['roof','Roof dry-in','Roofing',25,3,'site'],['windows','Windows & exterior doors','Carpentry',27,2,'site'],['rough-hvac','HVAC rough-in','Mechanical',28,3,'work'],['rough-plumb','Plumbing rough-in','Plumbing',29,3,'work'],['rough-electric','Electrical rough-in','Electrical',31,3,'work'],['rough-inspections','Rough trade inspections','Inspection',34,2,'inspection'],['insulation','Air seal & insulation','Insulation',36,3,'work'],['predrywall','Pre-drywall meeting','Milestone',38,1,'milestone'],['drywall-delivery','Drywall delivery','Delivery',39,1,'delivery'],['drywall','Drywall hang & finish','Drywall',40,6,'work'],['exterior','Siding & exterior trim','Exterior',41,5,'site'],['prime','Prime & first paint','Painting',46,3,'work'],['cabinets','Cabinets & interior trim','Carpentry',47,4,'work'],['hard-surface','Hard-surface install','Flooring',49,3,'work'],['mep-final','MEP final fixtures','Mechanical',50,4,'work'],['appliances','Appliance delivery & install','Delivery',53,2,'delivery'],['carpet','Carpet','Flooring',54,2,'work'],['driveway','Driveway & exterior concrete','Concrete',54,3,'site'],['final-grade','Final grade & landscaping','Sitework',56,3,'site'],['final-insp','Final inspections','Inspection',57,2,'inspection'],['punch','Punchout','Closeout',57,3,'work'],['final-clean','Final clean','Closeout',59,1,'work'],['settlement','Settlement','Milestone',59,1,'milestone']
  ].map(([id,title,trade,start,duration,type])=>({id,title,trade,start,duration,type,vendor:vendorFor(trade)}));}
  function fresh(meta){const tasks=seedTasks();tasks.forEach(task=>task.plannedDuration=task.duration);return {version:5,projectId:meta.id,hierarchy:{region:meta.region,division:meta.division,community:meta.community,lot:meta.lot,unit:meta.unit},tasks,acceleratedDays:[],activity:[],updatedAt:null,revision:0};}
  function normalize(value,meta){if(!value||!Array.isArray(value.tasks))return null;value.version=5;value.projectId=meta.id;value.hierarchy={region:meta.region,division:meta.division,community:meta.community,lot:meta.lot,unit:meta.unit};value.activity=Array.isArray(value.activity)?value.activity:[];value.acceleratedDays=Array.isArray(value.acceleratedDays)?value.acceleratedDays:[];value.revision=Number(value.revision||0);value.tasks.forEach(task=>{task.vendor=task.vendor||vendorFor(task.trade);task.plannedDuration=Math.max(1,Number(task.plannedDuration||task.duration||1));task.acceleratedAssignments=Array.isArray(task.acceleratedAssignments)?task.acceleratedAssignments:[];});return value;}
  ...
  function status(message){if(el('bootstrapScheduleStatus'))el('bootstrapScheduleStatus').textContent=message;}
  function install(){el('bootstrapSchedule')?.remove();document.body.insertAdjacentHTML('beforeend',`<section id="bootstrapSchedule" class="bootstrap-schedule bootstrap-build-schedule hidden" role="dialog" aria-modal="true" aria-hidden="true" aria-label="Bootstrap Build project schedules"><div class="bootstrap-schedule-shell"><header class="bootstrap-schedule-head"><img src="bootstrap_build_horizontal.png" alt="Bootstrap Build"><div><small>ADMIRAL-LEVEL PROGRAM · PROJECT SCHEDULES</small><h2 id="bootstrapScheduleTitleHeading">Construction Projects</h2><p id="bootstrapScheduleSubtitle">Choose a lot and unit to open its independent schedule.</p></div><div class="bootstrap-schedule-head-actions"><button id="bootstrapProjectsHome" class="hidden" type="button">ALL PROJECTS</button><button id="bootstrapFleetOpenFromSchedule" type="button">FLEET REGISTRY</button><button id="bootstrapScheduleClose" type="button">CLOSE</button></div></header><section id="bootstrapProjectLanding" class="bootstrap-project-landing"><div class="bootstrap-roster-summary"><div><small>ACTIVE TEST BUILDS</small><strong>3 independent project schedules</strong><span>Each lot and unit keeps its own dates, vendors, changes, and saved record.</span></div><span class="bootstrap-roster-badge">PROJECT ROSTER</span></div><div class="bootstrap-project-roster-wrap"><table class="bootstrap-project-roster"><thead><tr><th>Project</th><th>Division</th><th>Community</th><th>Start</th><th>Projected completion</th><th>Current task</th><th>Open issues</th><th>Schedule</th><th>Updated</th><th></th></tr></thead><tbody id="bootstrapProjectRows"></tbody></table></div></section><section id="bootstrapProjectWorkspace" class="hidden"><nav class="bootstrap-schedule-crumbs" aria-label="Project hierarchy"><span><small>REGION</small><b id="bootstrapRegionValue"></b></span><i>›</i><span><small>DIVISION</small><b id="bootstrapDivisionValue"></b></span><i>›</i><span><small>COMMUNITY</small><b id="bootstrapCommunityValue"></b></span><i>›</i><span><small>LOT</small><b id="bootstrapLotValue"></b></span><i>›</i><span><small>UNIT</small><b id="bootstrapUnitValue"></b></span></nav><div class="bootstrap-schedule-toolbar"><div><strong id="bootstrapScheduleRange"></strong></div><div class="bootstrap-schedule-legend" aria-label="Task colors"><span data-kind="work">WORK</span><span data-kind="site">SITE</span><span data-kind="inspection">INSPECTION</span><span data-kind="delivery">DELIVERY</span><span data-kind="milestone">MILESTONE</span></div><div><button id="bootstrapScheduleAdd" type="button">+ ADD TASK</button><button id="bootstrapScheduleReset" type="button">RESET THIS PROJECT</button></div></div><form id="bootstrapScheduleForm" class="bootstrap-schedule-form bootstrap-ground-form hidden"><label>Task<input id="bootstrapScheduleTaskTitle" maxlength="80" required placeholder="Construction task"></label><label>Trade<input id="bootstrapScheduleTrade" maxlength="40" required placeholder="Trade or crew"></label><label>Type<select id="bootstrapScheduleType"><option value="work">Work</option><option value="site">Site</option><option value="inspection">Inspection</option><option value="delivery">Delivery</option><option value="milestone">Milestone</option></select></label><label>Start<select id="bootstrapScheduleDay"></select></label><label>Days<input id="bootstrapScheduleDuration" type="number" min="1" max="15" value="1"></label><button type="submit">ADD TASK</button><button id="bootstrapScheduleCancel" type="button">CANCEL</button></form><p id="bootstrapScheduleStatus" class="bootstrap-schedule-status" role="status"></p><section id="bootstrapVendorTools" class="bootstrap-vendor-tools"><div><label>Schedule view<select id="bootstrapVendorView"><option value="">All work</option></select></label><span id="bootstrapVendorViewNote">All project work is visible.</span></div><div><label>Selected task vendor<select id="bootstrapVendorAssign"></select></label><button id="bootstrapVendorAssignButton" type="button" disabled>ASSIGN VENDOR</button></div></section><div id="bootstrapScheduleBoard" class="bootstrap-ground-board"></div><section id="bootstrapVendorActivityPanel" class="bootstrap-vendor-activity"><header><div><small>VENDOR COMMUNICATION LOG</small><b>Schedule changes vendors can see</b></div><span>LOCAL TEST · NO MESSAGE SENT</span></header><div id="bootstrapVendorActivity"></div></section><footer><b>Project boundary</b><span>Each test project is stored separately on this device. Moving work in one unit cannot move work in another.</span></footer></section></div></section>`);}
  function installQuickPassageControls(){
    const actions=el('bootstrapSchedule')?.querySelector('.bootstrap-schedule-toolbar>div:last-child');
    if(actions&&!el('bootstrapAcceleratedToggle'))actions.insertAdjacentHTML('afterbegin','<button id="bootstrapAcceleratedToggle" type="button" aria-expanded="false">ACCELERATED DAYS</button>');
    const toolbar=el('bootstrapSchedule')?.querySelector('.bootstrap-schedule-toolbar');
    if(toolbar&&!el('bootstrapAcceleratedPanel'))toolbar.insertAdjacentHTML('afterend','<section id="bootstrapAcceleratedPanel" class="bootstrap-accelerated-panel hidden"><header><div><small>MANAGER-CONTROLLED EXCEPTION</small><b>Activate a specific weekend shift</b><span>Monday–Friday stays standard. Weekend work remains visible, optional, and auditable.</span></div><button id="bootstrapAcceleratedClose" type="button">CLOSE</button></header><div class="bootstrap-accelerated-controls"><label>Week<select id="bootstrapAcceleratedWeek"></select></label><label>Day<select id="bootstrapAcceleratedDay"><option value="sat">Saturday</option><option value="sun">Sunday</option></select></label><label>Shift<select id="bootstrapAcceleratedShift"><option value="am">AM</option><option value="pm">PM</option><option value="full">Full day</option></select></label><label>Commitment<select id="bootstrapAcceleratedCommitment"><option value="proposed">Proposed</option><option value="confirmed">Vendor confirmed</option></select></label><button id="bootstrapAcceleratedAdd" type="button">ACTIVATE DATE</button></div><p class="bootstrap-accelerated-warning">⚠ Accelerated work may compress recovery time or create trade overlap. The manager may proceed; the exception and vendor commitment are recorded.</p><div id="bootstrapAcceleratedRows" class="bootstrap-accelerated-rows"></div></section>');
    const weeks=el('bootstrapAcceleratedWeek');if(weeks)weeks.innerHTML=Array.from({length:WEEKS},(_,week)=>'<option value="'+week+'">Week '+(week+1)+'</option>').join('');
  }
  function renderLanding(){const rows=el('bootstrapProjectRows');if(!rows)return;rows.innerHTML=projects.map(meta=>{const value=cache.get(meta.id)||fresh(meta),current=[...value.tasks].sort((a,b)=>a.start-b.start)[0],updated=value.updatedAt?new Date(value.updatedAt).toLocaleString(undefined,{month:'short',day:'numeric',hour:'numeric',minute:'2-digit'}):'Original plan';return '<tr><td><b>Lot '+esc(meta.lot)+' · Unit '+esc(meta.unit)+'</b><small>'+esc(meta.region)+' / '+esc(meta.division)+' / '+esc(meta.community)+'</small></td><td>'+esc(meta.division)+'</td><td>'+esc(meta.community)+'</td><td>'+esc(labelDate(0,true,meta))+'</td><td>'+esc(labelDate(DAYS-1,true,meta))+'</td><td><b>'+esc(current?.title||'No work')+'</b><small>'+esc(current?.trade||'')+'</small></td><td><span class="bootstrap-count-zero">0</span></td><td><span class="bootstrap-roster-status">PLANNED</span></td><td>'+esc(updated)+'</td><td><button type="button" data-open-project="'+esc(meta.id)+'">OPEN SCHEDULE</button></td></tr>';}).join('');}
  function showLanding(){activeId='';state=null;selectedId='';vendorView='';el('bootstrapProjectLanding')?.classList.remove('hidden');el('bootstrapProjectWorkspace')?.classList.add('hidden');el('bootstrapProjectsHome')?.classList.add('hidden');if(el('bootstrapScheduleTitleHeading'))el('bootstrapScheduleTitleHeading').textContent='Construction Projects';if(el('bootstrapScheduleSubtitle'))el('bootstrapScheduleSubtitle').textContent='Choose a lot and unit to open its independent schedule.';renderLanding();}
  function vendors(){return [...new Set(state.tasks.map(task=>task.vendor||vendorFor(task.trade)))].sort((a,b)=>a.localeCompare(b));}
  function segmentsForWeek(week){const first=week*5,last=first+5,segments=[];state.tasks.forEach(task=>{const start=Math.max(task.start,first),end=Math.min(task.start+Math.max(1,task.duration||1),last);if(start<end)segments.push({task,start,end});});segments.sort((a,b)=>a.start-b.start||b.end-a.end||a.task.title.localeCompare(b.task.title));const lanes=[];segments.forEach(segment=>{let lane=lanes.findIndex(end=>end<=segment.start);if(lane<0){lane=lanes.length;lanes.push(segment.end);}else lanes[lane]=segment.end;segment.lane=lane;});return {segments,lanes:Math.max(2,lanes.length)};}
  function weekendDate(entry){const date=anchor(project(activeId));date.setDate(date.getDate()+entry.week*7+(entry.day==='sun'?6:5));return date;}
  function weekendLabel(entry){return weekendDate(entry).toLocaleDateString(undefined,{weekday:'short',month:'short',day:'numeric'});}
  function renderAccelerated(){
    if(!state)return;
    const rows=el('bootstrapAcceleratedRows'),days=[...state.acceleratedDays].sort((a,b)=>a.week-b.week||a.day.localeCompare(b.day));
    if(rows)rows.innerHTML=days.length?days.map(entry=>{const assigned=state.tasks.filter(task=>task.acceleratedAssignments?.includes(entry.id));return '<article><div><small>'+esc(entry.commitment==='confirmed'?'VENDOR CONFIRMED':'PROPOSED')+'</small><b>'+esc(weekendLabel(entry))+' · '+esc(entry.shift.toUpperCase())+'</b><span>'+(assigned.length?assigned.map(task=>esc(task.title+' — '+task.vendor)).join('<br>'):'No task assigned yet. Select a task, then add it from this date.')+'</span></div><div>'+(selectedId?'<button type="button" data-weekend-assign="'+esc(entry.id)+'">ADD SELECTED TASK</button>':'')+'<button type="button" data-weekend-remove="'+esc(entry.id)+'">REMOVE</button></div></article>';}).join(''):'<p>No accelerated days are active. Saturday and Sunday remain off the schedule.</p>';
  }
  function riskWarnings(task){
    const warnings=[];
    if(task.duration<Math.max(1,task.plannedDuration||task.duration))warnings.push('Duration is shorter than the original plan');
    const conflict=state.tasks.find(other=>other.id!==task.id&&other.vendor===task.vendor&&task.start<other.start+other.duration&&other.start<task.start+task.duration);
    if(conflict)warnings.push(task.vendor+' overlaps '+conflict.title);
    return warnings;
  }
  function taskMarkup(task,start,end,lane,first){
    const begins=start===task.start,finishes=end===task.start+task.duration,warnings=Array.isArray(task.riskWarnings)?task.riskWarnings:[],weekendCount=task.acceleratedAssignments?.length||0;
    return '<article class="bootstrap-ground-task kind-'+esc(task.type||'work')+(selectedId===task.id?' is-selected':'')+(warnings.length?' has-risk':'')+'" draggable="true" tabindex="0" data-task-id="'+esc(task.id)+'" style="grid-column:'+(start-first+1)+' / span '+(end-start)+';grid-row:'+(lane+2)+'">'+(begins?'<button class="bootstrap-task-resize is-start" type="button" draggable="false" data-resize-edge="start" aria-label="Change start and duration of '+esc(task.title)+'">‹</button>':'')+'<small>'+esc(task.trade)+'</small><b>'+(begins?'':'← ')+esc(task.title)+(finishes?'':' →')+'</b><span>'+esc(task.duration)+' workday'+(task.duration===1?'':'s')+(weekendCount?' · '+weekendCount+' accelerated':'')+'</span><em class="bootstrap-task-vendor">'+esc(task.vendor)+'</em>'+(warnings.length?'<i class="bootstrap-task-risk" title="'+esc(warnings.join('. '))+'">!</i>':'')+(finishes?'<button class="bootstrap-task-resize is-end" type="button" draggable="false" data-resize-edge="end" aria-label="Change duration of '+esc(task.title)+'">›</button>':'')+'</article>';
  }
  function renderSchedule(){
    if(!state)return;const meta=project(activeId);
    [['bootstrapRegionValue','region'],['bootstrapDivisionValue','division'],['bootstrapCommunityValue','community'],['bootstrapLotValue','lot'],['bootstrapUnitValue','unit']].forEach(([id,key])=>{if(el(id))el(id).textContent=meta[key];});
    if(el('bootstrapScheduleRange'))el('bootstrapScheduleRange').textContent=labelDate(0)+' – '+workDate(DAYS-1).toLocaleDateString(undefined,{month:'short',day:'numeric',year:'numeric'})+' · 12 WEEKS · MON–FRI STANDARD';
    const select=el('bootstrapScheduleDay');if(select)select.innerHTML=Array.from({length:DAYS},(_,day)=>'<option value="'+day+'">'+esc(labelDate(day,true))+'</option>').join('');
    const board=el('bootstrapScheduleBoard');if(!board)return;
    board.innerHTML=Array.from({length:WEEKS},(_,week)=>{const first=week*5,{segments,lanes}=segmentsForWeek(week),accelerated=state.acceleratedDays.filter(entry=>entry.week===week);const heads=Array.from({length:5},(_,col)=>'<button type="button" class="bootstrap-ground-day-head'+(overDay===first+col?' is-over':'')+'" data-schedule-move="'+(first+col)+'" title="Move selected task here"><small>'+esc(workDate(first+col).toLocaleDateString(undefined,{weekday:'short'}))+'</small><b>'+esc(labelDate(first+col))+'</b></button>').join('');const cells=Array.from({length:5},(_,col)=>'<div class="bootstrap-ground-day-cell'+(overDay===first+col?' is-over':'')+'" data-schedule-day="'+(first+col)+'" style="grid-column:'+(col+1)+';grid-row:2 / span '+lanes+'"></div>').join('');const bars=segments.map(({task,start,end,lane})=>taskMarkup(task,start,end,lane,first)).join('');const acceleratedLane=accelerated.length?'<aside class="bootstrap-weekend-lane"><small>ACCELERATED EXCEPTION</small>'+accelerated.map(entry=>'<button type="button" data-open-accelerated="1"><b>'+esc(weekendLabel(entry))+'</b><span>'+esc(entry.shift.toUpperCase())+' · '+esc(entry.commitment.replace('_',' ').toUpperCase())+'</span></button>').join('')+'</aside>':'';return '<section class="bootstrap-ground-week"><header><small>WEEK '+(week+1)+'</small><b>'+esc(labelDate(first))+' – '+esc(labelDate(first+4))+'</b></header><div class="bootstrap-ground-week-grid" data-week="'+week+'" style="--task-lanes:'+lanes+'">'+heads+cells+bars+'</div>'+acceleratedLane+'</section>';}).join('');
    renderVendorLayer();renderAccelerated();
  }
  function activityCopy(item){if(item.assignment)return item.task+' reassigned from '+item.from+' to '+item.to+'.';if(item.type==='resize')return item.task+' duration changed from '+item.from+' to '+item.to+'.';if(item.type==='weekend')return item.task+' added to '+item.to+' as an accelerated exception.';if(item.type==='weekend-open')return item.to+' activated for accelerated work.';return item.task+' moved from '+item.from+' to '+item.to+'.';}
  function renderVendorLayer(){
    if(!state)return;const options=vendors(),view=el('bootstrapVendorView'),assign=el('bootstrapVendorAssign');
    if(view){view.innerHTML='<option value="">All work</option>'+options.map(v=>'<option value="'+esc(v)+'">'+esc(v)+'</option>').join('');view.value=vendorView;}
    if(assign){assign.innerHTML=options.map(v=>'<option value="'+esc(v)+'">'+esc(v)+'</option>').join('');const chosen=state.tasks.find(task=>task.id===selectedId);if(chosen)assign.value=chosen.vendor;}
    if(el('bootstrapVendorAssignButton'))el('bootstrapVendorAssignButton').disabled=!selectedId;
    if(el('bootstrapVendorViewNote'))el('bootstrapVendorViewNote').textContent=vendorView?vendorView+' sees only its assigned work and schedule changes.':'Select a task to move it, resize it, assign a vendor, or add an accelerated shift.';
    document.querySelectorAll('#bootstrapSchedule [data-task-id]').forEach(card=>{const task=state.tasks.find(item=>item.id===card.dataset.taskId);card.classList.toggle('vendor-hidden',!!vendorView&&task?.vendor!==vendorView);});
    const activity=el('bootstrapVendorActivity');if(activity)activity.innerHTML=state.activity.length?state.activity.slice(0,12).map(item=>'<article><b>'+esc(item.vendor||'Construction Manager')+'</b><span>'+esc(activityCopy(item))+'</span><small>'+esc(new Date(item.at).toLocaleString())+'</small></article>').join(''):'<p>No schedule changes are recorded for this project.</p>';
  }
  async function openProject(id){activeId=id;state=cache.get(id)||await loadProject(project(id));selectedId='';vendorView='';el('bootstrapProjectLanding')?.classList.add('hidden');el('bootstrapProjectWorkspace')?.classList.remove('hidden');el('bootstrapProjectsHome')?.classList.remove('hidden');const meta=project(id);el('bootstrapScheduleTitleHeading').textContent='Lot '+meta.lot+' · Unit '+meta.unit;el('bootstrapScheduleSubtitle').textContent='Move task bodies, resize from either edge, and authorize only the accelerated weekends you need.';renderSchedule();status(state._source==='recovered prior schedule'?'Prior Lot 1234 · Unit A schedule recovered. Changes now save in its independent record.':state.updatedAt?'Loaded saved revision '+state.revision+' from '+new Date(state.updatedAt).toLocaleString()+'.':'Original project plan loaded. Drag a task body to move it or an edge to change duration.');el('bootstrapSchedule').scrollTop=0;}
  async function move(id,day){
    const task=state?.tasks.find(item=>item.id===id);if(!task)return;const from=task.start,next=Math.max(0,Math.min(DAYS-Math.max(1,task.duration||1),Number(day)));task.start=next;task.riskWarnings=riskWarnings(task);
    if(from!==next)state.activity.unshift({id:'move-'+Date.now(),at:new Date().toISOString(),task:task.title,vendor:task.vendor,from:labelDate(from,true),to:labelDate(next,true)});
    state.activity=state.activity.slice(0,100);selectedId='';showOver(null);renderSchedule();document.querySelectorAll('#bootstrapSchedule [data-task-id="'+CSS.escape(task.id)+'"]').forEach(node=>node.classList.add('is-settling'));
    if(from===next){status(task.title+' already begins on '+labelDate(next,true)+'.');return;}
    const warning=task.riskWarnings.length?' Warning recorded: '+task.riskWarnings.join('; ')+'.':'';status(task.title+' moved to '+labelDate(next,true)+'.'+warning+' Saving this project…');await persist(task.duration+'-workday duration preserved; '+task.vendor+' can see the change.'+warning);
  }
  async function resizeTask(id,edge,day){
    const task=state?.tasks.find(item=>item.id===id);if(!task)return;const oldStart=task.start,oldDuration=Math.max(1,task.duration||1),oldEnd=oldStart+oldDuration;
    if(edge==='start'){const nextStart=Math.max(0,Math.min(oldEnd-1,Number(day)));task.start=nextStart;task.duration=oldEnd-nextStart;}else{const nextEnd=Math.max(task.start+1,Math.min(DAYS,Number(day)+1));task.duration=nextEnd-task.start;}
    if(task.start===oldStart&&task.duration===oldDuration){showOver(null);renderSchedule();status('Duration unchanged. Drag the edge into another workday.');return;}
    task.riskWarnings=riskWarnings(task);state.activity.unshift({id:'resize-'+Date.now(),type:'resize',at:new Date().toISOString(),task:task.title,vendor:task.vendor,from:oldDuration+' workday'+(oldDuration===1?'':'s'),to:task.duration+' workday'+(task.duration===1?'':'s')});state.activity=state.activity.slice(0,100);selectedId=task.id;showOver(null);renderSchedule();
    const warning=task.riskWarnings.length?' Calculated-risk warning: '+task.riskWarnings.join('; ')+'. The manager may proceed.':'';status(task.title+' now runs '+task.duration+' workday'+(task.duration===1?'':'s')+'.'+warning);await persist('Duration saved.'+warning);
  }
  function gridRects(){return [...document.querySelectorAll('.bootstrap-ground-week-grid')].map(grid=>({week:Number(grid.dataset.week),rect:grid.getBoundingClientRect()}));}
  function dayAtPoint(x,y,rects=gridRects()){for(const item of rects){const r=item.rect;if(x>=r.left&&x<=r.right&&y>=r.top&&y<=r.bottom)return item.week*5+Math.max(0,Math.min(4,Math.floor((x-r.left)/(r.width/5))));}return null;}
  function showOver(day){if(day===overDay)return;const prior=overDay;overDay=day;if(prior!==null)document.querySelectorAll('[data-schedule-day="'+prior+'"],[data-schedule-move="'+prior+'"]').forEach(node=>node.classList.remove('is-over'));if(day!==null)document.querySelectorAll('[data-schedule-day="'+day+'"],[data-schedule-move="'+day+'"]').forEach(node=>node.classList.add('is-over'));}
  function makeDragGhost(card,event){const rect=card.getBoundingClientRect(),ghost=card.cloneNode(true);ghost.removeAttribute('draggable');ghost.removeAttribute('tabindex');ghost.classList.remove('is-selected');ghost.classList.add('bootstrap-drag-ghost');ghost.style.width=rect.width+'px';ghost.style.left=rect.left+'px';ghost.style.top=rect.top+'px';document.body.appendChild(ghost);return {node:ghost,offsetX:event.clientX-rect.left,offsetY:event.clientY-rect.top};}
  function moveDragGhost(drag,event){if(!drag?.ghost)return;drag.ghost.node.style.transform='translate3d('+(event.clientX-drag.ghost.offsetX-parseFloat(drag.ghost.node.style.left))+'px,'+(event.clientY-drag.ghost.offsetY-parseFloat(drag.ghost.node.style.top))+'px,0)';}
  function clearTouchDrag(){touchDrag?.ghost?.node?.remove();touchDrag=null;}
  ...
  install();installQuickPassageControls();syncDoor();new MutationObserver(syncDoor).observe(document.body,{subtree:true,childList:true});window.DarkSkyOpenBootstrapSchedule=open;
  document.addEventListener('click',async event=>{const target=event.target;if(target.closest('#bootstrapScheduleOpen')){event.preventDefault();event.stopImmediatePropagation();open();return;}if(!target.closest('#bootstrapSchedule'))return;if(target.closest('#bootstrapFleetOpenFromSchedule'))return;if(target.closest('#bootstrapScheduleClose')){event.preventDefault();event.stopImmediatePropagation();el('bootstrapSchedule').classList.add('hidden');el('bootstrapSchedule').setAttribute('aria-hidden','true');return;}if(target.closest('#bootstrapProjectsHome')){event.preventDefault();event.stopImmediatePropagation();showLanding();el('bootstrapSchedule').scrollTop=0;return;}const rowButton=target.closest('[data-open-project]');if(rowButton){event.preventDefault();event.stopImmediatePropagation();await openProject(rowButton.dataset.openProject);return;}if(!state)return;if(target.closest('#bootstrapScheduleAdd')){event.preventDefault();event.stopImmediatePropagation();el('bootstrapScheduleForm').classList.remove('hidden');el('bootstrapScheduleTaskTitle').focus();return;}if(target.closest('#bootstrapScheduleCancel')){event.preventDefault();event.stopImmediatePropagation();el('bootstrapScheduleForm').classList.add('hidden');return;}if(target.closest('#bootstrapScheduleReset')){event.preventDefault();event.stopImmediatePropagation();const meta=project(activeId);if(window.confirm('Reset only Lot '+meta.lot+' · Unit '+meta.unit+' to its original plan?')){state=fresh(meta);cache.set(activeId,state);selectedId='';renderSchedule();await persist('This project was reset.');}return;}const moveButton=target.closest('[data-schedule-move]');if(moveButton&&selectedId){event.preventDefault();event.stopImmediatePropagation();move(selectedId,Number(moveButton.dataset.scheduleMove));return;}const card=target.closest('[data-task-id]');if(card){event.preventDefault();event.stopImmediatePropagation();if(Date.now()<suppressClickUntil)return;selectedId=selectedId===card.dataset.taskId?'':card.dataset.taskId;renderSchedule();status(selectedId?'Task selected. Drag the body to move, drag either edge to resize, or add an accelerated shift.':'Task selection cleared.');}},{capture:true});
  document.addEventListener('click',async event=>{
    const target=event.target;if(!target.closest?.('#bootstrapSchedule')||!state)return;
    const toggle=target.closest('#bootstrapAcceleratedToggle,[data-open-accelerated]');if(toggle){event.preventDefault();const panel=el('bootstrapAcceleratedPanel'),opening=panel?.classList.contains('hidden');panel?.classList.toggle('hidden',!opening);el('bootstrapAcceleratedToggle')?.setAttribute('aria-expanded',opening?'true':'false');if(opening){renderAccelerated();panel?.scrollIntoView({behavior:'smooth',block:'nearest'});}return;}
    if(target.closest('#bootstrapAcceleratedClose')){event.preventDefault();el('bootstrapAcceleratedPanel')?.classList.add('hidden');el('bootstrapAcceleratedToggle')?.setAttribute('aria-expanded','false');return;}
    if(target.closest('#bootstrapAcceleratedAdd')){event.preventDefault();const week=Number(el('bootstrapAcceleratedWeek')?.value||0),day=el('bootstrapAcceleratedDay')?.value==='sun'?'sun':'sat',shift=el('bootstrapAcceleratedShift')?.value||'am',commitment=el('bootstrapAcceleratedCommitment')?.value||'proposed',id='weekend-'+week+'-'+day;let entry=state.acceleratedDays.find(item=>item.id===id);if(entry){entry.shift=shift;entry.commitment=commitment;}else{entry={id,week,day,shift,commitment,createdAt:new Date().toISOString()};state.acceleratedDays.push(entry);}state.activity.unshift({id:'weekend-open-'+Date.now(),type:'weekend-open',at:new Date().toISOString(),vendor:'Construction Manager',task:'Accelerated shift',from:'Weekend off',to:weekendLabel(entry)+' · '+shift.toUpperCase()+' · '+commitment.replace('_',' ')});renderSchedule();await persist(weekendLabel(entry)+' activated as an accelerated '+shift.toUpperCase()+' exception.');return;}
    const assign=target.closest('[data-weekend-assign]');if(assign){event.preventDefault();const task=state.tasks.find(item=>item.id===selectedId),entry=state.acceleratedDays.find(item=>item.id===assign.dataset.weekendAssign);if(!task||!entry)return;task.acceleratedAssignments=Array.isArray(task.acceleratedAssignments)?task.acceleratedAssignments:[];if(!task.acceleratedAssignments.includes(entry.id))task.acceleratedAssignments.push(entry.id);state.activity.unshift({id:'weekend-'+Date.now(),type:'weekend',at:new Date().toISOString(),task:task.title,vendor:task.vendor,from:'Standard plan',to:weekendLabel(entry)+' · '+entry.shift.toUpperCase()});renderSchedule();await persist(task.title+' added to '+weekendLabel(entry)+'. Warning retained; standard weekdays remain unchanged.');return;}
    const remove=target.closest('[data-weekend-remove]');if(remove){event.preventDefault();const entry=state.acceleratedDays.find(item=>item.id===remove.dataset.weekendRemove);state.acceleratedDays=state.acceleratedDays.filter(item=>item.id!==remove.dataset.weekendRemove);state.tasks.forEach(task=>task.acceleratedAssignments=(task.acceleratedAssignments||[]).filter(id=>id!==remove.dataset.weekendRemove));renderSchedule();await persist((entry?weekendLabel(entry):'Weekend shift')+' removed; Saturday and Sunday return to off.');}
  },{capture:true});
  document.addEventListener('submit',async event=>{if(event.target.id!=='bootstrapScheduleForm'||!state)return;event.preventDefault();event.stopImmediatePropagation();const title=el('bootstrapScheduleTaskTitle')?.value?.trim(),trade=el('bootstrapScheduleTrade')?.value?.trim(),start=Number(el('bootstrapScheduleDay')?.value||0),duration=Math.max(1,Math.min(15,Number(el('bootstrapScheduleDuration')?.value||1))),type=el('bootstrapScheduleType')?.value||'work';if(!title||!trade)return;state.tasks.push({id:'task-'+Date.now(),title,trade,start:Math.min(start,DAYS-duration),duration,plannedDuration:duration,type,vendor:vendorFor(trade),acceleratedAssignments:[]});event.target.reset();el('bootstrapScheduleDuration').value='1';event.target.classList.add('hidden');renderSchedule();await persist(title+' added to Lot '+project(activeId).lot+' · Unit '+project(activeId).unit+'.');},{capture:true});
  document.addEventListener('dragstart',event=>{if(event.target.closest?.('[data-resize-edge]')){event.preventDefault();return;}const card=event.target.closest?.('#bootstrapSchedule [data-task-id]');if(!card||!state)return;event.stopImmediatePropagation();selectedId=card.dataset.taskId;event.dataTransfer.effectAllowed='move';event.dataTransfer.setData('text/plain',selectedId);card.classList.add('is-dragging');},{capture:true});
  document.addEventListener('dragover',event=>{if(!event.target.closest?.('#bootstrapSchedule')||!state)return;const day=dayAtPoint(event.clientX,event.clientY);if(day===null)return;event.preventDefault();event.stopImmediatePropagation();showOver(day);},{capture:true});
  document.addEventListener('drop',event=>{if(!event.target.closest?.('#bootstrapSchedule')||!state)return;const day=dayAtPoint(event.clientX,event.clientY);if(day===null)return;event.preventDefault();event.stopImmediatePropagation();move(event.dataTransfer.getData('text/plain')||selectedId,day);},{capture:true});
  document.addEventListener('dragend',event=>{if(!event.target.closest?.('#bootstrapSchedule'))return;event.stopImmediatePropagation();showOver(null);document.querySelectorAll('.bootstrap-ground-task').forEach(node=>node.classList.remove('is-dragging'));},{capture:true});
  document.addEventListener('pointerdown',event=>{const card=event.target.closest?.('#bootstrapSchedule [data-task-id]');if(!card||!state)return;const handle=event.target.closest?.('[data-resize-edge]');if(event.pointerType==='mouse'&&!handle)return;event.preventDefault();event.stopImmediatePropagation();selectedId=card.dataset.taskId;const mode=handle?'resize-'+handle.dataset.resizeEdge:'move';touchDrag={id:selectedId,mode,pointerId:event.pointerId,rects:gridRects(),ghost:handle?null:makeDragGhost(card,event)};(handle||card).setPointerCapture?.(event.pointerId);card.classList.add(handle?'is-resizing':'is-dragging');if(!handle)moveDragGhost(touchDrag,event);},{capture:true});
  document.addEventListener('pointermove',event=>{if(!touchDrag||event.pointerId!==touchDrag.pointerId)return;event.preventDefault();event.stopImmediatePropagation();if(touchDrag.mode==='move')moveDragGhost(touchDrag,event);showOver(dayAtPoint(event.clientX,event.clientY,touchDrag.rects));},{capture:true,passive:false});
  document.addEventListener('pointerup',event=>{if(!touchDrag||event.pointerId!==touchDrag.pointerId)return;event.preventDefault();event.stopImmediatePropagation();const drag=touchDrag,id=drag.id,mode=drag.mode,day=dayAtPoint(event.clientX,event.clientY,drag.rects);clearTouchDrag();suppressClickUntil=Date.now()+350;if(day!==null){if(mode.startsWith('resize-'))resizeTask(id,mode.replace('resize-',''),day);else move(id,day);}else{showOver(null);renderSchedule();status(mode.startsWith('resize-')?'Duration unchanged. Drag an edge across a workday.':'Task selected. Tap a workday heading to move it.');}},{capture:true});
  document.addEventListener('pointercancel',event=>{if(!touchDrag)return;event.stopImmediatePropagation();clearTouchDrag();showOver(null);renderSchedule();},{capture:true});
  document.addEventListener('keydown',event=>{const card=event.target.closest?.('#bootstrapSchedule [data-task-id]');if(!card||!state||!['ArrowLeft','ArrowRight'].includes(event.key))return;event.preventDefault();event.stopImmediatePropagation();const task=state.tasks.find(item=>item.id===card.dataset.taskId),handle=event.target.closest?.('[data-resize-edge]');if(!task)return;if(handle){const edge=handle.dataset.resizeEdge,day=edge==='start'?task.start+(event.key==='ArrowRight'?1:-1):task.start+task.duration-1+(event.key==='ArrowRight'?1:-1);resizeTask(task.id,edge,day);}else move(task.id,task.start+(event.key==='ArrowRight'?1:-1));},{capture:true});
  document.addEventListener('change',event=>{if(event.target.id!=='bootstrapVendorView'||!state)return;vendorView=event.target.value;renderSchedule();status(vendorView?'Vendor view opened for '+vendorView+'.':'All project work is visible.');},{capture:true});
  document.addEventListener('click',async event=>{if(!event.target.closest?.('#bootstrapVendorAssignButton')||!state)return;event.preventDefault();event.stopImmediatePropagation();const task=state.tasks.find(item=>item.id===selectedId),vendor=el('bootstrapVendorAssign')?.value;if(!task||!vendor)return;const prior=task.vendor;task.vendor=vendor;state.activity.unshift({id:'assign-'+Date.now(),at:new Date().toISOString(),task:task.title,vendor,from:prior,to:vendor,assignment:true});selectedId='';renderSchedule();await persist(task.title+' assigned to '+vendor+'.');},{capture:true});
})();
...
;(()=>{
  ...
  async function signIn(emailValue='',passwordValue=''){
    const c=cfg(),email=(emailValue||el('admiralIdentityEmail')?.value||'').trim().toLowerCase(),password=passwordValue||el('admiralIdentityPassword')?.value||'';
  ...
  window.DarkSkyAdmiralIdentityStatus=verifyAdmiral;
  window.DarkSkyAuthenticateAdmiral=async(email,password)=>{await signIn(email,password);return verifyAdmiral();};
  window.DarkSkySignOutAdmiral=()=>{clearSession();return true;};
...
/* Fleet Watchtower owns the final daily My Fleet route after legacy modules load. */
if(window.DarkSkyOpenFleetWatchtower)window.DarkSkyOpenAdmiralFleet=window.DarkSkyOpenFleetWatchtower;


/* 8.8.15.8 Station Registry — readable Control Plane, durable course preview and human Activity Spine. */
;(()=>{
  'use strict';
  const SESSION_KEY='darkSkySupabaseAdmiralSessionV1';
  const $=id=>document.getElementById(id),esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
  const cfg=()=>window.BlackFlagV3Identity?.productionAuth?.readClientConfig?.()||null;
  const headers=(token='')=>{const c=cfg(),h={apikey:c?.publishableKey||'','Content-Type':'application/json'};if(token)h.Authorization='Bearer '+token;return h;};
  const readSession=()=>{try{return JSON.parse(sessionStorage.getItem(SESSION_KEY)||'null')}catch(_){return null}};
  const saveSession=data=>{if(!data?.access_token)return null;const prior=readSession();const s={access_token:data.access_token,refresh_token:data.refresh_token||prior?.refresh_token||'',expires_at:Date.now()+Math.max(60,Number(data.expires_in||3600))*1000,user:data.user||prior?.user||null};sessionStorage.setItem(SESSION_KEY,JSON.stringify(s));return s;};
  const clearSession=()=>{try{sessionStorage.removeItem(SESSION_KEY)}catch(_){}};
  const READ_DEADLINE_MS=9000;
  let sessionRefreshPromise=null,readPromise=null,readGeneration=0,lastReadAt=0,readAbort=null;
  async function timedFetch(url,options={},timeout=READ_DEADLINE_MS){const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),timeout);try{return await fetch(url,{...options,signal:controller.signal});}catch(err){if(err?.name==='AbortError')throw new Error('Fleet Core read timed out. Retry when the connection is ready.');throw err;}finally{clearTimeout(timer);}}
  async function currentSession(){let s=readSession(),c=cfg();if(s&&s.expires_at>Date.now()+30000)return s;if(!s?.refresh_token||!c)return null;if(sessionRefreshPromise)return sessionRefreshPromise;sessionRefreshPromise=(async()=>{const r=await timedFetch(c.url+'/auth/v1/token?grant_type=refresh_token',{method:'POST',headers:headers(),body:JSON.stringify({refresh_token:s.refresh_token})});if(!r.ok){clearSession();return null;}return saveSession(await r.json());})().finally(()=>{sessionRefreshPromise=null;});return sessionRefreshPromise;}
  async function rpc(name,body={}){const c=cfg(),s=await currentSession();if(!c?.url||!s?.access_token)throw new Error('Admiral session expired. Re-authenticate; no fleet state changed.');const r=await timedFetch(c.url+'/rest/v1/rpc/'+name,{method:'POST',headers:{...headers(s.access_token),Accept:'application/json'},body:JSON.stringify(body)});if(!r.ok){if(r.status===401||r.status===403){clearSession();throw new Error('Admiral session expired or lost authority. Re-authenticate; no fleet state changed.');}let m='Fleet Core refused the command.';try{const e=await r.json();m=e?.message||e?.hint||m}catch(_){}throw new Error(String(m).replaceAll('_',' '));}return r.json();}
  let snapshot=null,preview=null,previewSignature='',selectedProject='';
  function setStatus(message,kind='info'){const node=$('fleetSpineStatus');if(!node)return;node.textContent=message;node.dataset.kind=kind;}
  function install(){if($('fleetSpineWorkspace'))return;document.body.insertAdjacentHTML('beforeend',`<section id="fleetSpineWorkspace" class="fleet-spine-workspace hidden" role="dialog" aria-modal="true" aria-hidden="true" aria-label="Fleet Spine Control Plane"><div class="fleet-spine-shell"><header><div><small>ADMIRAL CONTROL PLANE • 8.8.15.8 · SPINE CONTINUITY</small><h2>Fleet Spine</h2><p>One durable view of vessel identity, authority posture, release rings, station identity and the Activity Spine.</p></div><div><button id="fleetSpineRefresh" type="button">REFRESH SPINE</button><button id="fleetSpineClose" type="button">CLOSE</button></div></header><section class="fleet-spine-law"><b>OWNER OPERATES THE BUSINESS · ENGINE OPERATES THE PLATFORM · CAPTAIN COMMANDS OPERATIONS · ADMIRAL GOVERNS THE FLEET</b><span>Control Plane records coordinate the fleet. Private vessel business payloads remain vessel-scoped.</span></section><section id="fleetSpineIdentity" class="fleet-spine-identity"><div><small>SERVER AUTHORITY</small><strong id="fleetSpineIdentityState">ADMIRAL IDENTITY REQUIRED</strong></div><label>Email<input id="fleetSpineEmail" type="email" autocomplete="username" autocapitalize="none"></label><label>Password<input id="fleetSpinePassword" type="password" autocomplete="current-password"></label><button id="fleetSpineSignIn" type="button">AUTHENTICATE ADMIRAL</button><button id="fleetSpineSignOut" type="button" class="hidden">SIGN OUT</button></section><div id="fleetSpineStatus" class="fleet-spine-status" role="status" aria-live="polite">Fleet Spine closed until Admiral identity is verified.</div><section id="fleetSpineCounts" class="fleet-spine-counts"></section><main class="fleet-spine-grid"><section><header><small>CONTROL PLANE</small><h3>Vessels & release rings</h3><p class="fleet-spine-section-help">Select one vessel to inspect its course. Selection alone never changes it.</p></header><div id="fleetSpineVessels" class="fleet-spine-vessels"></div></section><aside><header><small>COURSE TABLE</small><h3>Release assignment</h3><p class="fleet-spine-section-help">Preview first. Issue only after the before-and-after course is visible.</p></header><div id="fleetSpineCourse" class="fleet-spine-course"><p>Select one vessel. Reading never changes it.</p></div></aside></main><section class="fleet-spine-activity"><header><div><small>ACTIVITY SPINE</small><h3>Recent durable fleet events</h3><p class="fleet-spine-section-help">Human-readable fleet history. Technical payloads stay behind Details.</p></div><span>APPEND-ONLY · BOUNDED PAYLOAD</span></header><div id="fleetSpineEvents"></div></section><footer><b>Fleet-first doctrine</b><span>Ike’s and every other vessel inherit shared infrastructure only after a capability is proven. No vessel-specific patch silently becomes Fleet Core.</span></footer></div></section>`);}
  function counts(){const c=snapshot?.counts||{},history=Number(c.release_assignment_records||0),pairs=[['VESSELS',c.vessels,'Fleet Core'],['ACTIVE MEMBERSHIPS',c.active_memberships,'Current authority'],['STATIONS',c.stations,'Registered endpoints'],['ACTIVITY EVENTS',c.activity_events,'Durable history'],['FOUNDRY CANDIDATES',c.foundry_candidates,'Active candidates'],['ACTIVE RELEASES',c.active_release_assignments,history+' HISTORY RECORD'+(history===1?'':'S')]];$('fleetSpineCounts').innerHTML=pairs.map(([k,v,n])=>`<article><small>${k}</small><strong>${Number(v||0)}</strong><span>${esc(n)}</span></article>`).join('');}
  function vesselRows(){const rows=Array.isArray(snapshot?.vessels)?snapshot.vessels:[];$('fleetSpineVessels').innerHTML=rows.length?rows.map(v=>{const selected=selectedProject===v.project_id;return `<button type="button" class="fleet-spine-vessel ${selected?'is-selected':''}" data-spine-vessel="${esc(v.project_id)}" aria-pressed="${selected?'true':'false'}"><div><small>${esc(String(v.lifecycle_state||'unknown').toUpperCase())} · ${esc(v.release_ring||'unassigned')}</small><b>${esc(v.display_name)}</b><span>${esc(v.project_id)}</span>${selected?'<em class="fleet-spine-selected-badge">SELECTED</em>':''}</div><div><strong>${esc(v.current_release||'NO RELEASE ASSIGNED')}</strong><span>${esc(String(v.rollout_state||'hold').replaceAll('_',' ').toUpperCase())} · ${Number(v.station_count||0)} STATIONS · ${Number(v.membership_count||0)} MEMBERS</span></div></button>`;}).join(''):'<p>No Fleet Core vessels found.</p>';}
  function humanEventTitle(type=''){const map={'fleet.spine.foundation.commissioned':'Fleet Spine commissioned','release.assignment.issued':'Release course issued','release.assignment.changed':'Release assignment changed','foundry.candidate.saved':'Foundry candidate saved','foundry.candidate.archived':'Foundry candidate archived','station.identity.registered':'Station identity registered','station.identity.updated':'Station identity updated','station.recovery.recorded':'Station recovery recorded'};return map[type]||String(type||'Fleet event').split('.').map(x=>x.charAt(0).toUpperCase()+x.slice(1)).join(' · ');}
  function eventSummary(e){const p=e?.payload||{},type=String(e?.event_type||'');if(type==='fleet.spine.foundation.commissioned')return `Control Plane foundation recorded for build ${p.build||'—'}${p.release?` · ${p.release}`:''}.`;if(type==='release.assignment.issued'||type==='release.assignment.changed'){const before=p.before||{},after=p.after||{};return `${p.project_id||'Vessel'} · ${String(before.release_ring||'unassigned').toUpperCase()} → ${String(after.release_ring||p.release_ring||'unassigned').toUpperCase()}${after.current_release||p.current_release?` · ${after.current_release||p.current_release}`:''}.`; }if(type==='foundry.candidate.saved')return `${p.status||'Candidate'} · ${p.product_route||'Foundry route'} · clearance record retained.`;if(type==='foundry.candidate.archived')return `Foundry candidate archived${p.reason?` · ${p.reason}`:''}.`;if(type==='station.identity.registered'||type==='station.identity.updated')return `${p.project_id||'Vessel'} · ${p.display_name||p.station_key||'Station'} · ${String(p.operating_mode||'web').toUpperCase()} · ${String(p.station_state||'registered').replaceAll('_',' ').toUpperCase()}.`;if(type==='station.recovery.recorded')return `${p.project_id||'Vessel'} · ${p.station_key||'Station'} · recovery #${Number(p.recovery_count||0)}${p.reason?` · ${p.reason}`:''}.`;return `Fleet event retained from ${e?.source||'Control Plane'}.`;}
  function eventRows(){const rows=Array.isArray(snapshot?.recent_events)?snapshot.recent_events:[];$('fleetSpineEvents').innerHTML=rows.length?rows.map(e=>`<article class="fleet-spine-event"><div class="fleet-spine-event-main"><small>${esc(new Date(e.created_at).toLocaleString())} · ${esc(e.actor_authority||'system')}</small><b>${esc(humanEventTitle(e.event_type))}</b><p>${esc(eventSummary(e))}</p><span>${esc(e.source||'fleet')} · durable event</span></div><details><summary>TECHNICAL DETAILS</summary><div><small>Correlation</small><code>${esc(e.correlation_id||'—')}</code><small>Event type</small><code>${esc(e.event_type||'—')}</code><small>Bounded payload</small><pre>${esc(JSON.stringify(e.payload||{},null,2))}</pre></div></details></article>`).join(''):'<p>No Activity Spine events yet.</p>';}
  function selectedVessel(){return (snapshot?.vessels||[]).find(v=>v.project_id===selectedProject)||null;}
  function courseValues(){return {project:selectedProject,ring:$('fleetSpineRing')?.value||'',rollout:$('fleetSpineRollout')?.value||'',release:$('fleetSpineRelease')?.value.trim()||'',intent:$('fleetSpineIntent')?.value.trim()||''};}
  function courseSignature(){return JSON.stringify(courseValues());}
  function invalidatePreview(message='Course details changed. Preview again before issuing.'){preview=null;previewSignature='';const box=$('fleetSpinePreviewBox'),issue=$('fleetSpineIssue');if(box){box.className='fleet-spine-preview is-stale';box.innerHTML=`<div class="fleet-spine-preview-state"><small>PREVIEW REQUIRED</small><strong>${esc(message)}</strong><span>No fleet state has changed.</span></div>`;}if(issue){issue.disabled=true;issue.setAttribute('aria-disabled','true');} }
  function course(){const v=selectedVessel(),host=$('fleetSpineCourse');preview=null;previewSignature='';if(!v){host.innerHTML='<div class="fleet-spine-course-empty"><strong>Select one vessel</strong><span>Reading never changes fleet state.</span></div>';return;}host.innerHTML=`<div class="fleet-spine-course-current"><small>SELECTED VESSEL · CURRENT COURSE</small><b>${esc(v.display_name)}</b><span>${esc(String(v.release_ring||'unassigned').toUpperCase())} · ${esc(v.current_release||'NO RELEASE')} · ${esc(String(v.rollout_state||'hold').replaceAll('_',' ').toUpperCase())}</span></div><label>Release ring<select id="fleetSpineRing">${['unassigned','canary','proving','limited','fleet'].map(x=>`<option value="${x}" ${x===v.release_ring?'selected':''}>${x.toUpperCase()}</option>`).join('')}</select></label><label>Rollout state<select id="fleetSpineRollout">${['hold','candidate','sea_trial','proven','live','rollback_ready'].map(x=>`<option value="${x}" ${x===v.rollout_state?'selected':''}>${x.replaceAll('_',' ').toUpperCase()}</option>`).join('')}</select></label><label>Release identity<input id="fleetSpineRelease" value="${esc(v.current_release||'')}" placeholder="Example: 8.8.15.8"></label><label>Admiral intent<textarea id="fleetSpineIntent" maxlength="500" rows="3" placeholder="Why this exact vessel should change course"></textarea></label><button id="fleetSpinePreview" class="fleet-spine-preview-action" type="button"><span>PREVIEW COURSE</span><small>READ-ONLY · NOTHING CHANGES</small></button><div id="fleetSpinePreviewBox" class="fleet-spine-preview is-awaiting" role="status" aria-live="polite"><div class="fleet-spine-preview-state"><small>AWAITING PREVIEW</small><strong>Exact before-and-after course will appear here.</strong><span>Issue remains locked until Fleet Core returns a fresh fingerprint.</span></div></div><button id="fleetSpineIssue" class="fleet-spine-issue-action" type="button" disabled aria-disabled="true">ISSUE RELEASE COURSE</button>`;}
  async function syncIdentity(){setStatus('VERIFYING ADMIRAL IDENTITY · Fleet data remains concealed until authority is current.','working');let ok=false;try{ok=!!(await Promise.race([Promise.resolve(window.DarkSkyAdmiralIdentityStatus?.()),new Promise((_,reject)=>setTimeout(()=>reject(new Error('Admiral identity verification timed out.')),READ_DEADLINE_MS))]));}catch(err){setStatus('ADMIRAL IDENTITY CHECK STOPPED · '+(err?.message||err),'error');throw err;}$('fleetSpineWorkspace')?.classList.toggle('identity-verified',ok);$('fleetSpineIdentityState').textContent=ok?'ADMIRAL IDENTITY VERIFIED':'ADMIRAL IDENTITY REQUIRED';['fleetSpineEmail','fleetSpinePassword'].forEach(id=>$(id)?.closest('label')?.classList.toggle('hidden',ok));$('fleetSpineSignIn')?.classList.toggle('hidden',ok);$('fleetSpineSignOut')?.classList.toggle('hidden',!ok);if(ok)await load();else{snapshot=null;setStatus('Authenticate the Admiral account. Fleet Spine remains closed.','warning');$('fleetSpineCounts').innerHTML='';$('fleetSpineVessels').innerHTML='';$('fleetSpineEvents').innerHTML='';course();window.dispatchEvent(new CustomEvent('darksky:fleet-spine-snapshot',{detail:{snapshot:null,generation:readGeneration,at:Date.now()}}));}return ok;}
  async function load({preserveStatus=false,force=false}={}){if(readPromise&&!force)return readPromise;if(force&&readAbort){try{readAbort.abort()}catch(_){}}const generation=++readGeneration;setStatus('READING CONTROL PLANE · One bounded server read is in progress.','working');const task=(async()=>{const data=await rpc('admiral_read_fleet_spine',{p_event_limit:30});if(generation!==readGeneration)return snapshot;snapshot=data;lastReadAt=Date.now();if(selectedProject&&!selectedVessel())selectedProject='';counts();vesselRows();eventRows();course();window.dispatchEvent(new CustomEvent('darksky:fleet-spine-snapshot',{detail:{snapshot:data,generation,at:lastReadAt}}));if(!preserveStatus)setStatus('CONTROL PLANE CURRENT · '+new Date(data.read_at).toLocaleString()+' · Reading is safe; issuing always requires a visible fresh preview.','current');return data;})().catch(err=>{if(generation===readGeneration){setStatus('CONTROL PLANE READ STOPPED · '+(err?.message||err)+' · No fleet state changed.','error');window.dispatchEvent(new CustomEvent('darksky:fleet-spine-read-failed',{detail:{message:String(err?.message||err),generation}}));}throw err;}).finally(()=>{if(readPromise===task)readPromise=null;});readPromise=task;return task;}
  function renderPreview(data,v){const b=data.before||{},a=data.after||{},box=$('fleetSpinePreviewBox');box.className=`fleet-spine-preview ${data.changes?'is-ready':'is-no-change'}`;box.innerHTML=`<div class="fleet-spine-preview-head"><small>${data.changes?'PREVIEW READY · NOT ISSUED':'NO CHANGE NEEDED'}</small><strong>${esc(v.display_name)}</strong><span>${data.changes?'Fleet Core verified this exact before-and-after course.':'Requested course already matches current state.'}</span></div><div class="fleet-spine-course-compare"><article><small>BEFORE</small><b>${esc(String(b.release_ring||'unassigned').toUpperCase())}</b><span>${esc(b.current_release||'NO RELEASE')}</span><em>${esc(String(b.rollout_state||'hold').replaceAll('_',' ').toUpperCase())}</em></article><div class="fleet-spine-course-arrow" aria-hidden="true">→</div><article><small>AFTER</small><b>${esc(String(a.release_ring||'unassigned').toUpperCase())}</b><span>${esc(a.current_release||'NO RELEASE')}</span><em>${esc(String(a.rollout_state||'hold').replaceAll('_',' ').toUpperCase())}</em></article></div><div class="fleet-spine-preview-proof"><b>Nothing has changed yet.</b><span>Issuing this course requires this exact fingerprint and the recorded Admiral intent.</span><details><summary>PREVIEW FINGERPRINT</summary><code>${esc(data.fingerprint||'—')}</code></details></div>`;}
  async function previewCourse(){const v=selectedVessel();if(!v)throw new Error('Select one vessel before previewing a course.');const intent=$('fleetSpineIntent')?.value.trim();if(!intent)throw new Error('Record the Admiral intent before previewing this course.');const button=$('fleetSpinePreview'),box=$('fleetSpinePreviewBox'),issue=$('fleetSpineIssue');preview=null;previewSignature='';issue.disabled=true;issue.setAttribute('aria-disabled','true');button.disabled=true;button.classList.add('is-busy');button.innerHTML='<span>VERIFYING COURSE…</span><small>READ-ONLY SERVER PREVIEW</small>';box.className='fleet-spine-preview is-working';box.innerHTML='<div class="fleet-spine-preview-state"><small>CHECKING FLEET CORE</small><strong>Verifying the exact before-and-after course…</strong><span>No fleet state can change during preview.</span></div>';setStatus('PREVIEW IN PROGRESS · Reading '+v.display_name+' / Fleet Core state only. Nothing is being issued.','working');try{const data=await rpc('admiral_preview_release_assignment',{p_project_id:v.project_id,p_release_ring:$('fleetSpineRing').value,p_rollout_state:$('fleetSpineRollout').value,p_current_release:$('fleetSpineRelease').value.trim()||null});preview=data;previewSignature=courseSignature();renderPreview(data,v);issue.disabled=!data.changes;issue.setAttribute('aria-disabled',String(!data.changes));setStatus(data.changes?'PREVIEW READY · Nothing changed. Review the before-and-after course below before issuing.':'PREVIEW COMPLETE · Requested course already matches current state.','success');requestAnimationFrame(()=>box.scrollIntoView({block:'nearest',behavior:'smooth'}));}catch(err){box.className='fleet-spine-preview is-error';box.innerHTML=`<div class="fleet-spine-preview-state"><small>PREVIEW FAILED</small><strong>${esc(err?.message||'Fleet Core could not return the preview.')}</strong><span>No fleet state changed.</span></div>`;setStatus('PREVIEW FAILED · '+(err?.message||'Fleet Core could not return the preview.')+' · Nothing changed.','error');throw err;}finally{button.disabled=false;button.classList.remove('is-busy');button.innerHTML='<span>PREVIEW COURSE</span><small>READ-ONLY · NOTHING CHANGES</small>';}}
  async function issueCourse(){const v=selectedVessel(),intent=$('fleetSpineIntent')?.value.trim();if(!preview||!v)throw new Error('Preview the release course first.');if(courseSignature()!==previewSignature){invalidatePreview('Course details changed after preview.');throw new Error('Course details changed. Preview again before issuing.');}if(!intent)throw new Error('Record the Admiral intent before issuing this course.');const button=$('fleetSpineIssue');button.disabled=true;button.setAttribute('aria-disabled','true');button.textContent='ISSUING VERIFIED COURSE…';setStatus('ISSUING VERIFIED COURSE · Fleet Core will accept only the visible preview fingerprint.','working');try{const data=await rpc('admiral_issue_release_assignment',{p_project_id:v.project_id,p_release_ring:$('fleetSpineRing').value,p_rollout_state:$('fleetSpineRollout').value,p_current_release:$('fleetSpineRelease').value.trim()||null,p_expected_fingerprint:preview.fingerprint,p_intent:intent});await load({preserveStatus:true});setStatus(data.changed?'COURSE VERIFIED · '+v.display_name+' updated. Activity Spine retained one durable command event.':'NO CHANGE · Fleet Core confirmed the requested course already matched.','success');}finally{button.textContent='ISSUE RELEASE COURSE';}}
  function open(){install();const w=$('fleetSpineWorkspace');w.classList.remove('hidden');w.setAttribute('aria-hidden','false');document.body.classList.add('fleet-spine-open');setStatus('OPENING CONTROL PLANE · Resolving one Admiral session and one Fleet Core snapshot.','working');requestAnimationFrame(()=>{w.scrollTo({top:0,behavior:'auto'});$('fleetSpineClose')?.focus({preventScroll:true});});syncIdentity().catch(e=>setStatus('CONTROL PLANE OPENED SAFELY · '+(e?.message||'Re-authentication required.')+' · No fleet state changed.','error'));}
  function close(){const w=$('fleetSpineWorkspace');w?.classList.add('hidden');w?.setAttribute('aria-hidden','true');document.body.classList.remove('fleet-spine-open');$('admiralFleetSpineOpen')?.focus({preventScroll:true});}
  function cancel(reason='cancelled'){readGeneration++;readPromise=null;setStatus('CONTROL PLANE PAUSED · '+String(reason).replaceAll('-',' ')+' · No fleet state changed.','warning');}
  window.DarkSkyFleetSpine={open,close,load,rpc,currentSession,cancel,getSnapshot:()=>snapshot,isOpen:()=>!!$('fleetSpineWorkspace')&&!$('fleetSpineWorkspace').classList.contains('hidden'),lastReadAt:()=>lastReadAt};
  document.addEventListener('visibilitychange',()=>{if(document.visibilityState!=='visible'||!window.DarkSkyFleetSpine?.isOpen?.())return;if(Date.now()-lastReadAt>60000)load().catch(err=>setStatus('RETURN READ STOPPED · '+(err?.message||err)+' · Re-authenticate if needed.','error'));});
  function installDoor(){const lane=$('admiralGovernLane');if(lane&&!$('admiralFleetSpineOpen')){const wrap=document.createElement('div');wrap.className='admiral-lane-summary fleet-spine-door';wrap.innerHTML='<b>Fleet Spine · Control Plane</b><span>Durable vessel identity, release rings, station posture, Foundry candidate count and append-only activity truth.</span><button id="admiralFleetSpineOpen" type="button">OPEN FLEET SPINE <em>READY</em></button>';lane.insertBefore(wrap,lane.querySelector('.admiral-finding-tabs'));}const rail=$('admiralCeremonialFoundry');if(rail&&!$('admiralCeremonialFleetSpine')){const b=document.createElement('button');b.id='admiralCeremonialFleetSpine';b.type='button';b.innerHTML='<b>Fleet Spine</b><small>Control Plane & release rings</small><em>READY</em>';rail.after(b);}}
  function showError(err,prefix=''){const message=err?.message||String(err||'Command failed.');setStatus((prefix?prefix+' · ':'')+message+' · Nothing changed.','error');}
  document.addEventListener('click',async e=>{const t=e.target;if(t.closest?.('#admiralFleetSpineOpen,#admiralCeremonialFleetSpine'))return open();if(t.closest?.('#fleetSpineClose'))return close();if(t.closest?.('#fleetSpineRefresh')){try{await load()}catch(err){showError(err,'REFRESH FAILED')}return;}if(t.closest?.('#fleetSpineSignIn')){const b=$('fleetSpineSignIn');try{b.disabled=true;b.textContent='AUTHENTICATING…';await window.DarkSkyAuthenticateAdmiral?.($('fleetSpineEmail').value,$('fleetSpinePassword').value);$('fleetSpinePassword').value='';await syncIdentity();}catch(err){showError(err,'ADMIRAL SIGN-IN FAILED')}finally{b.disabled=false;b.textContent='AUTHENTICATE ADMIRAL'}return;}if(t.closest?.('#fleetSpineSignOut')){window.DarkSkySignOutAdmiral?.();await syncIdentity();return;}const vessel=t.closest?.('[data-spine-vessel]');if(vessel){selectedProject=vessel.dataset.spineVessel;vesselRows();course();setStatus((selectedVessel()?.display_name||'Vessel')+' selected · Reading only. Set a proposed course and preview it before any issue action unlocks.','current');return;}if(t.closest?.('#fleetSpinePreview')){try{await previewCourse()}catch(_){ }return;}if(t.closest?.('#fleetSpineIssue')){try{await issueCourse()}catch(err){showError(err,'COURSE NOT ISSUED')}return;}},true);
  document.addEventListener('input',e=>{if(e.target.matches?.('#fleetSpineRelease,#fleetSpineIntent')&&preview)invalidatePreview('Course details changed. Preview again before issuing.');},true);
  document.addEventListener('change',e=>{if(e.target.matches?.('#fleetSpineRing,#fleetSpineRollout')&&preview)invalidatePreview('Course details changed. Preview again before issuing.');},true);
  const observer=new MutationObserver(()=>installDoor());observer.observe(document.documentElement,{childList:true,subtree:true});installDoor();window.DarkSkyOpenFleetSpine=open;
})();


/* 8.8.15.8 Station Registry — deliberate local-manifest adoption into durable Fleet Core station identity. */
;(()=>{
  'use strict';
  const SESSION_KEY='darkSkySupabaseAdmiralSessionV1';
  const $=id=>document.getElementById(id);
  const esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
  const spine=()=>window.DarkSkyFleetSpine||null;
  async function rpc(name,body={}){const broker=spine();if(!broker?.rpc)throw new Error('Fleet Spine session broker is not ready. Close and reopen Fleet Spine.');return broker.rpc(name,body);}
  const key=(projectId,stationKey)=>`${String(projectId||'')}::${String(stationKey||'')}`;
  const fmt=v=>v?new Date(v).toLocaleString():'Not established';
  let snapshot=null,selected='',preview=null,previewSig='';

  function install(){
    const shell=document.querySelector('#fleetSpineWorkspace .fleet-spine-shell');
    if(!shell||$('fleetSpineStationRegistry'))return false;
    const activity=shell.querySelector('.fleet-spine-activity');
    if(!activity)return false;
    activity.insertAdjacentHTML('beforebegin',`<section id="fleetSpineStationRegistry" class="fleet-spine-station-registry"><header><div><small>STATION IDENTITY · CONTROL PLANE</small><h3>Station Registry</h3><p class="fleet-spine-section-help">Durable WEB / KIOSK / HYBRID endpoints. Local outposts remain local until an Admiral deliberately previews and registers the exact identity.</p></div><span id="fleetSpineStationSummary">READING STATIONS…</span></header><div class="fleet-spine-station-grid"><div id="fleetSpineStationRows" class="fleet-spine-station-rows"><p>Reading station posture…</p></div><aside id="fleetSpineStationInspector" class="fleet-spine-station-inspector"><div class="fleet-spine-course-empty"><strong>Select one station</strong><span>Reading never registers or changes a station.</span></div></aside></div></section>`);
    return true;
  }
  function localCandidates(){try{const rows=window.DarkSkyFleetStationCandidates?.();return Array.isArray(rows)?rows:[]}catch(_){return []}}
  function serverStations(){return Array.isArray(snapshot?.stations)?snapshot.stations:[]}
  function merged(){
    const map=new Map();
    serverStations().forEach(s=>map.set(key(s.project_id,s.station_key),{id:key(s.project_id,s.station_key),server:s,local:null}));
    localCandidates().forEach(c=>{const k=key(c.projectId,c.stationKey),row=map.get(k)||{id:k,server:null,local:null};row.local=c;map.set(k,row);});
    return [...map.values()].sort((a,b)=>String(a.local?.vesselName||a.server?.vessel_name||'').localeCompare(String(b.local?.vesselName||b.server?.vessel_name||''))||String(a.local?.displayName||a.server?.display_name||'').localeCompare(String(b.local?.displayName||b.server?.display_name||'')));
  }
  function stationAligned(row){
    const l=row?.local,s=row?.server;if(!l||!s)return false;
    return String(s.display_name||'')===String(l.displayName||'')&&String(s.operating_mode||'')===String(l.operatingMode||'')&&String(s.station_state||'')===String(l.stationState||'')&&String(s.runtime_release||'')===String(l.runtimeRelease||'')&&String(s.policy_version||'')===String(l.policyVersion||'')&&String(s.station_profile||'')===String(l.stationProfile||'');
  }
  function statusFor(row){if(row.local&&!row.server)return ['LOCAL ONLY','local'];if(!row.local&&row.server)return ['FLEET CORE ONLY','server'];if(stationAligned(row))return ['REGISTERED · ALIGNED','aligned'];return ['SYNC NEEDED','drift'];}
  function renderRows(){
    const rows=merged(),host=$('fleetSpineStationRows'),summary=$('fleetSpineStationSummary');if(!host)return;
    const local=localCandidates().length,server=serverStations().length,awaiting=serverStations().filter(s=>String(s.heartbeat_status||'')==='awaiting').length;
    if(summary)summary.textContent=`${server} REGISTERED · ${local} LOCAL MANIFEST${local===1?'':'S'} · ${awaiting} AWAITING HEARTBEAT`;
    host.innerHTML=rows.length?rows.map(row=>{const l=row.local,s=row.server,[label,tone]=statusFor(row),sel=selected===row.id;return `<button type="button" class="fleet-spine-station-row ${sel?'is-selected':''}" data-spine-station="${esc(row.id)}" aria-pressed="${sel?'true':'false'}"><div><small>${esc(l?.vesselName||s?.vessel_name||s?.project_id||'VESSEL')}</small><b>${esc(l?.displayName||s?.display_name||'Customer station')}</b><span>${esc(l?.stationKey||s?.station_key||'')}</span></div><div class="fleet-spine-station-posture"><strong class="${esc(tone)}">${esc(label)}</strong><span>${esc(String(l?.operatingMode||s?.operating_mode||'web').toUpperCase())} · ${esc(String(l?.stationState||s?.station_state||'commissioning').replaceAll('_',' ').toUpperCase())}</span><em>${s?`HEARTBEAT ${esc(String(s.heartbeat_status||'awaiting').toUpperCase())}`:'NOT YET DURABLE'}</em></div></button>`;}).join(''):`<div class="fleet-spine-station-empty"><strong>No station identities yet.</strong><span>Fleet Core has no registered stations and this browser exposes no local deployment manifest candidates.</span></div>`;
  }
  function selectedRow(){return merged().find(r=>r.id===selected)||null}
  function values(row){const l=row?.local||{},intent=$('fleetSpineStationIntent')?.value.trim()||'';return {id:row?.id||'',project:l.projectId||row?.server?.project_id||'',station:l.stationKey||row?.server?.station_key||'',name:l.displayName||row?.server?.display_name||'',mode:l.operatingMode||row?.server?.operating_mode||'',state:l.stationState||row?.server?.station_state||'',release:l.runtimeRelease||row?.server?.runtime_release||'',policy:l.policyVersion||row?.server?.policy_version||'',profile:l.stationProfile||row?.server?.station_profile||'',intent};}
  function signature(row){return JSON.stringify(values(row))}
  function updateIntentCount(){const input=$('fleetSpineStationIntent'),node=$('fleetSpineStationIntentCount');if(input&&node)node.textContent=`${input.value.length} / 500 characters retained`;}
  function invalidate(message='Station details changed. Preview again before registration.'){preview=null;previewSig='';const box=$('fleetSpineStationPreviewBox'),issue=$('fleetSpineStationIssue');if(box){box.className='fleet-spine-preview is-stale';box.innerHTML=`<div class="fleet-spine-preview-state"><small>PREVIEW REQUIRED</small><strong>${esc(message)}</strong><span>No Fleet Core station record changed.</span></div>`;}if(issue){issue.disabled=true;issue.setAttribute('aria-disabled','true');}}
  function renderInspector(){
    const row=selectedRow(),host=$('fleetSpineStationInspector');preview=null;previewSig='';if(!host)return;
    if(!row){host.innerHTML='<div class="fleet-spine-course-empty"><strong>Select one station</strong><span>Reading never registers or changes a station.</span></div>';return;}
    const l=row.local,s=row.server,[label,tone]=statusFor(row);
    if(!l){host.innerHTML=`<div class="fleet-spine-station-current"><small>REGISTERED STATION</small><b>${esc(s.display_name)}</b><span>${esc(String(s.operating_mode).toUpperCase())} · ${esc(String(s.station_state).replaceAll('_',' ').toUpperCase())}</span></div><div class="fleet-spine-station-truth"><div><small>RUNTIME</small><strong>${esc(s.runtime_release||'NOT REPORTED')}</strong></div><div><small>HEARTBEAT</small><strong>${esc(String(s.heartbeat_status||'awaiting').toUpperCase())}</strong><span>${esc(fmt(s.last_heartbeat_at))}</span></div><div><small>RECOVERIES</small><strong>${Number(s.recovery_count||0)}</strong><span>${esc(fmt(s.last_recovery_at))}</span></div></div><div class="fleet-spine-station-boundary"><b>SERVER-ONLY ON THIS DEVICE</b><span>No local deployment manifest is visible here, so Fleet Spine offers no mutation.</span></div>`;return;}
    const aligned=stationAligned(row);
    host.innerHTML=`<div class="fleet-spine-station-current"><small>LOCAL DEPLOYMENT MANIFEST</small><b>${esc(l.displayName)}</b><span>${esc(String(l.operatingMode).toUpperCase())} · ${esc(String(l.stationState).replaceAll('_',' ').toUpperCase())} · ${esc(l.profileLabel||l.stationProfile||'DEPLOYMENT')}</span><em class="${esc(tone)}">${esc(label)}</em></div><div class="fleet-spine-station-truth"><div><small>LOCAL RUNTIME</small><strong>${esc(l.runtimeRelease||'—')}</strong><span>Manifest v${Number(l.manifestVersion||1)}</span></div><div><small>FLEET CORE</small><strong>${s?esc(String(s.station_state||'registered').replaceAll('_',' ').toUpperCase()):'NOT REGISTERED'}</strong><span>${s?`Heartbeat ${esc(String(s.heartbeat_status||'awaiting').toUpperCase())}`:'No durable station identity yet'}</span></div></div>${aligned?'<div class="fleet-spine-station-boundary is-aligned"><b>IDENTITY ALIGNED</b><span>The local manifest and durable Fleet Core identity agree. Heartbeat remains a separate runtime proof.</span></div>':`<label>Admiral registration intent<textarea id="fleetSpineStationIntent" maxlength="500" rows="3" placeholder="Why this exact local outpost should become or update a durable Fleet Core station"></textarea><small id="fleetSpineStationIntentCount" class="fleet-spine-intent-count">0 / 500 characters retained</small></label><button id="fleetSpineStationPreview" class="fleet-spine-preview-action" type="button"><span>PREVIEW STATION IDENTITY</span><small>READ-ONLY · NOTHING CHANGES</small></button><div id="fleetSpineStationPreviewBox" class="fleet-spine-preview is-awaiting" role="status" aria-live="polite"><div class="fleet-spine-preview-state"><small>AWAITING PREVIEW</small><strong>Exact station identity will appear here.</strong><span>Registration remains locked until Fleet Core returns a fresh fingerprint.</span></div></div><button id="fleetSpineStationIssue" class="fleet-spine-issue-action" type="button" disabled aria-disabled="true">REGISTER EXACT STATION</button>`}`;
    updateIntentCount();
  }
  function renderPreview(data,row){const b=data.before,a=data.after||{},box=$('fleetSpineStationPreviewBox');if(!box)return;box.className=`fleet-spine-preview ${data.changes?'is-ready':'is-no-change'}`;box.innerHTML=`<div class="fleet-spine-preview-head"><small>${data.changes?'STATION PREVIEW READY · NOT REGISTERED':'NO CHANGE NEEDED'}</small><strong>${esc(row.local?.displayName||row.server?.display_name||'Station')}</strong><span>${data.changes?'Fleet Core verified this exact station identity.':'Durable Fleet Core identity already matches this local manifest.'}</span></div><div class="fleet-spine-course-compare station-compare"><article><small>BEFORE</small><b>${b?esc(String(b.operating_mode||'registered').toUpperCase()):'NOT REGISTERED'}</b><span>${b?esc(String(b.station_state||'').replaceAll('_',' ').toUpperCase()):'NO DURABLE IDENTITY'}</span><em>${b?esc(b.runtime_release||'NO RUNTIME'):'LOCAL ONLY'}</em></article><div class="fleet-spine-course-arrow" aria-hidden="true">→</div><article><small>AFTER</small><b>${esc(String(a.operating_mode||'web').toUpperCase())}</b><span>${esc(String(a.station_state||'commissioning').replaceAll('_',' ').toUpperCase())}</span><em>${esc(a.runtime_release||'NO RUNTIME')}</em></article></div><div class="fleet-spine-preview-proof"><b>Nothing has changed yet.</b><span>Registration requires this exact fingerprint and the recorded Admiral intent. Registration does not fabricate a heartbeat.</span><details><summary>PREVIEW FINGERPRINT</summary><code>${esc(data.fingerprint||'—')}</code></details></div>`;}
  async function previewStation(){const row=selectedRow(),l=row?.local;if(!row||!l)throw new Error('Select a local station manifest first.');const intent=$('fleetSpineStationIntent')?.value.trim();if(!intent)throw new Error('Record the Admiral station intent before previewing.');const button=$('fleetSpineStationPreview'),issue=$('fleetSpineStationIssue'),box=$('fleetSpineStationPreviewBox');preview=null;previewSig='';issue.disabled=true;issue.setAttribute('aria-disabled','true');button.disabled=true;button.innerHTML='<span>VERIFYING STATION…</span><small>READ-ONLY SERVER PREVIEW</small>';box.className='fleet-spine-preview is-working';box.innerHTML='<div class="fleet-spine-preview-state"><small>CHECKING FLEET CORE</small><strong>Verifying exact station identity…</strong><span>No station record can change during preview.</span></div>';try{const data=await rpc('admiral_preview_station_registration',{p_project_id:l.projectId,p_station_key:l.stationKey,p_display_name:l.displayName,p_operating_mode:l.operatingMode,p_station_state:l.stationState,p_runtime_release:l.runtimeRelease||null,p_policy_version:l.policyVersion||null,p_station_profile:l.stationProfile||null});preview=data;previewSig=signature(row);renderPreview(data,row);issue.disabled=!data.changes;issue.setAttribute('aria-disabled',String(!data.changes));}finally{button.disabled=false;button.innerHTML='<span>PREVIEW STATION IDENTITY</span><small>READ-ONLY · NOTHING CHANGES</small>';}}
  async function issueStation(){const row=selectedRow(),l=row?.local,intent=$('fleetSpineStationIntent')?.value.trim();if(!row||!l||!preview)throw new Error('Preview the station identity first.');if(signature(row)!==previewSig){invalidate();throw new Error('Station details changed. Preview again before registration.');}if(!intent)throw new Error('Record the Admiral station intent before registering.');const button=$('fleetSpineStationIssue');button.disabled=true;button.textContent='REGISTERING VERIFIED STATION…';try{const data=await rpc('admiral_register_station',{p_project_id:l.projectId,p_station_key:l.stationKey,p_display_name:l.displayName,p_operating_mode:l.operatingMode,p_station_state:l.stationState,p_runtime_release:l.runtimeRelease||null,p_policy_version:l.policyVersion||null,p_station_profile:l.stationProfile||null,p_expected_fingerprint:preview.fingerprint,p_intent:intent});await spine()?.load?.({force:true,preserveStatus:true});const status=$('fleetSpineStatus');if(status){status.textContent='STATION IDENTITY VERIFIED · '+l.displayName+' is durable in Fleet Core. Heartbeat remains awaiting real runtime proof.';status.dataset.kind='success';}const host=$('fleetSpineStationInspector');if(host)host.insertAdjacentHTML('afterbegin',`<div class="fleet-spine-station-confirmation"><b>STATION IDENTITY VERIFIED</b><span>${esc(l.displayName)} is now durable in Fleet Core. Heartbeat remains awaiting real station runtime proof. Use REFRESH SPINE when you want the new Activity Spine event reflected above.</span></div>`);return data;}finally{button.textContent='REGISTER EXACT STATION';}}
  function consumeSnapshot(data){if(!$('fleetSpineStationRegistry'))install();snapshot=data&&typeof data==='object'?data:null;const summary=$('fleetSpineStationSummary'),rowsHost=$('fleetSpineStationRows');if(!snapshot){if(summary)summary.textContent='ADMIRAL AUTHENTICATION REQUIRED';if(rowsHost)rowsHost.innerHTML='<div class="fleet-spine-station-empty"><strong>Station Registry is closed.</strong><span>Authenticate the Admiral identity to read durable station posture.</span></div>';renderInspector();return;}if(selected&&!merged().some(r=>r.id===selected))selected='';renderRows();renderInspector();}
  function showReadFailure(message){snapshot=null;const summary=$('fleetSpineStationSummary'),rowsHost=$('fleetSpineStationRows');if(summary)summary.textContent='STATION READ FAILED';if(rowsHost)rowsHost.innerHTML=`<div class="fleet-spine-station-empty"><strong>Station posture could not be read.</strong><span>${esc(message||'Fleet Core read failed.')} No station identity changed.</span></div>`;renderInspector();}

  document.addEventListener('click',async e=>{
    const t=e.target;
    const station=t.closest?.('[data-spine-station]');if(station){selected=station.dataset.spineStation;renderRows();renderInspector();return;}
    if(t.closest?.('#fleetSpineStationPreview')){try{await previewStation()}catch(err){const box=$('fleetSpineStationPreviewBox');if(box){box.className='fleet-spine-preview is-error';box.innerHTML=`<div class="fleet-spine-preview-state"><small>PREVIEW FAILED</small><strong>${esc(err?.message||err)}</strong><span>No station record changed.</span></div>`;}}return;}
    if(t.closest?.('#fleetSpineStationIssue')){try{await issueStation()}catch(err){const box=$('fleetSpineStationPreviewBox');if(box){box.className='fleet-spine-preview is-error';box.innerHTML=`<div class="fleet-spine-preview-state"><small>REGISTRATION NOT ISSUED</small><strong>${esc(err?.message||err)}</strong><span>Fleet Core remains unchanged.</span></div>`;}}return;}
  },true);
  document.addEventListener('input',e=>{if(e.target.matches?.('#fleetSpineStationIntent')){updateIntentCount();if(preview)invalidate('Registration intent changed. Preview again.');}},true);
  window.addEventListener('darksky:fleet-spine-snapshot',e=>consumeSnapshot(e.detail?.snapshot||null));
  window.addEventListener('darksky:fleet-spine-read-failed',e=>showReadFailure(e.detail?.message||'Fleet Core read failed.'));
  const obs=new MutationObserver(()=>{if(install()){const existing=spine()?.getSnapshot?.();if(existing)consumeSnapshot(existing);}});
  obs.observe(document.documentElement,{childList:true,subtree:true});
  install();const existing=spine()?.getSnapshot?.();if(existing)consumeSnapshot(existing);
})();
