/* 8.8.15.3 Fleet Runtime — a full document load must never inherit
...
;(() => {
  ...
  window.DarkSkyAdmiralFreshGate=Object.freeze({build:'8.8.15.3',requiredPerDocument:true,engineAuthoritySeparate:true});
})();
/* Dark Sky 8.8.15.3 Fleet Runtime — source-aware read-only Admiral observability. */
;(()=>{
...
(() => {
  ...
  const UPPER_COMMAND_BUILD='8.8.15.3';
...
/* 8.8.15.3 Fleet Spine — durable Control Plane, Activity Spine and release rings. */
;(()=>{
  'use strict';
  const SESSION_KEY='darkSkySupabaseAdmiralSessionV1';
  ...
  function open(){install();const w=$('fleetSpineWorkspace');w.classList.remove('hidden');requestAnimationFrame(()=>w.scrollTo({top:0,behavior:'auto'}));syncIdentity().catch(e=>$('fleetSpineStatus').textContent=e?.message||'Fleet Spine could not open.');}
  function close(){$('fleetSpineWorkspace')?.classList.add('hidden');}
