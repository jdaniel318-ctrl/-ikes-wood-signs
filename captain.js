...
function bootCaptainCommand(){
  ...
  function renderHighWatch(intel=[]){
    ...
    return `<section class="captain-command-card high-watch-board easy-bearing-board">
      <div class="high-watch-head"><div><small>DARK SKY 4.3.6 • BRIGHTWORK</small><h3>Fleet Intelligence</h3><p>One prioritized operating picture across the admitted fleet. Read-only in Captain's Quarters.</p></div><strong class="${fleetClass}">${safe(fleetState)}</strong></div>
      ...
      <section class="cq-spyglass-search high-watch-search easy-bearing-search">
        <div><small>DARK SKY 4.3.6 • COMMAND FIND</small><h4>Find anything in the fleet</h4><p>Start typing a project, order number, customer, phone, email, or a phrase like “open Ike orders.” Results update automatically and never change production data.</p></div>
        ...
        <div id="captainCommandSearchResults" class="cq-spyglass-results easy-bearing-results"><span>Type two characters and Brightwork will find the best matches.</span></div>
      </section>
    </section>`;
  }
  ...
  function renderBroadsideCommand(projects=[]){
    ...
    return `<section class="captain-command-card v4-captain-broadside full-sail-captain">
      <div class="v4-captain-head"><div><small>DARK SKY 4.3.6 • BRIGHTWORK</small><h3>Captain's Command Brief</h3><p>Fleet posture, controlled experimentation, release discipline, and platform memory in one command surface.</p></div><strong class="${st.preflight.ok?'clear':'attention'}">${st.preflight.ok?'EASY BEARING':'REVIEW'}</strong></div>
  ...
  function bindBroadsideCommand(projects=[]){
    ...
    document.querySelectorAll('[data-v4-lab]').forEach(btn=>btn.onclick=()=>{try{window.DarkSkyV4.labMark(btn.dataset.v4Lab,btn.dataset.state,'Captain Brightwork review');audit('Captain Lab state changed',`${btn.dataset.v4Lab} → ${btn.dataset.state}`);renderBlackFlag();}catch(err){alert(err.message)}});
    ...
    const runSearch=async()=>{
      ...
      if(!searchHost)return;
      if(raw.length<2){searchHost.innerHTML='<span>Type two characters and Brightwork will find the best matches.</span>';return;}
