/* 8.8.15.4 Admiral Passage — a full document load must never inherit
...
/* Dark Sky 8.8.15.4 Recovery Helm — server-attested, read-only Admiral observability. */
;(()=>{
  ...
  function overview(id){const v=id===PROGRAM.project_id?PROGRAM:records.find(x=>x.project_id===id);if(!v)return;const f=facts(v);$('watchOverviewBody').innerHTML=`<header>${mark(v)}<div><small>ADMIRAL OBSERVATION · EXACT PROJECT CONTEXT</small><h2>${safe(v.display_name)}</h2><p>${safe(v.project_id)}</p></div><em>READ ONLY</em></header><section class="fleet-vessel-context"><article><small>LIFECYCLE</small><b>${safe(f.ready)}</b></article><article><small>CURRENT WORK</small><b>${safe(f.work)}</b></article><article><small>UNRESOLVED ISSUES</small><b>${f.issues===null?'Not reported':f.issues}</b></article><article><small>VESSEL REPORT</small><b>${safe(f.activity?new Date(f.activity).toLocaleString():'Not reported')}</b></article></section>${f.summary?`<aside><b>Issue summary</b><span>${safe(f.summary)}</span></aside>`:''}<aside><b>Scoped observation established</b><span>Only ${safe(v.display_name)} (${safe(v.project_id)}) is in context. Source: ${safe(words(f.source))}. No modifying control is exposed.</span></aside>`;$('watchOverview').classList.remove('hidden');$('watchOverview').setAttribute('aria-hidden','false');$('watchOverviewClose').focus({preventScroll:true});}
