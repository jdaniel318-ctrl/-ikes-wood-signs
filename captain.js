(() => {
  ...
  const UPPER_COMMAND_BUILD='8.6.60';
...
;(()=>{
  ...
  const actionButtons=()=>[document.getElementById('admiralMakeStandard'),document.getElementById('admiralGrantPaid')].filter(Boolean);
  ...
  async function syncIdentityUi(){const ok=await verifyAdmiral();if(state())state().textContent=ok?'ADMIRAL IDENTITY VERIFIED':'ADMIRAL IDENTITY REQUIRED';actionButtons().forEach(b=>b.disabled=!ok);document.getElementById('admiralIdentitySignOut')?.classList.toggle('hidden',!readSession());if(ok&&result())result().textContent='Admiral identity verified. Select a vessel and service, then choose Standard or Paid.';return ok;}
  ...
  document.addEventListener('click',async e=>{
    ...
    if(e.target.closest('#admiralMakeStandard')){try{if(result())result().textContent='Applying Standard entitlement…';await setEntitlement('standard');}catch(err){if(result())result().textContent=String(err?.message||err);}return;}
