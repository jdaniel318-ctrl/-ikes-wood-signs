/* 8.8.15.6 Fleet Runtime — a full document load must never inherit
...
;(() => {
  ...
  window.DarkSkyAdmiralFreshGate=Object.freeze({build:'8.8.15.6',requiredPerDocument:true,engineAuthoritySeparate:true});
})();
/* Dark Sky 8.8.15.6 Fleet Runtime — source-aware read-only Admiral observability. */
;(()=>{
...
(() => {
  ...
  const UPPER_COMMAND_BUILD='8.8.15.6';
...
/* 8.8.15.6 Station Registry — readable Control Plane, durable course preview and human Activity Spine. */
;(()=>{
  'use strict';
  const SESSION_KEY='darkSkySupabaseAdmiralSessionV1';
...
/* 8.8.15.6 Station Registry — deliberate local-manifest adoption into durable Fleet Core station identity. */
;(()=>{
  'use strict';
  const SESSION_KEY='darkSkySupabaseAdmiralSessionV1';
  ...
  async function load(){if(!install())return;const s=await currentSession();if(!s?.access_token){snapshot=null;renderRows();renderInspector();const summary=$('fleetSpineStationSummary');if(summary)summary.textContent='ADMIRAL AUTHENTICATION REQUIRED';return;}snapshot=await rpc('admiral_read_fleet_spine',{p_event_limit:30});if(selected&&!merged().some(r=>r.id===selected))selected='';renderRows();renderInspector();}
