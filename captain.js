/* 8.8.15.5 Fleet Runtime — a full document load must never inherit
...
;(() => {
  ...
  window.DarkSkyAdmiralFreshGate=Object.freeze({build:'8.8.15.5',requiredPerDocument:true,engineAuthoritySeparate:true});
})();
/* Dark Sky 8.8.15.5 Fleet Runtime — source-aware read-only Admiral observability. */
;(()=>{
...
(() => {
  ...
  const UPPER_COMMAND_BUILD='8.8.15.5';
...
/* 8.8.15.5 Admiral Lens — readable Control Plane, durable course preview and human Activity Spine. */
;(()=>{
  'use strict';
  const SESSION_KEY='darkSkySupabaseAdmiralSessionV1';
  ...
  function humanEventTitle(type=''){const map={'fleet.spine.foundation.commissioned':'Fleet Spine commissioned','release.assignment.issued':'Release course issued','release.assignment.changed':'Release assignment changed','foundry.candidate.saved':'Foundry candidate saved','foundry.candidate.archived':'Foundry candidate archived'};return map[type]||String(type||'Fleet event').split('.').map(x=>x.charAt(0).toUpperCase()+x.slice(1)).join(' · ');}
  function eventSummary(e){const p=e?.payload||{},type=String(e?.event_type||'');if(type==='fleet.spine.foundation.commissioned')return `Control Plane foundation recorded for build ${p.build||'—'}${p.release?` · ${p.release}`:''}.`;if(type==='release.assignment.issued'||type==='release.assignment.changed'){const before=p.before||{},after=p.after||{};return `${p.project_id||'Vessel'} · ${String(before.release_ring||'unassigned').toUpperCase()} → ${String(after.release_ring||p.release_ring||'unassigned').toUpperCase()}${after.current_release||p.current_release?` · ${after.current_release||p.current_release}`:''}.`; }if(type==='foundry.candidate.saved')return `${p.status||'Candidate'} · ${p.product_route||'Foundry route'} · clearance record retained.`;if(type==='foundry.candidate.archived')return `Foundry candidate archived${p.reason?` · ${p.reason}`:''}.`;return `Fleet event retained from ${e?.source||'Control Plane'}.`;}
