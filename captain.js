(() => {
  ...
  const UPPER_COMMAND_BUILD='8.8.6';
...
/* 8.8.6 Commissioning Orders — permanent keel, editable working name. */
;(()=>{
  const SESSION_KEY='darkSkySupabaseAdmiralSessionV1';
  let preview=null,confirmed=false,busy=false,fullLog=false,commissionedProject='';
  const el=id=>document.getElementById(id);
  ...
  function setBusy(next){busy=next;const verified=station()?.classList.contains('identity-verified');if(el('admiralCommissioningPreview'))el('admiralCommissioningPreview').disabled=next||!verified;if(el('admiralCommissioningIssue'))el('admiralCommissioningIssue').disabled=next||!verified||!preview||!confirmed;if(el('admiralCommissioningLog'))el('admiralCommissioningLog').disabled=next||!verified;if(el('admiralCommissioningRename'))el('admiralCommissioningRename').disabled=next||!verified;}
