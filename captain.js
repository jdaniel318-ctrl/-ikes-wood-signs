...
;(()=>{
  ...
  function vendors(){return [...new Set(state.tasks.map(task=>task.vendor||vendorFor(task.trade)))].sort((a,b)=>a.localeCompare(b));}
  function segmentsForWeek(week){const first=week*5,last=first+5,segments=[];state.tasks.forEach(task=>{const start=Math.max(task.start,first),end=Math.min(task.start+Math.max(1,task.duration||1),last);if(start<end)segments.push({task,start,end});});segments.sort((a,b)=>a.start-b.start||b.end-a.end||a.task.title.localeCompare(b.task.title));const lanes=[];segments.forEach(segment=>{let lane=lanes.findIndex(end=>end<=segment.start);if(lane<0){lane=lanes.length;lanes.push(segment.end);}else lanes[lane]=segment.end;segment.lane=lane;});return {segments,lanes:Math.max(2,lanes.length)};}
  function renderSchedule(){if(!state)return;const meta=project(activeId);[['bootstrapRegionValue','region'],['bootstrapDivisionValue','division'],['bootstrapCommunityValue','community'],['bootstrapLotValue','lot'],['bootstrapUnitValue','unit']].forEach(([id,key])=>{if(el(id))el(id).textContent=meta[key];});if(el('bootstrapScheduleRange'))el('bootstrapScheduleRange').textContent=labelDate(0)+' – '+workDate(DAYS-1).toLocaleDateString(undefined,{month:'short',day:'numeric',year:'numeric'})+' · 12 WEEKS';const select=el('bootstrapScheduleDay');if(select)select.innerHTML=Array.from({length:DAYS},(_,day)=>'<option value="'+day+'">'+esc(labelDate(day,true))+'</option>').join('');const board=el('bootstrapScheduleBoard');if(!board)return;board.innerHTML=Array.from({length:WEEKS},(_,week)=>{const first=week*5,{segments,lanes}=segmentsForWeek(week);const heads=Array.from({length:5},(_,col)=>'<button type="button" class="bootstrap-ground-day-head'+(overDay===first+col?' is-over':'')+'" data-schedule-move="'+(first+col)+'" title="Move selected task here"><small>'+esc(workDate(first+col).toLocaleDateString(undefined,{weekday:'short'}))+'</small><b>'+esc(labelDate(first+col))+'</b></button>').join('');const cells=Array.from({length:5},(_,col)=>'<div class="bootstrap-ground-day-cell'+(overDay===first+col?' is-over':'')+'" data-schedule-day="'+(first+col)+'" style="grid-column:'+(col+1)+';grid-row:2 / span '+lanes+'"></div>').join('');const bars=segments.map(({task,start,end,lane})=>'<article class="bootstrap-ground-task kind-'+esc(task.type||'work')+(selectedId===task.id?' is-selected':'')+'" draggable="true" tabindex="0" data-task-id="'+esc(task.id)+'" style="grid-column:'+(start-first+1)+' / span '+(end-start)+';grid-row:'+(lane+2)+'"><small>'+esc(task.trade)+'</small><b>'+(start>task.start?'← ':'')+esc(task.title)+(end<task.start+task.duration?' →':'')+'</b><span>'+esc(task.duration)+' workday'+(task.duration===1?'':'s')+'</span><em class="bootstrap-task-vendor">'+esc(task.vendor)+'</em></article>').join('');return '<section class="bootstrap-ground-week"><header><small>WEEK '+(week+1)+'</small><b>'+esc(labelDate(first))+' – '+esc(labelDate(first+4))+'</b></header><div class="bootstrap-ground-week-grid" data-week="'+week+'" style="--task-lanes:'+lanes+'">'+heads+cells+bars+'</div></section>';}).join('');renderVendorLayer();}
  function renderVendorLayer(){if(!state)return;const options=vendors(),view=el('bootstrapVendorView'),assign=el('bootstrapVendorAssign');if(view){view.innerHTML='<option value="">All work</option>'+options.map(v=>'<option value="'+esc(v)+'">'+esc(v)+'</option>').join('');view.value=vendorView;}if(assign){assign.innerHTML=options.map(v=>'<option value="'+esc(v)+'">'+esc(v)+'</option>').join('');const chosen=state.tasks.find(task=>task.id===selectedId);if(chosen)assign.value=chosen.vendor;}if(el('bootstrapVendorAssignButton'))el('bootstrapVendorAssignButton').disabled=!selectedId;if(el('bootstrapVendorViewNote'))el('bootstrapVendorViewNote').textContent=vendorView?vendorView+' sees only its assigned work and schedule changes.':'All project work is visible.';document.querySelectorAll('#bootstrapSchedule [data-task-id]').forEach(card=>{const task=state.tasks.find(item=>item.id===card.dataset.taskId);card.classList.toggle('vendor-hidden',!!vendorView&&task?.vendor!==vendorView);});const activity=el('bootstrapVendorActivity');if(activity)activity.innerHTML=state.activity.length?state.activity.slice(0,12).map(item=>'<article><b>'+esc(item.vendor)+'</b><span>'+esc(item.task)+(item.assignment?' reassigned from '+esc(item.from)+' to '+esc(item.to):' moved from '+esc(item.from)+' to '+esc(item.to))+'.</span><small>'+esc(new Date(item.at).toLocaleString())+'</small></article>').join(''):'<p>No schedule changes are recorded for this project.</p>';}
  async function openProject(id){activeId=id;state=cache.get(id)||await loadProject(project(id));selectedId='';vendorView='';el('bootstrapProjectLanding')?.classList.add('hidden');el('bootstrapProjectWorkspace')?.classList.remove('hidden');el('bootstrapProjectsHome')?.classList.remove('hidden');const meta=project(id);el('bootstrapScheduleTitleHeading').textContent='Lot '+meta.lot+' · Unit '+meta.unit;el('bootstrapScheduleSubtitle').textContent='Independent construction schedule and vendor assignments.';renderSchedule();status(state._source==='recovered prior schedule'?'Prior Lot 1234 · Unit A schedule recovered. Changes now save in its independent record.':state.updatedAt?'Loaded saved revision '+state.revision+' from '+new Date(state.updatedAt).toLocaleString()+'.':'Original project plan loaded. Move a task to begin this unit’s change record.');el('bootstrapSchedule').scrollTop=0;}
  async function move(id,day){const task=state?.tasks.find(item=>item.id===id);if(!task)return;const from=task.start,next=Math.max(0,Math.min(DAYS-Math.max(1,task.duration||1),Number(day)));task.start=next;if(from!==next)state.activity.unshift({id:'move-'+Date.now(),at:new Date().toISOString(),task:task.title,vendor:task.vendor,from:labelDate(from,true),to:labelDate(next,true)});state.activity=state.activity.slice(0,100);selectedId='';showOver(null);renderSchedule();document.querySelectorAll('#bootstrapSchedule [data-task-id="'+CSS.escape(task.id)+'"]').forEach(node=>node.classList.add('is-settling'));if(from===next){status(task.title+' already begins on '+labelDate(next,true)+'.');return;}status(task.title+' moved to '+labelDate(next,true)+'. Saving this project…');await persist(task.duration+'-workday duration preserved; '+task.vendor+' can see the change.');}
  function gridRects(){return [...document.querySelectorAll('.bootstrap-ground-week-grid')].map(grid=>({week:Number(grid.dataset.week),rect:grid.getBoundingClientRect()}));}
  ...
  document.addEventListener('dragstart',event=>{const card=event.target.closest?.('#bootstrapSchedule [data-task-id]');if(!card||!state)return;event.stopImmediatePropagation();selectedId=card.dataset.taskId;event.dataTransfer.effectAllowed='move';event.dataTransfer.setData('text/plain',selectedId);card.classList.add('is-dragging');},{capture:true});
  ...
  document.addEventListener('pointerdown',event=>{const card=event.target.closest?.('#bootstrapSchedule [data-task-id]');if(!card||event.pointerType==='mouse'||!state)return;event.preventDefault();event.stopImmediatePropagation();selectedId=card.dataset.taskId;touchDrag={id:selectedId,pointerId:event.pointerId,rects:gridRects(),ghost:makeDragGhost(card,event)};card.setPointerCapture?.(event.pointerId);card.classList.add('is-dragging');moveDragGhost(touchDrag,event);},{capture:true});
  document.addEventListener('pointermove',event=>{if(!touchDrag||event.pointerId!==touchDrag.pointerId)return;event.preventDefault();event.stopImmediatePropagation();moveDragGhost(touchDrag,event);showOver(dayAtPoint(event.clientX,event.clientY,touchDrag.rects));},{capture:true,passive:false});
  document.addEventListener('pointerup',event=>{if(!touchDrag||event.pointerId!==touchDrag.pointerId)return;event.preventDefault();event.stopImmediatePropagation();const drag=touchDrag,id=drag.id,day=dayAtPoint(event.clientX,event.clientY,drag.rects);clearTouchDrag();suppressClickUntil=Date.now()+350;if(day!==null)move(id,day);else{showOver(null);renderSchedule();status('Task selected. Tap a workday heading to move it.');}},{capture:true});
