/* 8.8.14.0 Fleet Ready — a full document load must never inherit
...
;(() => {
  ...
  window.DarkSkyAdmiralFreshGate=Object.freeze({build:'8.8.14.0',requiredPerDocument:true,engineAuthoritySeparate:true});
})();
/* Dark Sky 8.8.14.0 Fleet Ready — read-only Admiral observability. */
;(()=>{
...
(() => {
  ...
  const UPPER_COMMAND_BUILD='8.8.14.0';
  ...
  function openSignalReport(projectId){
    ...
    <div class="cq-signal-vessel-summary">
      <span>${htmlSafe(vessel.launchLabel||'VESSEL')}</span>
    ...
    <div class="cq-fleet-ready-course"><small>FLEET READY COURSE</small><strong>STEP ${Number(vessel.launchStep)||1} OF 5 · ${htmlSafe(vessel.launchLabel||'REVIEW')}</strong><p>${htmlSafe(vessel.launchDetail||'Review this vessel’s current operating posture.')}</p></div>
    <div class="cq-signal-outposts">
      ${outposts.length?outposts.map(o=>{
        ...
        return `<article class="cq-signal-outpost ${o.state}">
          <i></i>
          <div><strong>${htmlSafe(o.name)}</strong><small>${stateLabel(o.state)} • Manifest v${Number(o.manifestVersion)||1}</small><span class="cq-outpost-evidence"><em class="${o.customerTestRecorded?'pass':'open'}">${o.customerTestRecorded?'TEST RECORDED':'TEST PENDING'}</em><em class="${o.customerExperienceApproved?'pass':'open'}">${o.customerExperienceApproved?'EXPERIENCE APPROVED':'EXPERIENCE REVIEW'}</em><em class="${o.deviceLockVerified?'pass':'before'}">${o.deviceLockVerified?'DEVICE VERIFIED':'BEFORE LIVE'}</em></span>${reasons.length?`<p>${reasons.map(htmlSafe).join(' • ')}</p>`:''}<p class="cq-outpost-next"><b>NEXT:</b> ${htmlSafe(o.nextStep||'Review this outpost in Project Control.')}</p></div>
          <button type="button" data-captain-route-project="${htmlSafe(vessel.projectId)}" data-captain-route-outpost="${htmlSafe(o.id)}">${htmlSafe(o.routeLabel||'OPEN OUTPOST')} <small>Exact route in Engine</small></button>
        </article>`;
  ...
  function refreshChartroomLive(){
    ...
    if(act) act.innerHTML=signalRows.length?signalRows.slice(0,1).map(({v})=>{const priority=(v.outposts||[]).find(o=>o.state==='sea_trial'||o.state==='paused'||(o.attentionReasons||[]).length)||(v.outposts||[])[0];return `<article><b>${htmlSafe(v.name)}</b><p>${htmlSafe(priority?.nextStep||v.launchDetail||'Open the scoped signal report and review the exact route.')}</p><button type="button" data-captain-pro-vessel="${htmlSafe(v.projectId)}">OPEN EXACT COURSE</button></article>`;}).join(''):`<article><b>Fleet steady</b><p>No attention route is required right now.</p><button type="button" data-captain-pro-route="fleet">OPEN FLEET MAP</button></article>`;
