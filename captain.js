(() => {
  ...
  const UPPER_COMMAND_BUILD='8.8.9';
...
/* 8.8.9 Fleet Door Repair — canonical vessel identity, Admiral-controlled. */
;(()=>{
  ...
  function installFleetDoor(){if(el('bootstrapFleetOpenPromote'))return;const commissioning=el('admiralCommissioningOpen');if(!commissioning)return;commissioning.insertAdjacentHTML('afterend','<div class="admiral-lane-summary bootstrap-fleet-door-summary"><b>Admiral Fleet</b><span>See every Fleet Core vessel, lifecycle state, permanent key, and current mark in one read-only view.</span></div><button id="bootstrapFleetOpenPromote" class="admiral-pro-button bootstrap-fleet-primary-door" type="button">OPEN ADMIRAL FLEET</button>');}
  function installCommissionBrand(){if(el('admiralCommissioningBrandKit'))return;const anchor=el('admiralCommissioningSummary')?.closest('label');if(!anchor)return;const kit=document.createElement('section');kit.id='admiralCommissioningBrandKit';kit.className='admiral-commissioning-brand-kit';kit.innerHTML='<div><small>APPROVED VESSEL IDENTITY</small><strong>Bootstrap Build founding mark</strong><span>Primary badge · horizontal lockup · compact fleet icon</span></div><figure><img src="bootstrap_build_logo.png" alt="Bootstrap Build primary vessel badge"><img src="bootstrap_build_horizontal.png" alt="Bootstrap Build horizontal lockup"><img src="bootstrap_build_icon.png" alt="Bootstrap Build compact fleet icon"></figure><button id="bootstrapFleetOpen" class="admiral-pro-button is-primary" type="button">OPEN ADMIRAL FLEET</button>';anchor.after(kit);}
...
/* 8.8.9 Bootstrap Build — local-first construction schedule proving ground. */
;(()=>{
  ...
  function install(){const kit=el('admiralCommissioningBrandKit');if(kit&&!el('bootstrapScheduleOpen'))kit.insertAdjacentHTML('beforeend','<button id="bootstrapScheduleOpen" class="admiral-pro-button" type="button">OPEN TEST CONSTRUCTION SCHEDULE</button>');if(!el('bootstrapFleet'))document.body.insertAdjacentHTML('beforeend',`<section id="bootstrapFleet" class="bootstrap-fleet hidden" aria-label="Admiral Fleet"><div class="bootstrap-fleet-shell"><header><img src="bootstrap_build_horizontal.png" alt="Bootstrap Build"><div><small>ADMIRAL FLEET · READ ONLY</small><h2>Your Fleet</h2><p>Every Fleet Core vessel in one unmistakable place.</p></div><div><button id="bootstrapFleetRefresh" type="button">REFRESH FLEET</button><button id="bootstrapFleetClose" type="button">CLOSE</button></div></header><p id="bootstrapFleetStatus" class="bootstrap-fleet-status" role="status">Reading the Admiral Fleet…</p><div id="bootstrapFleetRows" class="bootstrap-fleet-rows"></div><footer><b>Read-only fleet boundary</b><span>This view cannot rename, commission, publish, entitle, or otherwise change a vessel.</span></footer></div></section>`);if(el('bootstrapSchedule'))return;document.body.insertAdjacentHTML('beforeend',`<section id="bootstrapSchedule" class="bootstrap-schedule hidden" aria-label="Bootstrap Build construction schedule prototype"><div class="bootstrap-schedule-shell"><header class="bootstrap-schedule-head"><img src="bootstrap_build_horizontal.png" alt="Bootstrap Build"><div><small>TEST WORKSPACE · NO FLEET CORE WRITES</small><h2>Construction Schedule</h2><p>Drag familiar work between days for the test unit.</p></div><div class="bootstrap-schedule-head-actions"><button id="bootstrapFleetOpenFromSchedule" type="button">ADMIRAL FLEET</button><button id="bootstrapScheduleClose" type="button">CLOSE</button></div></header><nav class="bootstrap-schedule-crumbs" aria-label="Test project hierarchy"><span><small>REGION</small><b>ABC</b></span><i>›</i><span><small>DIVISION</small><b>CBA</b></span><i>›</i><span><small>COMMUNITY</small><b>AB</b></span><i>›</i><span><small>LOT</small><b>1234</b></span><i>›</i><span><small>UNIT</small><b>A</b></span></nav><div class="bootstrap-schedule-toolbar"><div><button id="bootstrapSchedulePrev" type="button">← PRIOR WEEK</button><strong id="bootstrapScheduleWeek"></strong><button id="bootstrapScheduleNext" type="button">NEXT WEEK →</button></div><div><button id="bootstrapScheduleAdd" type="button">+ ADD TASK</button><button id="bootstrapScheduleReset" type="button">RESET TEST DATA</button></div></div><form id="bootstrapScheduleForm" class="bootstrap-schedule-form hidden"><label>Task<input id="bootstrapScheduleTitle" maxlength="80" required placeholder="Construction task"></label><label>Trade<input id="bootstrapScheduleTrade" maxlength="40" required placeholder="Trade or crew"></label><label>Day<select id="bootstrapScheduleDay"></select></label><button type="submit">ADD TO SCHEDULE</button><button id="bootstrapScheduleCancel" type="button">CANCEL</button></form><p id="bootstrapScheduleStatus" class="bootstrap-schedule-status" role="status">Test schedule ready. Drag a task card to another day.</p><div id="bootstrapScheduleBoard" class="bootstrap-schedule-board"></div><footer><b>Test boundary</b><span>This calendar is browser-local test data for Region ABC. It does not create a Fleet Core vessel, customer commitment, owner assignment, or live construction record.</span></footer></div></section>`);}
  ...
  function renderFleet(records=[]){const rows=el('bootstrapFleetRows'),statusNode=el('bootstrapFleetStatus');if(!rows)return;if(!records.length){rows.innerHTML='<article class="bootstrap-fleet-empty"><img src="bootstrap_build_icon.png" alt="Bootstrap Build fleet mark"><div><b>Fleet Core is awaiting an authenticated read</b><span>Authenticate the Admiral identity, then tap REFRESH FLEET. The prepared Fleet Core upgrade must be installed before live vessels can appear.</span></div></article>';if(statusNode)statusNode.textContent='No live Fleet Core vessels are available in this read-only view yet.';return;}rows.innerHTML=records.map(vessel=>'<article class="bootstrap-fleet-card'+(vessel.project_id==='construction-scheduling-01'?' is-bootstrap':'')+'"><img src="'+esc(vessel.logo_url||'black_flag_primary_lockup.png')+'" alt="'+esc(vessel.display_name)+' vessel mark"><div><small>'+(vessel.project_id==='construction-scheduling-01'?'BOOTSTRAP BUILD · CURRENT VESSEL':'FLEET CORE VESSEL')+'</small><b>'+esc(vessel.display_name)+'</b><span>'+esc(vessel.project_id)+'</span></div><em>'+esc(String(vessel.lifecycle_state||'fleet').replaceAll('_',' ').toUpperCase())+'</em></article>').join('');if(statusNode)statusNode.textContent=records.length+' Fleet Core vessel'+(records.length===1?'':'s')+' visible · read only.';}
  ...
  function openFleet(){install();el('bootstrapFleet')?.classList.remove('hidden');loadFleet();}
...
/* 8.8.9 Fleet Door Repair — permanent keel, editable working name. */
;(()=>{
  const SESSION_KEY='darkSkySupabaseAdmiralSessionV1';
