/* 8.8.13.5 Proof Lantern — a full document load must never inherit
...
;(() => {
  ...
  window.DarkSkyAdmiralFreshGate=Object.freeze({build:'8.8.13.5',requiredPerDocument:true,engineAuthoritySeparate:true});
})();
/* Dark Sky 8.8.13.5 Proof Lantern — read-only Admiral observability. */
;(()=>{
...
(() => {
  ...
  const UPPER_COMMAND_BUILD='8.8.13.5';
  ...
  function openAdmiralGate(source='captain',directTarget=''){
    ...
    if(title)title.textContent=directTarget==='fleet'?'Secure Fleet Access':"Admiral's Gate";
    if(copy)copy.textContent=directTarget==='fleet'?'Verify the Admiral PIN once, then go directly to the read-only Fleet Registry. Account identity remains separately protected.':"The Admiral's Deck governs Dark Sky, Black Flag and the fleet. This trial entrance exists so the Captain can prove the machinery before the rank is earned.";
    if(unlock)unlock.textContent=directTarget==='fleet'?'VERIFY & OPEN MY FLEET →':"ENTER ADMIRAL'S DECK →";
    if(deckReturn){
