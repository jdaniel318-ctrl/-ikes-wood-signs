...
function bootCaptainCommand(){
  ...
  function renderBroadsideCommand(projects=[]){
    ...
    return `<section class="captain-command-card v4-captain-broadside full-sail-captain">
      <div class="v4-captain-head"><div><small>DARK SKY 4.1.1 • CLEAR HORIZON</small><h3>Captain's Command Brief</h3><p>Fleet posture, controlled experimentation, release discipline, and platform memory in one command surface.</p></div><strong class="${st.preflight.ok?'clear':'attention'}">${st.preflight.ok?'CLEAR HORIZON':'REVIEW'}</strong></div>
      <div class="v4-captain-metrics"><span><b>${fleet.length}</b> admitted vessels</span><span><b>${st.recoveryPoints}</b> recovery points</span><span><b>${bb.recentFaults||0}</b> recent Black Box faults</span><span><b>${st.decisions}</b> decisions</span></div>
