(() => {
  ...
  const UPPER_COMMAND_BUILD='8.7.0';
...
;(()=>{
  ...
  async function syncIdentityUi(){const ok=await verifyAdmiral();if(state())state().textContent=ok?'ADMIRAL IDENTITY VERIFIED':'ADMIRAL IDENTITY REQUIRED';actionButtons().forEach(b=>b.disabled=!ok);document.getElementById('admiralIdentitySignOut')?.classList.toggle('hidden',!readSession());if(ok&&result())result().textContent='Admiral identity verified. Select a vessel and feature, then choose Off, Free or Paid.';return ok;}
  async function signIn(){const c=cfg(),email=document.getElementById('admiralIdentityEmail')?.value?.trim().toLowerCase(),password=document.getElementById('admiralIdentityPassword')?.value||'';if(!c?.url||!c?.publishableKey)throw new Error('Supabase identity is not configured.');if(!email||!password)throw new Error('Enter the Admiral email and password.');const r=await fetch(c.url+'/auth/v1/token?grant_type=password',{method:'POST',headers:headers(),body:JSON.stringify({email,password})});if(!r.ok)throw new Error('Admiral sign-in failed.');saveSession(await r.json());if(!(await verifyAdmiral())){clearSession();throw new Error('This account is authenticated but does not hold active Admiral authority.');}await syncIdentityUi();}
  async function setEntitlement(mode){const c=cfg(),s=await currentSession();if(!c||!s?.access_token)throw new Error('Authenticate the Admiral identity first.');const projectId=document.getElementById('admiralEntitlementVessel')?.value||'',capabilityKey=document.getElementById('admiralEntitlementCapability')?.value||'';const r=await fetch(c.url+'/rest/v1/rpc/admiral_set_service_entitlement',{method:'POST',headers:{...headers(s.access_token),Accept:'application/json'},body:JSON.stringify({p_project_id:projectId,p_capability_key:capabilityKey,p_commercial_mode:mode,p_status:'entitled'})});if(!r.ok){let text='Admiral entitlement change was refused.';try{const e=await r.json();text=e?.message||e?.hint||text}catch(_){}throw new Error(text);}const data=await r.json();const label=mode==='off'?'OFF':mode==='free'?'FREE • $0':'PAID';if(result())result().textContent=label+' • '+projectId+' • '+capabilityKey;const n=document.getElementById('admiralDeckNotice');if(n)n.textContent='Feature entitlement updated for the selected vessel only.';return data;}
  document.addEventListener('click',async e=>{
    const open=e.target.closest('#admiralServiceEntitlements');if(open){document.getElementById('admiralEntitlementStation')?.classList.toggle('hidden');await syncIdentityUi();return;}
    if(e.target.closest('#admiralIdentitySignIn')){try{if(result())result().textContent='Authenticating Admiral identity…';await signIn();}catch(err){if(result())result().textContent=String(err?.message||err);}return;}
    if(e.target.closest('#admiralIdentityRecover')){location.href='./index.html?surface=admiral-recovery-request';return;}
    if(e.target.closest('#admiralIdentitySignOut')){clearSession();await syncIdentityUi();if(result())result().textContent='Admiral identity signed out.';return;}
