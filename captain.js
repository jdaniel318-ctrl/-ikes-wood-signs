/* 8.8.15.2 Fleet Runtime — a full document load must never inherit
...
;(() => {
  ...
  window.DarkSkyAdmiralFreshGate=Object.freeze({build:'8.8.15.2',requiredPerDocument:true,engineAuthoritySeparate:true});
})();
/* Dark Sky 8.8.15.2 Fleet Runtime — source-aware read-only Admiral observability. */
;(()=>{
...
(() => {
  ...
  const UPPER_COMMAND_BUILD='8.8.15.2';
  ...
  const FOUNDRY_SCOUT_DRAFT_KEY='darkSkyFoundryScoutDrafts88151';
  function foundryScoutRead(){try{const v=JSON.parse(localStorage.getItem(FOUNDRY_SCOUT_DRAFT_KEY)||'[]');return Array.isArray(v)?v:[];}catch(_){return[];}}
  function foundryScoutWrite(rows){try{localStorage.setItem(FOUNDRY_SCOUT_DRAFT_KEY,JSON.stringify((Array.isArray(rows)?rows:[]).slice(-40)));}catch(_){}}
  function foundryScoutEsc(v){return String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));}
  function foundryScoutStatus(row){const required=['commercial','attribution','identity','security','dependencies','privacy','maintenance'];const cleared=required.every(k=>row?.checks?.[k]===true)&&/^https:\/\//i.test(String(row?.sourceUrl||''))&&String(row?.licenseId||'').trim()&&String(row?.name||'').trim()&&String(row?.mission||'').trim();return cleared?'SEA TRIAL CLEARED':'HOLD';}
  function foundryScoutRowsMarkup(){const rows=foundryScoutRead().slice().reverse();if(!rows.length)return '<div class="foundry-scout-empty">No Scout candidates yet. Start with a real mission or recurring customer friction—not a random repository.</div>';return rows.map(r=>{const st=foundryScoutStatus(r);return `<article class="foundry-scout-row" data-foundry-candidate="${foundryScoutEsc(r.id)}"><div><small>${foundryScoutEsc(r.route||'utility-dock').replaceAll('-',' ').toUpperCase()} • ADMIRAL DRAFT</small><strong>${foundryScoutEsc(r.name||'Unnamed candidate')}</strong><span>${foundryScoutEsc(r.sourceUrl||'No source URL')}</span><p>${foundryScoutEsc(r.mission||'No mission recorded')}</p></div><div class="foundry-scout-row-state"><b class="${st==='SEA TRIAL CLEARED'?'clear':'hold'}">${st}</b><em>${foundryScoutEsc(r.licenseId||'LICENSE UNSET')}</em><button type="button" data-foundry-delete="${foundryScoutEsc(r.id)}">REMOVE DRAFT</button></div></article>`}).join('');}
  function bindFoundryScout(overlay){
    const notice=byId('foundryNotice');
    const recalc=()=>{const req=['commercial','attribution','identity','security','dependencies','privacy','maintenance'];const ready=req.every(k=>byId('foundryCheck_'+k)?.checked)&&/^https:\/\//i.test(String(byId('foundrySourceUrl')?.value||''))&&String(byId('foundryCandidateName')?.value||'').trim()&&String(byId('foundryLicenseId')?.value||'').trim()&&String(byId('foundryMission')?.value||'').trim();const badge=byId('foundryClearanceState');if(badge){badge.textContent=ready?'SEA TRIAL CLEARED':'HOLD';badge.className='foundry-clearance-state '+(ready?'clear':'hold');}return ready;};
    ...
    byId('foundrySaveCandidate')?.addEventListener('click',()=>{const row={id:'FG-'+Date.now().toString(36),createdAt:new Date().toISOString(),name:String(byId('foundryCandidateName')?.value||'').trim(),sourceUrl:String(byId('foundrySourceUrl')?.value||'').trim(),mission:String(byId('foundryMission')?.value||'').trim(),licenseId:String(byId('foundryLicenseId')?.value||'').trim(),route:String(byId('foundryProductRoute')?.value||'utility-dock'),dataClass:String(byId('foundryDataClass')?.value||'local-files-only'),checks:{commercial:!!byId('foundryCheck_commercial')?.checked,attribution:!!byId('foundryCheck_attribution')?.checked,identity:!!byId('foundryCheck_identity')?.checked,security:!!byId('foundryCheck_security')?.checked,dependencies:!!byId('foundryCheck_dependencies')?.checked,privacy:!!byId('foundryCheck_privacy')?.checked,maintenance:!!byId('foundryCheck_maintenance')?.checked}};if(!row.name||!row.sourceUrl||!row.mission){if(notice)notice.textContent='Candidate name, public source URL and mission are required before saving a Foundry draft.';return;}const rows=foundryScoutRead();rows.push(row);foundryScoutWrite(rows);const list=byId('foundryScoutRows');if(list)list.innerHTML=foundryScoutRowsMarkup();if(notice)notice.textContent=foundryScoutStatus(row)==='SEA TRIAL CLEARED'?'Candidate saved as an Admiral draft and cleared only for Sea Trial. Nothing was published or commercially activated.':'Candidate saved on HOLD. The missing clearance evidence must be resolved before Sea Trial.';bindFoundryScoutDeletes(overlay);});
    function bindFoundryScoutDeletes(root){root.querySelectorAll('[data-foundry-delete]').forEach(btn=>btn.onclick=()=>{const rows=foundryScoutRead().filter(x=>x.id!==btn.dataset.foundryDelete);foundryScoutWrite(rows);const list=byId('foundryScoutRows');if(list)list.innerHTML=foundryScoutRowsMarkup();if(notice)notice.textContent='Foundry draft removed. No Fleet capability or vessel state was changed.';bindFoundryScoutDeletes(root);});}
    bindFoundryScoutDeletes(overlay);recalc();
  }
  ...
  function ensureFoundryWorkspace(){
    ...
    document.body.appendChild(overlay);byId('foundryClose').onclick=()=>{hide('foundryWorkspace');show('admiralDeck');};byId('foundryAudit').onclick=()=>{const n=byId('foundryNotice');if(n)n.textContent=`${registry.length} capability foundations classified across ${Object.keys(domains).length} Foundry domains. Scout drafts remain local Admiral working records; nothing was published.`;};bindFoundryScout(overlay);return overlay;
  }
