/* 8.8.15.7 Fleet Runtime — a full document load must never inherit
...
;(() => {
  ...
  window.DarkSkyAdmiralFreshGate=Object.freeze({build:'8.8.15.7',requiredPerDocument:true,engineAuthoritySeparate:true});
})();
/* Dark Sky 8.8.15.7 Fleet Runtime — source-aware read-only Admiral observability. */
;(()=>{
...
(() => {
  ...
  const UPPER_COMMAND_BUILD='8.8.15.7';
...
/* 8.8.15.7 Station Registry — readable Control Plane, durable course preview and human Activity Spine. */
;(()=>{
  'use strict';
  const SESSION_KEY='darkSkySupabaseAdmiralSessionV1';
  ...
  const readSession=()=>{try{return JSON.parse(sessionStorage.getItem(SESSION_KEY)||'null')}catch(_){return null}};
  const saveSession=data=>{if(!data?.access_token)return null;const s={access_token:data.access_token,refresh_token:data.refresh_token||'',expires_at:Date.now()+Math.max(60,Number(data.expires_in||3600))*1000,user:data.user||null};sessionStorage.setItem(SESSION_KEY,JSON.stringify(s));return s;};
  async function currentSession(){let s=readSession(),c=cfg();if(s&&s.expires_at>Date.now()+30000)return s;if(!s?.refresh_token||!c)return null;const r=await fetch(c.url+'/auth/v1/token?grant_type=refresh_token',{method:'POST',headers:headers(),body:JSON.stringify({refresh_token:s.refresh_token})});return r.ok?saveSession(await r.json()):null;}
  async function rpc(name,body={}){const c=cfg(),s=await currentSession();if(!c?.url||!s?.access_token)throw new Error('Authenticate the Admiral identity first.');const r=await fetch(c.url+'/rest/v1/rpc/'+name,{method:'POST',headers:{...headers(s.access_token),Accept:'application/json'},body:JSON.stringify(body)});if(!r.ok){let m='Fleet Core refused the command.';try{const e=await r.json();m=e?.message||e?.hint||m}catch(_){}throw new Error(String(m).replaceAll('_',' '));}return r.json();}
  ...
  function open(){install();const w=$('fleetSpineWorkspace');w.classList.remove('hidden');w.setAttribute('aria-hidden','false');document.body.classList.add('fleet-spine-open');requestAnimationFrame(()=>{w.scrollTo({top:0,behavior:'auto'});$('fleetSpineClose')?.focus({preventScroll:true});});syncIdentity().catch(e=>setStatus(e?.message||'Fleet Spine could not open.','error'));}
...
/* 8.8.15.7 Station Registry — deliberate local-manifest adoption into durable Fleet Core station identity. */
;(()=>{
  'use strict';
  const SESSION_KEY='darkSkySupabaseAdmiralSessionV1';
  ...
  async function currentSession(){let s=readSession(),c=cfg();if(s&&s.expires_at>Date.now()+30000)return s;if(!s?.refresh_token||!c)return null;const r=await fetch(c.url+'/auth/v1/token?grant_type=refresh_token',{method:'POST',headers:headers(),body:JSON.stringify({refresh_token:s.refresh_token})});if(!r.ok)return null;const data=await r.json();const next={access_token:data.access_token,refresh_token:data.refresh_token||s.refresh_token||'',expires_at:Date.now()+Math.max(60,Number(data.expires_in||3600))*1000,user:data.user||s.user||null};sessionStorage.setItem(SESSION_KEY,JSON.stringify(next));return next;}
  async function rpc(name,body={}){const c=cfg(),s=await currentSession();if(!c?.url||!s?.access_token)throw new Error('Authenticate the Admiral identity first.');const r=await fetch(c.url+'/rest/v1/rpc/'+name,{method:'POST',headers:{...headers(s.access_token),Accept:'application/json'},body:JSON.stringify(body)});if(!r.ok){let m='Fleet Core refused the station command.';try{const e=await r.json();m=e?.message||e?.hint||m}catch(_){}throw new Error(String(m).replaceAll('_',' '));}return r.json();}
  ...
  async function issueStation(){const row=selectedRow(),l=row?.local,intent=$('fleetSpineStationIntent')?.value.trim();if(!row||!l||!preview)throw new Error('Preview the station identity first.');if(signature(row)!==previewSig){invalidate();throw new Error('Station details changed. Preview again before registration.');}if(!intent)throw new Error('Record the Admiral station intent before registering.');const button=$('fleetSpineStationIssue');button.disabled=true;button.textContent='REGISTERING VERIFIED STATION…';try{const data=await rpc('admiral_register_station',{p_project_id:l.projectId,p_station_key:l.stationKey,p_display_name:l.displayName,p_operating_mode:l.operatingMode,p_station_state:l.stationState,p_runtime_release:l.runtimeRelease||null,p_policy_version:l.policyVersion||null,p_station_profile:l.stationProfile||null,p_expected_fingerprint:preview.fingerprint,p_intent:intent});await load();const status=$('fleetSpineStatus');if(status){status.textContent='STATION IDENTITY VERIFIED · '+l.displayName+' is durable in Fleet Core. Heartbeat remains awaiting real runtime proof.';status.dataset.kind='success';}const host=$('fleetSpineStationInspector');if(host)host.insertAdjacentHTML('afterbegin',`<div class="fleet-spine-station-confirmation"><b>STATION IDENTITY VERIFIED</b><span>${esc(l.displayName)} is now durable in Fleet Core. Heartbeat remains awaiting real station runtime proof. Use REFRESH SPINE when you want the new Activity Spine event reflected above.</span></div>`);return data;}finally{button.textContent='REGISTER EXACT STATION';}}
  async function load(){if(!$('fleetSpineStationRegistry')&&!install())return;const summary=$('fleetSpineStationSummary'),rowsHost=$('fleetSpineStationRows');if(summary)summary.textContent='READING STATIONS…';const s=await currentSession();if(!s?.access_token){snapshot=null;renderRows();renderInspector();if(summary)summary.textContent='ADMIRAL AUTHENTICATION REQUIRED';return;}try{snapshot=await rpc('admiral_read_fleet_spine',{p_event_limit:30});if(selected&&!merged().some(r=>r.id===selected))selected='';renderRows();renderInspector();}catch(err){snapshot={stations:[]};if(summary)summary.textContent='STATION READ FAILED';if(rowsHost)rowsHost.innerHTML=`<div class="fleet-spine-station-empty"><strong>Station posture could not be read.</strong><span>${esc(err?.message||'Fleet Core read failed.')} No station identity changed.</span></div>`;renderInspector();throw err;}}
  ...
  document.addEventListener('click',async e=>{
    ...
    if(t.closest?.('#fleetSpineRefresh,#fleetSpineSignIn'))setTimeout(()=>load().catch(()=>{}),450);
  },true);
  ...
  const obs=new MutationObserver(()=>{if(install())load().catch(()=>{});else if($('fleetSpineWorkspace')&&!$('fleetSpineWorkspace').classList.contains('hidden')&&$('fleetSpineStationRegistry')&&!snapshot)load().catch(()=>{});});
