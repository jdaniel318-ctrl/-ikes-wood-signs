/* 8.8.15.6 Admiral Airlock — a full document load must never inherit
...
/* Dark Sky 8.8.15.6 Admiral Airlock — server-attested, read-only Admiral observability. */
;(()=>{
  ...
  function install(){$('bootstrapFleet')?.remove();document.body.insertAdjacentHTML('beforeend',`<section id="bootstrapFleet" class="bootstrap-fleet fleet-watchtower hidden" role="dialog" aria-modal="true" aria-hidden="true" aria-label="Black Flag Admiral security and Fleet Watchtower"><section id="watchSecurityLayer" class="admiral-security-layer" aria-labelledby="watchSecurityTitle"><div class="admiral-security-card"><header><img src="black_flag_primary_lockup.png" alt="Black Flag Business Command Platform"><button id="watchGateClose" type="button">CLOSE</button></header><div class="admiral-security-heading"><small>8.8.15.6 · ADMIRAL AIRLOCK</small><h2 id="watchSecurityTitle">Admiral Security Layer</h2><p>The PIN opened Admiral Passage. Your dedicated Admiral account must now be authenticated before any fleet data or work controls can open.</p></div><ol class="admiral-security-steps" aria-label="Admiral authentication progress"><li class="is-complete"><span>01</span><div><small>PASSAGE PIN</small><b>VERIFIED</b></div></li><li class="is-current"><span>02</span><div><small>ADMIRAL ACCOUNT</small><b>REQUIRED</b></div></li><li><span>03</span><div><small>FLEET WORKSPACE</small><b>LOCKED</b></div></li></ol><form id="watchSecurityForm" class="admiral-security-form"><label>Admiral email<input id="watchEmail" type="email" inputmode="email" autocomplete="username" autocapitalize="none" spellcheck="false" placeholder="Dedicated Admiral email" required></label><label>Password<input id="watchPassword" type="password" autocomplete="current-password" placeholder="Admiral account password" required></label><button id="watchSignIn" type="submit">AUTHENTICATE ADMIRAL</button></form><p id="watchGateStatus" class="admiral-security-status" role="status">Second security check required. The Fleet workspace remains locked.</p><aside><b>Two credentials. Two purposes.</b><span>The PIN permits entry to Admiral Passage. Supabase verifies the named Admiral account and its global read authority. Owner accounts cannot open this workspace.</span></aside></div></section><div id="watchWorkspace" class="bootstrap-fleet-shell hidden" aria-hidden="true"><header><img src="black_flag_primary_lockup.png" alt="Black Flag Business Command Platform"><div><small>8.8.15.6 · ADMIRAL AIRLOCK</small><h2>My Fleet</h2><p>Authenticated, read-only operational truth across independent vessels.</p></div><div><button id="watchRefresh">REFRESH WATCH</button><button id="watchSignOut">SIGN OUT</button><button id="watchClose">CLOSE</button></div></header><p id="watchStatus" class="bootstrap-fleet-status" role="status">Reading Fleet Watch…</p><section id="watchKeel" class="fleet-supabase-keel hidden" aria-label="Supabase fleet commissioning posture"></section><section id="watchTools" class="fleet-watch-tools hidden"><label>Search the fleet<input id="watchSearch" type="search" placeholder="Vessel name, key, or operating model"></label><div>${[['all','ALL'],['attention','NEEDS ATTENTION'],['stale','STALE'],['commissioning','COMMISSIONING'],['staging','STAGING'],['live','LIVE'],['owner','OWNER-OPERATED'],['fleet','FLEET-OPERATED']].map(([v,l],i)=>`<button data-watch-filter="${v}" class="${i?'':'is-selected'}" aria-pressed="${i?'false':'true'}">${l}</button>`).join('')}</div></section><section class="fleet-watch-section"><header><div><small>FLEET CORE</small><h3>Independent vessels</h3></div><span id="watchCount">0 VESSELS</span></header><div id="watchRows" class="bootstrap-fleet-rows"></div></section><section class="fleet-watch-section fleet-watch-programs"><header><div><small>ADMIRAL PROGRAMS</small><h3>Programs outside Fleet Core</h3></div><span>SEPARATE COMMAND CLASS</span></header><div id="watchPrograms" class="bootstrap-fleet-rows"></div></section><aside class="bootstrap-fleet-passage"><b>Admiral Observability</b><span>Observation is read-only and never becomes ownership. Any modifying command remains exact-vessel, previewed, authenticated, reasoned, and audited.</span></aside><footer><b>Read-only Watchtower boundary</b><span>No control here can rename, commission, publish, entitle, schedule, or otherwise change a vessel.</span></footer></div><section id="watchOverview" class="fleet-vessel-overview hidden" role="dialog" aria-modal="true" aria-hidden="true"><div class="fleet-vessel-overview-shell"><button id="watchOverviewClose">← BACK TO MY FLEET</button><div id="watchOverviewBody"></div></div></section></section>`);}
  function controlLabel(v){return ({owner_ready:'OWNER READY',owner_attention:'OWNER ATTENTION',fleet_seeded:'FLEET SEEDED',registry_only:'REGISTRY ONLY'})[v.supabase_control_state]||'POSTURE UNKNOWN';}
  function paintKeel(){const c=spine?.counts||{},host=$('watchKeel');if(!host)return;host.classList.toggle('hidden',!records.length);host.innerHTML=`<header><div><small>ADMIRAL AIRLOCK · SERVER POSTURE</small><b>${Number(c.vessels||records.length)} registered vessels</b><span>Registry, settings, identity, and exact-vessel membership are read from Supabase.</span></div><em>READ ONLY</em></header><div><span><b>${Number(c.supabase_settings_rows||0)}</b><small>SETTINGS SEEDED</small></span><span><b>${Number(c.owner_ready_vessels||0)}</b><small>OWNER READY</small></span><span><b>${Number(c.fleet_seeded_vessels||0)}</b><small>FLEET SEEDED</small></span><span><b>${Number(c.owner_attention_vessels||0)+Number(c.registry_only_vessels||0)}</b><small>NEEDS ATTENTION</small></span></div><p>Fleet-seeded means the vessel has a server record but no outside owner has been invented. Owner authority exists only after a real identity and exact Project ID membership are both present.</p>`;}
  ...
  function render(next=records){records=next.filter(v=>v.mission_class!=='admiral_program');const shown=records.filter(matches),fresh=records.filter(v=>facts(v).reported&&!facts(v).stale).length;$('watchRows').innerHTML=shown.length?shown.map(v=>card(v)).join(''):'<article class="bootstrap-fleet-empty"><div><b>No vessels match this watch</b><span>Clear the search or choose another filter. Fleet state was not changed.</span></div></article>';$('watchPrograms').innerHTML=card(PROGRAM,true);$('watchCount').textContent=shown.length+' OF '+records.length+' VESSELS';$('watchStatus').textContent='SERVER-ATTESTED READ · '+records.length+' Fleet Core vessels · '+fresh+' fresh reports · synchronized '+(lastSync?new Date(lastSync).toLocaleString():'not yet')+'. Never-reported and stale vessels remain explicit.';paintKeel();}
