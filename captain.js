/* 8.8.15.4 Fleet Runtime — a full document load must never inherit
...
;(() => {
  ...
  window.DarkSkyAdmiralFreshGate=Object.freeze({build:'8.8.15.4',requiredPerDocument:true,engineAuthoritySeparate:true});
})();
/* Dark Sky 8.8.15.4 Fleet Runtime — source-aware read-only Admiral observability. */
;(()=>{
...
(() => {
  ...
  const UPPER_COMMAND_BUILD='8.8.15.4';
...
/* 8.8.15.4 Fleet Spine — durable Control Plane, Activity Spine and release rings. */
;(()=>{
  'use strict';
  const SESSION_KEY='darkSkySupabaseAdmiralSessionV1';
  ...
  function vesselRows(){const rows=Array.isArray(snapshot?.vessels)?snapshot.vessels:[];$('fleetSpineVessels').innerHTML=rows.length?rows.map(v=>`<button type="button" class="fleet-spine-vessel ${selectedProject===v.project_id?'is-selected':''}" data-spine-vessel="${esc(v.project_id)}"><div><small>${esc(String(v.lifecycle_state||'unknown').toUpperCase())} · ${esc(v.release_ring||'unassigned')}</small><b>${esc(v.display_name)}</b><span>${esc(v.project_id)}</span></div><div><strong>${esc(v.current_release||'NO RELEASE ASSIGNED')}</strong><span>${esc(String(v.rollout_state||'hold').replaceAll('_',' ').toUpperCase())} · ${Number(v.station_count||0)} STATIONS · ${Number(v.membership_count||0)} MEMBERS</span></div></button>`).join(''):'<p>No Fleet Core vessels found.</p>';}
  function eventRows(){const rows=Array.isArray(snapshot?.recent_events)?snapshot.recent_events:[];$('fleetSpineEvents').innerHTML=rows.length?rows.map(e=>`<article><div><small>${esc(new Date(e.created_at).toLocaleString())} · ${esc(e.actor_authority||'system')}</small><b>${esc(e.event_type)}</b><span>${esc(e.source)} · ${esc(e.correlation_id)}</span></div><pre>${esc(JSON.stringify(e.payload||{}))}</pre></article>`).join(''):'<p>No Activity Spine events yet.</p>';}
  function selectedVessel(){return (snapshot?.vessels||[]).find(v=>v.project_id===selectedProject)||null;}
  function course(){const v=selectedVessel(),host=$('fleetSpineCourse');preview=null;if(!v){host.innerHTML='<p>Select one vessel. Reading never changes it.</p>';return;}host.innerHTML=`<div class="fleet-spine-course-current"><small>CURRENT COURSE</small><b>${esc(v.display_name)}</b><span>${esc(v.release_ring||'unassigned')} · ${esc(v.current_release||'no release')} · ${esc(v.rollout_state||'hold')}</span></div><label>Release ring<select id="fleetSpineRing">${['unassigned','canary','proving','limited','fleet'].map(x=>`<option value="${x}" ${x===v.release_ring?'selected':''}>${x.toUpperCase()}</option>`).join('')}</select></label><label>Rollout state<select id="fleetSpineRollout">${['hold','candidate','sea_trial','proven','live','rollback_ready'].map(x=>`<option value="${x}" ${x===v.rollout_state?'selected':''}>${x.replaceAll('_',' ').toUpperCase()}</option>`).join('')}</select></label><label>Release identity<input id="fleetSpineRelease" value="${esc(v.current_release||'')}" placeholder="Example: 8.8.15.4"></label><label>Admiral intent<input id="fleetSpineIntent" maxlength="500" placeholder="Why this exact vessel should change course"></label><button id="fleetSpinePreview" type="button">PREVIEW COURSE</button><div id="fleetSpinePreviewBox" class="fleet-spine-preview"><span>No change is authorized until a preview fingerprint is returned.</span></div><button id="fleetSpineIssue" type="button" disabled>ISSUE RELEASE COURSE</button>`;}
  async function syncIdentity(){const ok=!!(await window.DarkSkyAdmiralIdentityStatus?.());$('fleetSpineWorkspace')?.classList.toggle('identity-verified',ok);$('fleetSpineIdentityState').textContent=ok?'ADMIRAL IDENTITY VERIFIED':'ADMIRAL IDENTITY REQUIRED';['fleetSpineEmail','fleetSpinePassword'].forEach(id=>$(id)?.closest('label')?.classList.toggle('hidden',ok));$('fleetSpineSignIn')?.classList.toggle('hidden',ok);$('fleetSpineSignOut')?.classList.toggle('hidden',!ok);if(ok)await load();else{snapshot=null;$('fleetSpineStatus').textContent='Authenticate the Admiral account. Fleet Spine remains closed.';$('fleetSpineCounts').innerHTML='';$('fleetSpineVessels').innerHTML='';$('fleetSpineEvents').innerHTML='';course();}return ok;}
  async function load(){const data=await rpc('admiral_read_fleet_spine',{p_event_limit:30});snapshot=data;if(selectedProject&&!selectedVessel())selectedProject='';counts();vesselRows();eventRows();course();$('fleetSpineStatus').textContent='CONTROL PLANE CURRENT · '+new Date(data.read_at).toLocaleString()+' · Read-only until a previewed course is deliberately issued.';}
  async function previewCourse(){const v=selectedVessel();if(!v)return;const data=await rpc('admiral_preview_release_assignment',{p_project_id:v.project_id,p_release_ring:$('fleetSpineRing').value,p_rollout_state:$('fleetSpineRollout').value,p_current_release:$('fleetSpineRelease').value.trim()||null});preview=data;const b=data.before||{},a=data.after||{};$('fleetSpinePreviewBox').innerHTML=`<b>${data.changes?'COURSE CHANGE':'NO CHANGE NEEDED'}</b><span>${esc(String(b.release_ring||'unassigned').toUpperCase())} / ${esc(b.current_release||'—')} / ${esc(b.rollout_state||'hold')} → ${esc(String(a.release_ring||'unassigned').toUpperCase())} / ${esc(a.current_release||'—')} / ${esc(a.rollout_state||'hold')}</span><small>Fingerprint ${esc(data.fingerprint)}</small>`;$('fleetSpineIssue').disabled=!data.changes;$('fleetSpineStatus').textContent='Preview returned. Nothing has changed.';}
  async function issueCourse(){const v=selectedVessel(),intent=$('fleetSpineIntent')?.value.trim();if(!preview||!v)throw new Error('Preview the release course first.');if(!intent)throw new Error('Record the Admiral intent before issuing this course.');const button=$('fleetSpineIssue');button.disabled=true;button.textContent='ISSUING…';try{const data=await rpc('admiral_issue_release_assignment',{p_project_id:v.project_id,p_release_ring:$('fleetSpineRing').value,p_rollout_state:$('fleetSpineRollout').value,p_current_release:$('fleetSpineRelease').value.trim()||null,p_expected_fingerprint:preview.fingerprint,p_intent:intent});$('fleetSpineStatus').textContent=data.changed?'COURSE VERIFIED · '+v.display_name+' updated. Activity Spine retained the command.':'No change was needed.';await load();}finally{button.textContent='ISSUE RELEASE COURSE';}}
  function open(){install();const w=$('fleetSpineWorkspace');w.classList.remove('hidden');w.setAttribute('aria-hidden','false');document.body.classList.add('fleet-spine-open');requestAnimationFrame(()=>{w.scrollTo({top:0,behavior:'auto'});$('fleetSpineClose')?.focus({preventScroll:true});});syncIdentity().catch(e=>$('fleetSpineStatus').textContent=e?.message||'Fleet Spine could not open.');}
  ...
  document.addEventListener('click',async e=>{const t=e.target;if(t.closest?.('#admiralFleetSpineOpen,#admiralCeremonialFleetSpine'))return open();if(t.closest?.('#fleetSpineClose'))return close();if(t.closest?.('#fleetSpineRefresh')){try{await load()}catch(err){$('fleetSpineStatus').textContent=err?.message||'Refresh failed.'}return;}if(t.closest?.('#fleetSpineSignIn')){const b=$('fleetSpineSignIn');try{b.disabled=true;b.textContent='AUTHENTICATING…';await window.DarkSkyAuthenticateAdmiral?.($('fleetSpineEmail').value,$('fleetSpinePassword').value);$('fleetSpinePassword').value='';await syncIdentity();}catch(err){$('fleetSpineStatus').textContent=err?.message||'Admiral sign-in failed.'}finally{b.disabled=false;b.textContent='AUTHENTICATE ADMIRAL'}return;}if(t.closest?.('#fleetSpineSignOut')){window.DarkSkySignOutAdmiral?.();await syncIdentity();return;}const vessel=t.closest?.('[data-spine-vessel]');if(vessel){selectedProject=vessel.dataset.spineVessel;vesselRows();course();return;}if(t.closest?.('#fleetSpinePreview')){try{await previewCourse()}catch(err){$('fleetSpineStatus').textContent=err?.message||'Preview failed.'}return;}if(t.closest?.('#fleetSpineIssue')){try{await issueCourse()}catch(err){$('fleetSpineStatus').textContent=err?.message||'Course issue failed.'}return;}},true);
