/* 8.8.14.23 Deck Signal — a full document load must never inherit
...
;(() => {
  ...
  window.DarkSkyAdmiralFreshGate=Object.freeze({build:'8.8.14.23',requiredPerDocument:true,engineAuthoritySeparate:true});
})();
/* Dark Sky 8.8.14.23 Deck Signal — source-aware read-only Admiral observability. */
;(()=>{
...
(() => {
  ...
  const UPPER_COMMAND_BUILD='8.8.14.23';
  ...
  function refreshCaptainFleetChart(){
    ...
    if(summary) summary.innerHTML=`<strong>ROUTE SAFE</strong><span>${fleet.length} vessels</span><span>${active} outpost${active===1?'':'s'} sailing</span>${attention?`<span class="attention">${attention} needing attention</span>`:'<span>waters steady</span>'}`;    refreshCaptainWatch();
  }
