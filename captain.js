/* 8.8.14.13 Session Current — a full document load must never inherit
...
;(() => {
  ...
  window.DarkSkyAdmiralFreshGate=Object.freeze({build:'8.8.14.13',requiredPerDocument:true,engineAuthoritySeparate:true});
})();
/* Dark Sky 8.8.14.13 Session Current — source-aware read-only Admiral observability. */
;(()=>{
...
(() => {
  ...
  const UPPER_COMMAND_BUILD='8.8.14.13';
  ...
  function refreshCaptainFleetChart(){
    ...
    if(summary) summary.innerHTML=`<strong>OWNER STEADY</strong><span>${fleet.length} vessels</span><span>${active} outpost${active===1?'':'s'} sailing</span>${attention?`<span class="attention">${attention} needing attention</span>`:'<span>waters steady</span>'}`;    refreshCaptainWatch();
  }
