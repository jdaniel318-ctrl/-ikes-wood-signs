...
;(()=>{
  ...
  async function syncIdentityUi(){const ok=await verifyAdmiral();if(state())state().textContent=ok?'ADMIRAL IDENTITY VERIFIED':'ADMIRAL IDENTITY REQUIRED';station()?.classList.toggle('identity-verified',ok);setBusy(!ok);document.getElementById('admiralIdentitySignOut')?.classList.toggle('hidden',!ok);if(ok){await loadEntitlement();}else{if(currentSetting())currentSetting().textContent='SIGN IN TO VIEW';if(currentDetail())currentDetail().textContent='The current setting appears here before you make a change.';}return ok;}
