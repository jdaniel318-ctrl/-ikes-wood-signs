/* 8.8.16.1 Clear Watch — a full document load must never inherit
...
;(() => {
  ...
  window.DarkSkyAdmiralFreshGate=Object.freeze({build:'8.8.16.1',requiredPerDocument:true,engineAuthoritySeparate:true});
})();
/* Dark Sky 8.8.16.1 Clear Watch — server-attested, read-only Admiral observability. */
;(()=>{
  ...
  function paintKeel(){const c=spine?.counts||{},host=$('watchKeel');if(!host)return;const fresh=records.filter(v=>facts(v).reported&&!facts(v).stale).length,action=records.filter(v=>facts(v).attention).length;host.classList.toggle('hidden',!records.length);host.innerHTML=`<header><div><small>CLEAR WATCH · SERVER POSTURE</small><b>${Number(c.vessels||records.length)} registered vessels</b><span>Registry, settings, identity, membership, and reporting posture are read from Supabase.</span></div><em>READ ONLY</em></header><div><span><b>${Number(c.supabase_settings_rows||0)}</b><small>SETTINGS READY</small></span><span><b>${Number(c.owner_ready_vessels||0)}</b><small>OWNER READY</small></span><span><b>${fresh}</b><small>FRESH REPORTS</small></span><span class="${action?'is-action':''}"><b>${action}</b><small>ACTION QUEUE</small></span></div><p>Action Queue includes never-reported, stale, unresolved, held, or blocked vessels. Owner-unassigned means a server record exists, but no owner identity has been invented.</p>`;}
...
(() => {
  ...
  const UPPER_COMMAND_BUILD='8.8.16.1';
...
;(()=>{
  ...
  const cfg=()=>window.BlackFlagV3Identity?.productionAuth?.readClientConfig?.()||null;
  const client=()=>{const c=cfg();if(!c?.url||!c?.publishableKey)throw new Error('Supabase identity is not configured.');return window.DarkSkySupabase.create({url:c.url,publishableKey:c.publishableKey,sessionKey:SESSION_KEY,build:'8.8.16.1'});};
  ...
  async function syncIdentity(){
    ...
    rankLabels.forEach(node=>node.textContent=verified?'VERIFIED ADMIRAL':'PROVISIONAL');
    const gateBadge=el('admiralGateOverlay')?.querySelector('.admiral-trial-badge');
    if(gateBadge)gateBadge.textContent=verified?'ADMIRAL STATUS • SERVER VERIFIED':'ADMIRAL STATUS • NOT YET COMMISSIONED';
    ...
    if(passageStatus)passageStatus.textContent=verified?'ADMIRAL PASSAGE COMPLETE • Active Fleet Core authority verified for this authenticated identity.':'Captain access can prove the passage. Only verified server authority commissions the rank.';
