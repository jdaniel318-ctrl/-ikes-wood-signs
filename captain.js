/* 8.8.11.9 Project Roster — a full document load must never inherit
...
;(() => {
  ...
  window.DarkSkyAdmiralFreshGate=Object.freeze({build:'8.8.11.9',requiredPerDocument:true,engineAuthoritySeparate:true});
})();
...
(() => {
  ...
  const UPPER_COMMAND_BUILD='8.8.11.9';
...
/* 8.8.11.9 Quick Passage — fleet registry isolated from retired schedules. */
;(()=>{
  const el=id=>document.getElementById(id);
  const esc=value=>String(value??'').replace(/[&<>"']/g,char=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[char]));
  function install(){
...
/* 8.8.11.9 Project Roster — three isolated schedules with durable local records. */
;(()=>{
