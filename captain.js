(() => {
  ...
  const UPPER_COMMAND_BUILD='8.8.7';
...
/* 8.8.7 Bootstrap Build Helm — canonical vessel identity, Admiral-controlled. */
;(()=>{
  ...
  install();installCommissionBrand();window.DarkSkySyncVesselBrandIdentity=sync;
...
/* 8.8.7 Bootstrap Build — local-first construction schedule proving ground. */
;(()=>{
  ...
  function install(){const kit=el('admiralCommissioningBrandKit');if(kit&&!el('bootstrapScheduleOpen'))kit.insertAdjacentHTML('beforeend','<button id="bootstrapScheduleOpen" class="admiral-pro-button" type="button">OPEN TEST CONSTRUCTION SCHEDULE</button>');if(el('bootstrapSchedule'))return;document.body.insertAdjacentHTML('beforeend',`<section id="bootstrapSchedule" class="bootstrap-schedule hidden" aria-label="Bootstrap Build construction schedule prototype"><div class="bootstrap-schedule-shell"><header class="bootstrap-schedule-head"><img src="bootstrap_build_horizontal.png" alt="Bootstrap Build"><div><small>LOCAL PROVING GROUND · NO FLEET CORE WRITES</small><h2>Construction Schedule</h2><p>Drag familiar work between days for the test unit.</p></div><button id="bootstrapScheduleClose" type="button">CLOSE</button></header><nav class="bootstrap-schedule-crumbs" aria-label="Test project hierarchy"><span><small>REGION</small><b>ABC</b></span><i>›</i><span><small>DIVISION</small><b>CBA</b></span><i>›</i><span><small>COMMUNITY</small><b>AB</b></span><i>›</i><span><small>LOT</small><b>1234</b></span><i>›</i><span><small>UNIT</small><b>A</b></span></nav><div class="bootstrap-schedule-toolbar"><div><button id="bootstrapSchedulePrev" type="button">← PRIOR WEEK</button><strong id="bootstrapScheduleWeek"></strong><button id="bootstrapScheduleNext" type="button">NEXT WEEK →</button></div><div><button id="bootstrapScheduleAdd" type="button">+ ADD TASK</button><button id="bootstrapScheduleReset" type="button">RESET TEST DATA</button></div></div><form id="bootstrapScheduleForm" class="bootstrap-schedule-form hidden"><label>Task<input id="bootstrapScheduleTitle" maxlength="80" required placeholder="Construction task"></label><label>Trade<input id="bootstrapScheduleTrade" maxlength="40" required placeholder="Trade or crew"></label><label>Day<select id="bootstrapScheduleDay"></select></label><button type="submit">ADD TO SCHEDULE</button><button id="bootstrapScheduleCancel" type="button">CANCEL</button></form><p id="bootstrapScheduleStatus" class="bootstrap-schedule-status" role="status">Test schedule ready. Drag a task card to another day.</p><div id="bootstrapScheduleBoard" class="bootstrap-schedule-board"></div><footer><b>Prototype boundary</b><span>This calendar is browser-local test data for Region ABC. It does not create a Fleet Core vessel, customer commitment, owner assignment, or live construction record.</span></footer></div></section>`);}
...
/* 8.8.7 Bootstrap Build Helm — permanent keel, editable working name. */
;(()=>{
  const SESSION_KEY='darkSkySupabaseAdmiralSessionV1';
