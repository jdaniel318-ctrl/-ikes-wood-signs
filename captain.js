/* 8.8.15.5 Supabase Keel — a full document load must never inherit
...
/* Dark Sky 8.8.15.5 Supabase Keel — server-attested, read-only Admiral observability. */
;(()=>{
  ...
  function paintKeel(){const c=spine?.counts||{},host=$('watchKeel');if(!host)return;host.classList.toggle('hidden',!records.length);host.innerHTML=`<header><div><small>SUPABASE KEEL · SERVER POSTURE</small><b>${Number(c.vessels||records.length)} registered vessels</b><span>Registry, settings, identity, and exact-vessel membership are read from Supabase.</span></div><em>READ ONLY</em></header><div><span><b>${Number(c.supabase_settings_rows||0)}</b><small>SETTINGS SEEDED</small></span><span><b>${Number(c.owner_ready_vessels||0)}</b><small>OWNER READY</small></span><span><b>${Number(c.fleet_seeded_vessels||0)}</b><small>FLEET SEEDED</small></span><span><b>${Number(c.owner_attention_vessels||0)+Number(c.registry_only_vessels||0)}</b><small>NEEDS ATTENTION</small></span></div><p>Fleet-seeded means the vessel has a server record but no outside owner has been invented. Owner authority exists only after a real identity and exact Project ID membership are both present.</p>`;}
