...
## Project Registry Recovery Vault Law — v3.9.13
...
## Durable Recovery Vault Law — v3.9.13
The Project Registry Recovery Vault is mirrored in IndexedDB and local storage. Continuity analysis hydrates the durable vault before fleet rendering. Every verified fleet generation is retained in rotating history. A Project ID may disappear from the current registry only with explicit retirement evidence; otherwise the Engine must surface a continuity alert and attempt immutable-ID recovery from verified history.
