# 6.0.7 — Benchmark True North
- Removed the remaining inherited Legacy benchmark styling conflicts by adding a final isolated renderer layer.
- Fixed the hero image stacking bug that placed the photograph behind the hero background on Safari/iPad.
- Locked the saved benchmark hierarchy across masthead, hero, trust rail, service grid and How It Works.
- Kept the exact canonical Legacy logo unchanged and project-controlled.
- Reduced internal test-navigation visual interference on benchmark pages.

# 6.0.7 — Benchmark True North
- Replaced the benchmark hero fallback with a clean project image crop; benchmark screenshots are never used as live hero artwork.
- Promoted Legacy's saved benchmark to an explicit compiled renderer profile instead of a styling toggle.
- Locked masthead, hero, trust rail, six-service grid, and How It Works proportions to the approved responsive benchmark.
- Rewrote benchmark service microcopy to match the approved reference while preserving the underlying service actions.
- Strengthened cache/build identity so stale 6.0.4/6.0.5 shells are easier to detect and replace.

# 6.0.7 — Benchmark True North
- Replaced the boolean-only benchmark styling approach with a compiled Legacy benchmark profile that owns masthead, hero, trust rail, services grid, process band, and device breakpoints.
- Added a benchmark-derived Legacy hero fallback from the approved project reference so the live composition no longer falls back to the unrelated blueprint/wrench photograph when no Welcome Hero Graphic is assigned.
- Replaced inherited generic trust-card markup in benchmark mode with dedicated benchmark trust markup so proof items render directly on the dark rail instead of as white pills.
- Fully isolated benchmark service-card and process-band typography/layout from older contractor styles to prevent title/description collisions.
- Preserved exact canonical logo authority, project isolation, Control Center asset precedence, private-test contact safety, and benchmark reference-only semantics.
...
- Tightened hero image treatment toward the approved benchmark without using the benchmark itself as customer artwork.
- On coarse-pointer/touch devices, internal Dark Sky/Test Deck navigation collapses behind the compact dock so controls do not cover customer content.
- Canonical logo, benchmark reference, project data, isolation, and test-safety contracts remain unchanged.

- Activated project Design Benchmark / Reference as a composition contract rather than passive storage.
- Legacy Plumbing is the first flagship benchmark implementation: exact canonical logo, benchmark masthead, unified photo-led hero, integrated trust rail, compact service grid, and benchmark How It Works closure.
- Saved project benchmark assets select benchmark-aware Customer Site Kit behavior without ever becoming live artwork themselves.
- Canonical logo remains authoritative and is rendered intact; Control Center replacement remains the only intentional logo-change path.
...
- Advanced runtime and cache identity to 6.0.7.
...
- Added a reusable project brand card for hero, header, and confidence surfaces; real project graphics remain authoritative when provided.
...
- Advanced runtime/cache identity to 6.0.7.
...
- Updated runtime/build references and cache identity to 6.0.7.
