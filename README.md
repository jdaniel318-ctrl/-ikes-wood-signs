...
Kiosk Watch hardens the fleet owner spine for scale: exact-vessel owner settings sync through a dedicated bounded write before slower order reconciliation, Bootstrap Witness reports authority/settings/order phases separately, and owner work never waits on durability reconciliation. Kiosk/customer persistence remains a separate fleet operating mode concern from privileged owner sessions.
