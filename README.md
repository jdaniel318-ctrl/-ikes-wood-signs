...
## v2.9.53 — Cinematic Safe Rebuild
This release corrects the v2.9.52 launch regression.

### What was wrong with v2.9.52
The cinematic conversion was implemented too aggressively: it disabled/hid major Captain's Quarters structural containers and made the chamber presentation depend directly on a new background asset. The package also inherited a very old service-worker cache name and a precache list containing a file that no longer exists (`assets/ike_logo.jpg`). That combination was too fragile for Safari/GitHub Pages deployment.

### v2.9.53 corrections
- Rebased on the last known-good v2.9.51 application code.
- Cinematic cabin is now progressive enhancement, never a boot dependency.
- The v2.9.51 chamber remains fully functional if the cinematic image fails to load.
- No functional parent container is disabled.
- Only decorative legacy layers are suppressed after the cinematic image successfully loads.
- Removed the unneeded 1.8 MB source PNG; only the optimized ~394 KB cinematic JPG ships.
- Rebuilt Service Worker as v2.9.53 with an existing-assets-only cache list.
