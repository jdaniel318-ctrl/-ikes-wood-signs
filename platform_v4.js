/* Dark Sky 4.0.5 — Ghost Ship Sweep platform generation layer */
(function(g){
'use strict';
const VERSION='4.0.5';
...
function completeCommissioning(projects=[],meta={}){
 ...
 core()?.audit?.({actorRole:'system',category:'migration',action:'v4.0.5.commissioning.ghost_sweep_sealed',detail:`${sealed}/${expected} project envelopes read-back verified`});
 return row;
}
function markCommissioningFailed(projects=[],reason='Commissioning invariant failed'){
 ...
 write(KEYS.migration,row);core()?.audit?.({actorRole:'system',category:'migration',action:'v4.0.5.commissioning.failed',detail:row.failure});diagnostic('commissioning.failed',row.failure,{projectCount:expected});return row;
}
